...
Version 0.26 adds a truthful Cash Flow Home preset with dedicated six-month inflow, outflow, and net-cash-flow facts. Card purchases remain accrual expenses while the later bank settlement appears once as cash outflow, so the two dates are never combined into duplicate spending.
