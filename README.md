...
- [Explicit import account mapping](docs/EXPLICIT_IMPORT_ACCOUNT_MAPPING.md) for generic Japanese bank, PayPay, Rakuten Card, Amazon Mastercard, JCB MyJCB, SMBC Vpass, and the strict [AEON finalized-statement contract](docs/AEON_CARD_IMPORT.md), with adapter-compatible account filtering, per-preview selection, and no default or name-based inference. The AEON fixture is screen-derived synthetic because the provider does not publish a literal consumer CSV schema.
