...
| `npm run lint` | Passed. |
| `npm run test:functional` | Passed; 133 files, 935/935 tests. |
...
| opaque updater-signing environment injection plus `npm run desktop:build:mac:ci` | Pending after the identity-hardening commit; the required opaque injection was unavailable to this execution context. |
| `npm run test:packaged` | Pending the new-source rebuild; the smoke rejects the previous artifact identity as stale before launch. |
| `npm run test:dmg` | Pending the new-source rebuild; the smoke rejects the previous artifact identity as stale before mount. |

The earlier artifact set was valid for its recorded source identity, but this hardening changes that identity. Its hashes and package/DMG successes are intentionally not carried forward. A controller with the previously verified opaque signing injection must perform one clean rebuild and then replace these pending rows with new hashes and fresh smoke, privacy, architecture, and codesign evidence. The injection mechanism, location, and key material remain outside tracked documentation.

As a negative gate, both `npm run test:packaged` and `npm run test:dmg` were run against the previous artifacts after source hardening. Each exited 1 with `Native build input identity mismatch` before launch or mount, proving the stale artifacts cannot satisfy the current gate.
...
- Native application, updater archive, detached signature, DMG, and `kakeflow-build-identity.json`: pending the clean arm64 rebuild for this source identity. No prior-source hash is current evidence.

Fresh codesign evidence is also pending that rebuild. The supported boundary remains ad-hoc signing with no Developer ID or Apple notarization claim; these local artifacts are not a release and are not presented as a frictionless production installer.

## Privacy and claim inspection

The fresh PWA build, Poppler manifest/checklist/PNGs, Playwright result, connector batch test sources, and this evidence were inspected for credential values, authorization-code values, cursor values, personal absolute paths, provider folder/label values, personal emails, real financial payloads, premium branding, and unsupported direct-institution or installer claims. The equivalent native package inspection is pending the new-source rebuild and is not inferred from the prior artifact set.
...
Each physical checkout/target has an atomic lock. Any live PID, including possible PID reuse, fails closed. Explicit dead-owner recovery atomically renames the observed lock to a unique quarantine, validates the same owner token after rename, deletes only that quarantine, and restores or preserves a raced replacement instead of deleting the live lock path. Active or malformed locks still require inspection of the exact reported path. The success identity is written under the lock only after stable inputs and complete outputs. Any build, publication, or final lock-release failure removes a newly published identity and returns combined primary/cleanup errors; app and DMG smokes reject missing, stale-source, or byte-mismatched identity before launch or mount.

A prior whole-bundle scan exposed two generated dependencies that an executable-only scan missed. The PWA core generator uses its own neutral Cargo target and compile-time Rust remapping; its contract rejects personal roots in both the tracked WASM and production PWA WASM. The macOS OCR stage builds under a neutral temporary vcpkg root, disables restoration from a user binary cache, and makes OCR verification reject personal roots in Tesseract before packaging. File counts and zero-finding claims for the replacement app, updater, and DMG remain pending rather than being copied from the invalidated artifact identity.
