...
| opaque updater-signing environment injection plus `npm run desktop:build:mac:ci` | Pending after the fail-closed lock correction; the controller must rebuild without exposing or rediscovering the opaque injection. |
...
The artifact set recorded at `5b6150e` was valid only for its prior build-input identity. This lock correction changes that identity, so its hashes and package/DMG successes are intentionally not carried forward. The controller must perform one clean arm64 rebuild and rerun package, DMG, privacy, architecture, and codesign checks. The injection mechanism, location, and key material remain outside tracked documentation.

Both `npm run test:packaged` and `npm run test:dmg` were rerun against the invalidated `5b6150e` artifacts after this lock correction. Each exited 1 with `Native build input identity mismatch` before app launch or DMG mount. Positive runtime and mount results remain pending the replacement artifacts.
