...
describe('Japanese brokerage transaction adapter', () => {
  ...
  it('does not claim incomplete Monex US-stock screen fields as a generic brokerage CSV', () => {
    const monex = [
    ...
    expect(japaneseBrokerageTransactionsAdapter.detect({ text: monex, filename: 'monex_us_tradehistory.csv' }).score).toBe(0)
    expect(detectImportAdapter({ text: monex, filename: 'monex_us_tradehistory.csv' })).toBeNull()
  })
})
