...
function StateAction({ item, busy, onPreview, onOpenImport, onRetry, onDiscard }: {
  ...
  if (action === 'PROCESS') return <div className="capture-row-actions"><button className="primary-btn" disabled={busy} onClick={() => onPreview(item)}><FileImage size={16} />{busy ? '原本を読込中…' : '原本画像を確認'}</button><button className="text-btn capture-discard" disabled={busy} onClick={() => onDiscard(item)}>破棄</button></div>
  if (action === 'IMPORT') return <button className="secondary-btn" disabled={busy} onClick={onOpenImport}>取引候補を確認</button>
  if (action === 'RETRY') return <div className="capture-row-actions"><button className="secondary-btn" disabled={busy} onClick={() => onRetry(item)}><RotateCcw size={16} />{busy ? '再試行中…' : 'もう一度読み取る'}</button><button className="text-btn capture-discard" disabled={busy} onClick={() => onDiscard(item)}>破棄</button></div>
  return null
}
...
export function CaptureInboxPage({ householdId, items, loading, busyArtifactId, token, preview, previewBusy, notice, background = null, backgroundInterval = 30, backgroundBusy = false, showConnectorControls = true, onBackgroundIntervalChange = () => undefined, onEnableBackground = () => undefined, onDisableBackground = () => undefined, onRunBackgroundNow = () => undefined, onTokenChange, onPreview, onClosePreview, onRefresh, onProcess, onOpenImport, onRetry, onDiscard = () => undefined, localBusy = false, localPendingFiles = [], ocrProgress = {}, ocrConfidence = {}, watchedFolderPath = null, onLocalFiles = () => undefined, onConfigureWatchedFolder = () => undefined }: CaptureInboxPageProps) {
  ...
  return <>
    ...
    <section className={`capture-local-intake${dragging ? ' is-dragging' : ''}`} aria-label="ローカルレシート取り込み" onDragEnter={(event) => { event.preventDefault(); setDragging(true) }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'copy' }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDragging(false) }} onDrop={(event) => { event.preventDefault(); setDragging(false); onLocalFiles(Array.from(event.dataTransfer.files)) }}>
      <FileImage size={24} aria-hidden="true" />
      <div><strong>レシート画像をここにドラッグ＆ドロップ</strong><span>JPEG / PNG / PDF · 原本は暗号化してこの端末だけに保存されます</span></div>
      <button className="primary-btn" type="button" disabled={!householdId || localBusy} onClick={() => localInput.current?.click()}>{localBusy ? '保存中…' : 'ファイルを選択…'}</button>
      <input ref={localInput} className="visually-hidden" aria-label="撮影 Inboxへ追加するレシート画像" type="file" accept="image/png,image/jpeg,application/pdf,.png,.jpg,.jpeg,.pdf" multiple onChange={(event) => { const files = Array.from(event.currentTarget.files ?? []); event.currentTarget.value = ''; onLocalFiles(files) }} />
      <button className="secondary-btn" type="button" disabled={!householdId || localBusy} onClick={onConfigureWatchedFolder}>監視フォルダを設定…</button>
      <small>{watchedFolderPath ? `監視中: ${watchedFolderPath}` : '監視フォルダは未設定'} ・ 明細書CSVは <button type="button" className="text-btn" onClick={onOpenImport}>Import Inbox</button></small>
    </section>
    <p className="capture-boundary">画像を受信しても、台帳には反映されません。原本とOCRの抽出結果を確認してからImport Inboxへ昇格します。</p>
    {showConnectorControls && <section className="panel capture-receive-controls" aria-label="モバイルからの受信">
    ...
    {showConnectorControls && <section className="panel capture-background-controls" aria-label="原本画像の自動受信">
      <div>
        <h2>アプリ起動中の自動受信</h2>
    ...
    <section className="panel capture-inbox-panel" aria-busy={loading || Boolean(busyArtifactId)}>
      ...
      {!householdId ? <p className="empty-state">先に家族スペースを選択してください。</p> : items.length === 0 && localPendingFiles.length === 0 ? <div className="capture-empty"><FileImage size={36} /><h3>届いた原本はありません</h3><p>JPEG、PNG、PDFを追加するか、監視フォルダを設定してください。</p></div> : <div className="capture-list">{localPendingFiles.map((filename, index) => <article className="capture-row state-RECEIVED capture-row-pending" key={`${filename}:${index}`}><div className="capture-thumb" aria-hidden="true"><FileImage size={23} /></div><div className="capture-copy"><strong>{filename}</strong><span>この端末から追加</span><small>暗号化原本のSHA-256を計算しています</small></div><span className="capture-state">受信済み（ハッシュ計算中）</span></article>)}{items.map((item) => {
        ...
        return <article className={`capture-row state-${item.state}`} key={item.artifactId}>
          ...
          <div className="capture-copy"><strong>{item.originalFilename}</strong><span>{item.senderMembershipId === 'watched-folder' ? '監視フォルダ' : item.senderMembershipId === 'local-desktop' ? 'この端末から追加' : item.senderMemberName ? `${item.senderMemberName}さんから` : 'モバイル転送'}・{dateTime(item.capturedAt)}</span><small>{item.audienceVisibility === 'SHARED' ? '世帯共有' : `個人・${item.audienceMemberName ?? item.senderMemberName ?? '本人'}`} ・ {size(item.byteSize)} ・ 受信 {dateTime(item.receivedAt)}{ocrConfidence[item.artifactId] != null ? ` ・ OCR ${Math.round(ocrConfidence[item.artifactId] / 100)}%` : ''}</small>
          ...
          <span className={`capture-state state-${item.state}`}>{ocrProgress[item.artifactId] != null ? `OCR実行中 ${ocrProgress[item.artifactId]}%` : stateLabels[item.state]}</span>
