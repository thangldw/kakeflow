...
describe('forecast action platform', () => {
  it('invokes the household-scoped command and validates the response', async () => {
    ...
    const result = await createForecastActionPlatform(invoke).query({ householdId: 'family', accountGroupId: 'daily', attributionScope: { kind: 'MEMBER', memberId: 'taro' }, asOf: '2026-07-13' })
    expect(invoke).toHaveBeenCalledWith('forecast_action_query', { request: { householdId: 'family', accountGroupId: 'daily', attributionScope: { kind: 'MEMBER', memberId: 'taro' }, asOf: '2026-07-13' } })
