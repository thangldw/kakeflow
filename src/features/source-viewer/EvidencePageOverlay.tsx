...
export function EvidencePageOverlay({ pageNumber, regions, image, selectedRegionIndexes = [], onSelectRegion }: EvidencePageOverlayProps) {
  ...
  const width = image?.width ?? inferred.width
  const height = image?.height ?? inferred.height
  if (located.length === 0) return <p className="evidence-overlay-empty">Page {pageNumber} には表示できる座標がありません。</p>
