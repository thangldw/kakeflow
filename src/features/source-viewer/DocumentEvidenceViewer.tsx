...
export function DocumentEvidenceViewer({ evidence, filename, onSelectRegion }: DocumentEvidenceViewerProps) {
  ...
  return <article className="evidence-viewer" aria-labelledby="evidence-title">
    ...
    <section className="evidence-pages" aria-labelledby="evidence-pages-title"><header><div><p>Located evidence</p><h3 id="evidence-pages-title">ページ・領域</h3></div><span>{evidence.pages.length}ページ</span></header>
      {evidence.pages.length === 0 ? <p className="evidence-empty">この抽出結果には座標情報がありません。</p> : evidence.pages.map((page) => <section className="evidence-page" key={page.pageNumber} aria-labelledby={`evidence-page-${page.pageNumber}`}><header><h4 id={`evidence-page-${page.pageNumber}`}>Page {page.pageNumber}</h4><span>{page.regions.length} regions</span></header><ol>{page.regions.map((region, index) => {
          ...
          return <li key={`${region.provenance}-${index}`}>{onSelectRegion ? <button type="button" onClick={() => onSelectRegion(page.pageNumber, region, index)} aria-label={`Page ${page.pageNumber} region ${index + 1}を表示`}>{content}</button> : <div className="region-static">{content}</div>}</li>
        })}</ol></section>)}
    </section>
