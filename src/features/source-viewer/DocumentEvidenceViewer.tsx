...
export function DocumentEvidenceViewer({ evidence, filename, pageImages, pdfSource, pdfPageLoader, onSelectRegion }: DocumentEvidenceViewerProps) {
  ...
  return <div className="evidence-viewer-backdrop" role="presentation"><article className="evidence-viewer" role="dialog" aria-modal="true" aria-labelledby={evidenceTitleId}>
    ...
    {receipt && <section className="receipt-evidence" aria-labelledby="receipt-evidence-title">
      <header><div><p>レシート</p><h3 id="receipt-evidence-title">{receipt.merchant ?? '店舗名未確認'}</h3></div><div><span>{receipt.occurredOn ?? '日付未確認'}</span><strong>{receipt.totalAmountJpy == null ? '金額未確認' : yen(receipt.totalAmountJpy)}</strong></div></header>
      <div className="receipt-adjustments" aria-label="税・値引・ポイント">
        {receipt.subtotalJpy != null && <span><b>小計</b>{yen(receipt.subtotalJpy)}</span>}
        {receipt.taxes.map((tax, index) => <span key={`${tax.ratePercent}-${index}`}><b>消費税 {tax.ratePercent}%</b>{tax.taxAmountJpy != null ? yen(tax.taxAmountJpy) : '税額不明'}<small>行 {tax.provenance.lineNumber} · {confidence(tax.confidenceBps)}</small></span>)}
        {receipt.couponEvidence.length > 0 ? receipt.couponEvidence.map((item, index) => <span key={`coupon-${item.provenance.lineNumber}-${index}`}><b>クーポン・値引</b>{item.amountJpy === null ? '金額不明' : `−${yen(item.amountJpy)}`}<small>行 {item.provenance.lineNumber} · {confidence(item.confidenceBps)}</small></span>) : receipt.couponAmountJpy != null && <span><b>クーポン・値引</b>−{yen(receipt.couponAmountJpy)}</span>}
        {receipt.pointsUsedEvidence.length > 0 ? receipt.pointsUsedEvidence.map((item, index) => <span key={`points-${item.provenance.lineNumber}-${index}`}><b>ポイント利用</b>{item.amountJpy === null ? '金額不明' : `−${yen(item.amountJpy)}`}<small>行 {item.provenance.lineNumber} · {confidence(item.confidenceBps)}</small></span>) : receipt.pointsUsedJpy != null && <span><b>ポイント利用</b>−{yen(receipt.pointsUsedJpy)}</span>}
      ...
      {receipt.items.length > 0 && <div className="receipt-items-wrap"><table className="receipt-items"><caption>レシート明細と抽出元</caption><thead><tr><th scope="col">品目</th><th scope="col">数量</th><th scope="col">税率</th><th scope="col">金額</th><th scope="col">根拠</th></tr></thead><tbody>{receipt.items.map((item, index) => <tr key={`${item.description}-${index}`}><th scope="row">{item.description}</th><td>{item.quantity ?? '—'}</td><td>{item.taxRatePercent === null ? '未確認' : `${item.taxRatePercent}%`}</td><td>{yen(item.amountJpy)}</td><td>行 {item.provenance.lineNumber} · 領域 {item.provenance.regionIndexes.length ? item.provenance.regionIndexes.join(', ') : '—'} · {confidence(item.confidenceBps)}</td></tr>)}</tbody></table></div>}
    </section>}

    <section className="evidence-pages" aria-labelledby="evidence-pages-title"><header><div><p>位置付き証跡</p><h3 id="evidence-pages-title">ページ・領域</h3></div><span>{evidence.pages.length}ページ</span></header>
      {evidence.pages.length === 0 ? <p className="evidence-empty">この抽出結果にはページ情報がありません。</p> : <div className="evidence-workspace">
        <nav className="evidence-page-rail" aria-label="原本ページ"><strong>ページ</strong>{evidence.pages.map((page) => <button key={page.pageNumber} type="button" className={page.pageNumber === selectedPage?.pageNumber ? 'selected' : ''} aria-current={page.pageNumber === selectedPage?.pageNumber ? 'page' : undefined} aria-label={`ページ${page.pageNumber}を表示`} onClick={() => { setSelectedPageNumber(page.pageNumber); setSelected(null) }}><span className="evidence-page-thumbnail"><span>ページ</span><b>{page.pageNumber}</b><small>{page.regions.length} 領域</small></span><span>ページ {page.pageNumber}</span></button>)}</nav>
        {selectedPage && <section className="evidence-page evidence-page-canvas" aria-labelledby={`${evidenceTitleId}-page-${selectedPage.pageNumber}`}><header><h4 id={`${evidenceTitleId}-page-${selectedPage.pageNumber}`}>ページ {selectedPage.pageNumber}</h4><span>{pendingPdfPages.includes(selectedPage.pageNumber) ? '原本を描画中…' : failedPdfPages.includes(selectedPage.pageNumber) ? '原本プレビューを表示できません' : `${selectedPage.regions.length} 領域 · ${confidence(selectedPage.confidenceBps ?? evidence.confidenceBps)}`}</span></header>{(selectedPage.issues?.length ?? 0) > 0 && <p className="evidence-overlay-empty" role="status">{selectedPage.issues.join(' / ')}</p>}<EvidencePageOverlay pageNumber={selectedPage.pageNumber} regions={selectedPage.regions} image={resolvedPageImages[selectedPage.pageNumber]} widthPixels={selectedPage.widthPixels} heightPixels={selectedPage.heightPixels} selectedRegionIndexes={selected?.pageNumber === selectedPage.pageNumber ? [selected.regionIndex] : []} onSelectRegion={(region, index) => selectRegion(selectedPage.pageNumber, region, index)} /></section>}
        {selectedPage && <aside className="evidence-region-panel" aria-label={`ページ ${selectedPage.pageNumber} の抽出領域`}><header><div><p>抽出済み領域</p><h4>抽出領域</h4></div><strong>{confidence(selectedPage.confidenceBps ?? evidence.confidenceBps)}</strong></header><ol>{selectedPage.regions.map((region, index) => {
          ...
          return <li key={`${region.provenance}-${index}`} className={selected?.pageNumber === selectedPage.pageNumber && selected.regionIndex === index ? 'selected' : ''}><button type="button" onClick={() => selectRegion(selectedPage.pageNumber, region, index)} aria-label={`ページ ${selectedPage.pageNumber} の領域 ${index + 1} を表示`}>{content}</button></li>
