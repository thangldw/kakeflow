...
export function InvestmentFxSummary({
  ...
  if (loading && !report) return <section className="panel investment-fx-summary" aria-busy="true"><p className="investment-fx-loading">{localize("円換算を計算しています…")}</p></section>
  if (!report) {
    return <section className="panel investment-fx-summary investment-fx-notice" role="status">
      <strong>{notice === 'MISSING_RATE' ? localize("円換算に必要な為替レートが不足しています") : localize("円換算を読み込めませんでした")}</strong>
      <span>{localize("元通貨の保有残高と実績はそのまま利用できます。換算値を推測して補完することはありません。")}</span>
    </section>
  }

  const converted = report.convertedTotals
  return <section className="panel investment-fx-summary" aria-label={localize("投資実績の円換算")}>
    <div className="panel-head">
      <div><h2>{localize("投資実績の円換算")}</h2><p>{localize("元通貨を保持したまま、確認済みレートだけで集計")}</p></div>
      <span>{report.fxAsOf} {localize("レート")}</span>
    </div>
    <div className="investment-fx-kpis">
      <article><span>{localize("実現損益")}</span><strong className={converted.realizedPnl >= 0 ? 'amount-positive' : ''}>{amount(converted.currency, converted.realizedPnl)}</strong></article>
      <article><span>{localize("配当")}</span><strong>{amount(converted.currency, converted.dividendGross)}</strong></article>
      <article><span>{localize("買付 / 売却")}</span><strong>{amount(converted.currency, converted.buyGross)} / {amount(converted.currency, converted.sellGross)}</strong></article>
      <article><span>{localize("手数料・税")}</span><strong>{amount(converted.currency, converted.fees + converted.taxes)}</strong></article>
    </div>
    <div className="investment-fx-native" aria-label={localize("元通貨の実績")}>
      {report.originalTotalsByCurrency.map((total) => <span key={total.currency}><b>{total.currency}</b> {localize("実現損益")} {amount(total.currency, total.realizedPnl)}</span>)}
    </div>
    <div className="investment-fx-provenance">
      <h3>{localize("換算レートと出典")}</h3>
      {report.conversions.map((conversion) => <article key={`${conversion.originalCurrency}-${conversion.rateId}`}>
        ...
        <small>{conversion.rateId === 'IDENTITY' ? localize("同一通貨") : conversion.sourceDocumentId ? localize(`原本 ${conversion.sourceDocumentId}${conversion.sourceRow == null ? '' : ` ・ 行 ${conversion.sourceRow}`}`) : conversion.sourceKind}{conversion.inverted ? localize(" ・ 逆数利用") : ''}</small>
      </article>)}
    </div>
  </section>
}
