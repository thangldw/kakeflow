...
describe('investment performance platform boundary', () => {
  it('queries and validates FIFO holdings with auditable event IDs', async () => {
    const invoke = vi.fn(async () => ({
      ...
      realizedAllocations: [allocation], uncoveredSales: [], skippedEventIds: [],
  ...
  it('rejects malformed native responses instead of coercing them', async () => {
    const invoke = vi.fn(async () => ({ asOf: '2026-12-31', costBasisMethod: 'AVERAGE', positions: [], openLots: [], realizedAllocations: [], uncoveredSales: [], skippedEventIds: [] })) as unknown as InvestmentPerformanceInvoke
