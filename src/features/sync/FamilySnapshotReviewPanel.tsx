...
export function FamilySnapshotReviewPanel({ householdId, revision = 0 }: Props) {
  ...
  return <section className="panel family-snapshot-review" aria-busy={busy}>
    ...
    {!review ? <p className="empty-state">確認待ちの家族データはありません。</p> : <>
      ...
      {review.records.some((record) => record.entityKind === 'RECURRING_SERIES_PREFERENCES') && <p className="family-review-config-warning" role="status">定期支出の確認状態は世帯共有です。正規化した支払先名と「確認済み・対象外」の判断が共有され、予測と固定費表示に影響しますが、過去の取引は書き換えません。</p>}
