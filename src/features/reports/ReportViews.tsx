...
const monthLabel = (month: string) => {
  ...
  return match ? localize(`${match[1]}年${Number(match[2])}月`) : month
}
...
function DataQualityWarning({ quality, onOpenImports }: { readonly quality: DataQualitySummaryDto; readonly onOpenImports?: () => void }) {
  ...
  const messages = [
    quality.reviewRequiredImports > 0 ? localize(`${quality.reviewRequiredImports}件が確認待ち`) : '',
    quality.failedImports > 0 ? localize(`${quality.failedImports}件が失敗`) : '',
    quality.inProgressImports > 0 ? localize(`${quality.inProgressImports}件を処理中`) : '',
    quality.staleDays != null && quality.staleDays >= 7 ? localize(`最終取込から${quality.staleDays}日`) : '',
  ].filter(Boolean)
  return <aside className="report-quality-warning" role="status"><div><strong>{localize("データの完全性を確認してください")}</strong><span>{messages.join(' ・ ')}</span></div>{onOpenImports && <button type="button" onClick={onOpenImports}>{localize("インポートを確認")}</button>}</aside>
}
...
function CalendarEvent({ date, event, onSelect }: { readonly date: string; readonly event: CalendarEventDto; readonly onSelect?: (date: string, event: CalendarEventDto) => void }) {
  const content = <><span>{localize(eventLabels[event.kind])}</span><strong>{event.title}</strong>{event.amountJpy != null && <em>{yen(event.amountJpy)}</em>}</>
...
export function FinancialCalendarView({ data, basis = 'ACCRUAL', onBasisChange, onSelectDate, onSelectEvent, onOpenImports }: FinancialCalendarViewProps) {
  ...
  return <div className="report-view financial-calendar-view">
    <header className="report-view-head"><div><p>{localize("家計カレンダー")}</p><h2>{monthLabel(data.month)}</h2><span>{data.asOf} {localize("現在・計算対象の確定取引のみ（集計対象外を除く）。資金移動レポートも同条件ですが、口座残高は集計対象外を含みます。")}</span></div><div className="report-segmented" aria-label={localize("カレンダーの計上基準")}><button type="button" aria-pressed={basis === 'ACCRUAL'} onClick={() => onBasisChange?.('ACCRUAL')}>{localize("発生ベース")}</button><button type="button" aria-pressed={basis === 'CASH'} onClick={() => onBasisChange?.('CASH')}>{localize("資金移動")}</button></div></header>
    <DataQualityWarning quality={data.dataQuality} onOpenImports={onOpenImports} />
    <section className="report-summary-grid" aria-label={localize("月間サマリー")}>
      <SummaryCard label={basis === 'ACCRUAL' ? localize("収入") : localize("入金")} value={yen(monthIncome)} detail={localize(`${data.days.reduce((sum, day) => sum + day.postedTransactionCount, 0)}件の計算対象の確定取引`)} />
      <SummaryCard label={basis === 'ACCRUAL' ? localize("支出") : localize("出金")} value={yen(monthExpense)} detail={localize(`差引 ${signedYen(monthIncome - monthExpense)}`)} />
      <SummaryCard label={localize("予算")} value={yen(data.budget.remainingJpy)} detail={localize(`${data.budget.categoryCount}カテゴリー・超過 ${data.budget.overBudgetCount}件`)} progress={data.budget.utilizationBps ?? undefined} warning={data.budget.overBudgetCount > 0} />
      <SummaryCard label={localize("無支出日")} value={localize(`${noSpendDays}日`)} detail={localize(`貯蓄目標 ${data.goals.activeCount}件`)} />
    </section>
    <div className="calendar-table-wrap">
      <table className="financial-calendar-table">
        <caption className="report-visually-hidden">{monthLabel(data.month)}{localize("の日別収支カレンダー")}</caption>
        <thead><tr>{weekdays.map((weekday) => <th key={localize(weekday)} scope="col">{localize(weekday)}</th>)}</tr></thead>
        <tbody>{Array.from({ length: 6 }, (_, week) => <tr key={week}>{cells.slice(week * 7, week * 7 + 7).map((cell) => {
          ...
          return <td className={`calendar-day${day?.noSpendDay ? ' calendar-day--no-spend' : ''}`} key={cell.key}>
            <button type="button" className="calendar-date" aria-label={localize(`${date}の取引を表示`)} onClick={() => onSelectDate?.(date)}><b>{cell.dayOfMonth}</b>{day?.noSpendDay && <span>{localize("支出なし")}</span>}</button>
            <div className="calendar-day-totals">{income > 0 && <span className="calendar-income">+{yen(income)}</span>}{expense > 0 && <span className="calendar-expense">−{yen(expense)}</span>}{day && day.postedTransactionCount > 0 && <small>{day.postedTransactionCount}{localize("件")}</small>}</div>
    ...
    <footer className="calendar-legend" aria-label={localize("カレンダー凡例")}><span><i className="legend-income" />{localize("入金")}</span><span><i className="legend-expense" />{localize("支出・出金")}</span><span><i className="legend-card" />{localize("カード予定")}</span><span><i className="legend-no-spend" />{localize("無支出日")}</span></footer>
  </div>
}
...
function DriverTable({ title, kind, rows, onSelect }: { readonly title: string; readonly kind: 'CATEGORY' | 'MERCHANT'; readonly rows: readonly MonthlyDriverDto[]; readonly onSelect?: (kind: 'CATEGORY' | 'MERCHANT', driver: MonthlyDriverDto) => void }) {
  return <section className="report-section driver-section"><div className="report-section-head"><div><h3>{title}</h3><p>{localize("比較期間からの支出増減")}</p></div></div>{rows.length === 0 ? <p className="report-empty">{localize("比較できる支出はありません。")}</p> : <div className="report-table-wrap"><table><thead><tr><th scope="col">{kind === 'CATEGORY' ? localize("カテゴリー") : localize("支払先")}</th><th scope="col">{localize("当月")}</th><th scope="col">{localize("比較期間")}</th><th scope="col">{localize("増減")}</th></tr></thead><tbody>{rows.map((row, index) => {
    const label = row.name ?? row.merchant ?? localize("名称未設定")
...
export function MonthlyReportView({ data, comparison = 'PRIOR_MONTH', savingCsv = false, savingXlsx = false, savingPdf = false, onComparisonChange, onSelectDriver, onOpenBudget, onOpenGoals, onOpenImports, onOpenReconciliation, onSaveCsv, onSaveXlsx, onSavePdf }: MonthlyReportViewProps) {
  ...
  const comparisonLabel = comparison === 'PRIOR_MONTH' ? localize("前月比") : localize("前年同月比")
  ...
  return <div className="report-view monthly-report-view">
    <header className="report-view-head"><div><p>{localize("月次家計レビュー")}</p><h2>{monthLabel(data.period)}</h2><span>{data.current.postedTransactionCount}{localize("件の計算対象の確定取引（集計対象外を除く）・データ品質基準日")} {data.asOf}</span></div><div className="monthly-review-head-actions"><div className="report-segmented" aria-label={localize("レポートの比較期間")}><button type="button" aria-pressed={comparison === 'PRIOR_MONTH'} onClick={() => onComparisonChange?.('PRIOR_MONTH')}>{localize("前月比")}</button><button type="button" aria-pressed={comparison === 'PRIOR_YEAR'} onClick={() => onComparisonChange?.('PRIOR_YEAR')}>{localize("前年同月比")}</button></div><div className="monthly-review-export-actions">{onSaveCsv && <button type="button" className="secondary-btn" disabled={savingExport} onClick={onSaveCsv}>{savingCsv ? localize("CSVを作成中…") : localize("月次CSVを保存")}</button>}{onSaveXlsx && <button type="button" className="primary-btn" disabled={savingExport} onClick={onSaveXlsx}>{savingXlsx ? localize("Excelを作成中…") : localize("月次Excelを保存")}</button>}{onSavePdf && <button type="button" className="secondary-btn" disabled={savingExport} onClick={onSavePdf}>{savingPdf ? localize("PDFを作成中…") : localize("月次PDFを保存")}</button>}</div></div></header>
    <DataQualityWarning quality={data.dataQuality} onOpenImports={onOpenImports} />
    <section className="report-kpi-grid" aria-label={localize("月次KPI")}>
      <article><span>{localize("収入")}</span><strong>{yen(data.current.incomeJpy)}</strong><Delta delta={delta.income} /></article>
      <article><span>{localize("支出")}</span><strong>{yen(data.current.expenseJpy)}</strong><Delta delta={delta.expense} inverse /></article>
      <article><span>{localize("貯蓄")}</span><strong>{yen(data.current.savingsJpy)}</strong><Delta delta={delta.savings} /></article>
      <article><span>{localize("貯蓄率")}</span><strong>{data.current.savingsRateBps == null ? '—' : percent(data.current.savingsRateBps)}</strong><small>{comparisonLabel}</small></article>
    </section>
    <div className="report-driver-grid"><DriverTable title={localize("支出を動かしたカテゴリー")} kind="CATEGORY" rows={data.topCategoryDrivers} onSelect={onSelectDriver} /><DriverTable title={localize("支出を動かした支払先")} kind="MERCHANT" rows={data.topMerchantDrivers} onSelect={onSelectDriver} /></div>
    <section className="report-health-grid" aria-label={localize("家計の進捗とデータ品質")}>
      <SummaryCard label={localize("予算進捗")} value={localize(`${optionalPercent(data.budget.utilizationBps)} 使用`)} detail={localize(`${yen(data.budget.actualJpy)} / ${yen(data.budget.budgetJpy)}・超過 ${data.budget.overBudgetCount}件`)} progress={data.budget.utilizationBps ?? undefined} warning={data.budget.overBudgetCount > 0} action={onOpenBudget && <button type="button" onClick={onOpenBudget}>{localize("予算を見る")}</button>} />
      <SummaryCard label={localize("貯蓄目標")} value={yen(data.goals.savedJpy)} detail={localize(`残り ${yen(data.goals.remainingJpy)}・期限間近 ${data.goals.dueWithinPeriodCount}件`)} progress={goalProgress} warning={data.goals.dueWithinPeriodCount > 0} action={onOpenGoals && <button type="button" onClick={onOpenGoals}>{localize("目標を見る")}</button>} />
      <SummaryCard label={localize("データ完全性")} value={optionalPercent(data.dataQuality.importCompletionBps)} detail={localize(`${data.dataQuality.postedImports}/${data.dataQuality.totalImports}件反映・最終取込 ${data.dataQuality.latestImportedAt ?? localize("なし")}`)} progress={data.dataQuality.importCompletionBps ?? undefined} warning={data.dataQuality.hasUnresolvedImports} action={onOpenImports && <button type="button" onClick={onOpenImports}>{localize("取込状況を見る")}</button>} />
      <SummaryCard label={localize("カード照合")} value={localize(`${data.reconciliation.fullyReconciled}/${data.reconciliation.totalStatements}件`)} detail={localize(`引落 ${yen(data.reconciliation.paymentTotalJpy)}・要確認 ${data.reconciliation.mismatchCount + data.reconciliation.unmatched}件`)} progress={reconciledBps} warning={data.reconciliation.mismatchCount + data.reconciliation.unmatched > 0} action={onOpenReconciliation && <button type="button" onClick={onOpenReconciliation}>{localize("照合を見る")}</button>} />
    </section>
    <section className="report-section comparison-table"><div className="report-section-head"><div><h3>{localize("収支比較")}</h3><p>{localize("同じ定義の計算対象の確定台帳を期間比較（集計対象外を除く）")}</p></div></div><div className="report-table-wrap"><table><thead><tr><th scope="col">{localize("指標")}</th><th scope="col">{localize("当月")}</th><th scope="col">{localize("前月")}</th><th scope="col">{localize("前年同月")}</th></tr></thead><tbody><tr><th scope="row">{localize("収入")}</th><td>{yen(data.current.incomeJpy)}</td><td>{yen(data.priorMonth.incomeJpy)}</td><td>{yen(data.priorYear.incomeJpy)}</td></tr><tr><th scope="row">{localize("支出")}</th><td>{yen(data.current.expenseJpy)}</td><td>{yen(data.priorMonth.expenseJpy)}</td><td>{yen(data.priorYear.expenseJpy)}</td></tr><tr><th scope="row">{localize("貯蓄")}</th><td>{yen(data.current.savingsJpy)}</td><td>{yen(data.priorMonth.savingsJpy)}</td><td>{yen(data.priorYear.savingsJpy)}</td></tr><tr><th scope="row">{localize("取引件数")}</th><td>{data.current.postedTransactionCount}{localize("件")}</td><td>{data.priorMonth.postedTransactionCount}{localize("件")}</td><td>{data.priorYear.postedTransactionCount}{localize("件")}</td></tr></tbody></table></div></section>
  </div>
}
