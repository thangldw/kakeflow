...
export function AnnualReviewView({ data, savingCsv = false, savingXlsx = false, savingPdf = false, onSelectDriver, onOpenBudget, onOpenImports, onOpenReconciliation, onSaveCsv, onSaveXlsx, onSavePdf }: AnnualReviewViewProps) {
  ...
  return <div className="report-view annual-review-view" aria-label="年次家計レビュー">
    <header className="report-view-head"><div><p>年次家計レビュー</p><h2>{data.period}年</h2><span>{data.completedMonthCount}か月の完了月・{comparisonLabel}と同じ期間で比較</span></div>{(onSaveCsv || onSaveXlsx || onSavePdf) && <div className="annual-review-export-actions">{onSaveCsv && <button type="button" className="secondary-btn" disabled={savingCsv || savingXlsx || savingPdf || data.completedMonthCount === 0} onClick={onSaveCsv}>{savingCsv ? 'CSVを作成中…' : '年次CSVを保存'}</button>}{onSaveXlsx && <button type="button" className="primary-btn" disabled={savingCsv || savingXlsx || savingPdf || data.completedMonthCount === 0} onClick={onSaveXlsx}>{savingXlsx ? 'Excelを作成中…' : '年次Excelを保存'}</button>}{onSavePdf && <button type="button" className="secondary-btn" disabled={savingCsv || savingXlsx || savingPdf || data.completedMonthCount === 0} onClick={onSavePdf}>{savingPdf ? 'PDFを作成中…' : '年次PDFを保存'}</button>}</div>}</header>
    ...
    <section className="report-health-grid" aria-label="年間レビューの確認事項">
      ...
      <article className="report-summary-card"><span>計算対象の確定取引</span><strong>{data.currentComparable.postedTransactionCount}件</strong><small>選択した口座・家族内帰属の完了月・集計対象外を除く</small></article>
    ...
    <section className="report-section annual-limitations"><div className="report-section-head"><div><h3>集計範囲と制約</h3><p>表示値を判断する前に確認してください</p></div></div><ul><li>計算対象の確定済み取引と完了した暦月だけを年間KPIと前年同期間比に使用し、集計対象外は除きます。</li><li>予算・目標・カード照合・取込状況には、選択した家族内帰属だけに配分できない世帯全体の情報が含まれます。</li><li>未取込・確認待ち・失敗したデータは集計に含まれません。</li></ul></section>
  </div>
}
