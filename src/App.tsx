...
import { accountKindLabel, accountSubtypeLabel, brokerageEventTypeLabel, directionLabel, evidenceRoleLabel, sourceTypeLabel, transactionTypeLabel } from './displayLabels'
  ...
  const extractDocument = async (item: ImportPreview, password?: string, requestedOperation?: 'EXTRACT' | 'OCR') => {
    ...
    try {
      ...
      if (!normalized.request) {
        setNotice(normalized.fields.issues.includes('STATEMENT_LIKELY') ? localize("明細書形式のPDFは、1件の支出として取り込みません。原本内容を確認してください。") : localize("日付または合計金額を読み取れませんでした。内容を確認してください。"))
        return
      }
        ...
        {previews.filter((item) => /\.(?:csv|tsv)$/i.test(item.filename) && item.fileBytes).map((item) => {
          ...
          return <div className="custom-parser-file" key={item.id}>
            ...
            {preview && <div className="custom-parser-result">
              ...
              {preview.issues.length > 0 && <ul>{preview.issues.slice(0, 6).map((issue, index) => <li key={`${issue.code}-${issue.row ?? 0}-${index}`}>{issue.row ? localize(`行 ${issue.row}: `) : ''}{issue.message}</li>)}</ul>}
