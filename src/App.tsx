...
import { HIGH_CONFIDENCE_CATEGORY_BPS, suggestLocalCategory } from './features/classification/localCategorySuggestion'
import type { LocalCategorySuggestion } from './features/classification/localCategorySuggestion'
...
import type { CustomColumnRole, CustomDelimitedPreview } from './ingestion'
...
const customColumnRoleLabels: Readonly<Record<CustomColumnRole, string>> = {
  DATE: '取引日', DESCRIPTION: '摘要', PAYEE: '支払先', EXTERNAL_ID: '外部ID',
  ACCOUNT_HINT: '口座ヒント', SIGNED_AMOUNT: '符号付き金額', DEBIT_AMOUNT: '支出額', CREDIT_AMOUNT: '収入額',
}
...
const navigation: NavigationItem[] = [
  ...
  { id: 'recurring', label: '定期取引・固定費', icon: Clock3 },
  { id: 'rules', label: '分類ルール', icon: Sparkles },
  { id: 'family', label: '家族スペース', icon: Users },
  { id: 'audit', label: '監査・証跡', icon: ShieldCheck },
]
...
const navigationSections: readonly { label: string; pages: readonly PageId[] }[] = [
  ...
  { label: '計画・分析', pages: ['reports', 'budgets', 'recurring', 'rules'] },
  { label: '世帯', pages: ['family', 'audit'] },
]
...
const pageMeta: Readonly<Record<PageId, { title: string; description: string }>> = {
  ...
  recurring: { title: '定期取引・固定費', description: 'サブスクリプションと固定費の変化' },
  rules: { title: '分類ルール', description: '決定的で説明可能な分類ルール' },
  family: { title: '家族スペース', description: '世帯メンバー・帰属・共有レビュー' },
  audit: { title: '監査・証跡', description: '原本、レビュー、確定台帳の来歴' },
  settings: { title: '設定', description: '口座・ローカルデータ・バックアップ' },
}

function householdInitials(name: string): string { return name.trim().slice(0, 2) || localize("家計") }

function Sidebar({ page, setPage, open, close, collapsed, toggleCollapsed, bootstrap, households, activeHouseholdId, selectHousehold, createHousehold, importActionableCount, capturePendingCount }: { page: PageId; setPage: (page: PageId) => void; open: boolean; close: () => void; collapsed: boolean; toggleCollapsed: () => void; bootstrap: AppBootstrapDto | null; households: readonly HouseholdDto[]; activeHouseholdId: string | null; selectHousehold: (id: string) => void; createHousehold: () => void; importActionableCount: number; capturePendingCount: number }) {
  const { locale, text } = useI18n()
  const [householdOpen, setHouseholdOpen] = useState(false)
  const householdRef = useRef<HTMLDivElement>(null)
  const activeHouseholdName = households.find((household) => household.id === activeHouseholdId)?.name ?? (platformClient.runtime === 'web' ? localize("田中家") : localize("家計"))
  useEffect(() => {
  ...
  return (
    <>
      ...
      <aside className={`sidebar ${open ? 'sidebar--open' : ''}${collapsed ? ' sidebar--collapsed' : ''}`} aria-label={text('メインナビゲーション')}>
        <div className="brand">
        ...
        <div className="household-picker" ref={householdRef}>
          ...
          <button type="button" className="household-trigger" aria-haspopup="menu" aria-expanded={householdOpen} onClick={() => setHouseholdOpen((value) => !value)}><strong>{activeHouseholdName}</strong><small>{households.length > 1 ? localize(`${households.length}世帯`) : text('ローカル世帯')}</small></button><ChevronDown size={14} aria-hidden="true" />
        ...
        <nav>
          ...
          {navigation.filter((item) => section.pages.includes(item.id)).map((item) => (
            <button
              ...
              aria-label={item.id === 'import' && importActionableCount > 0 ? locale === 'ja' ? localize(`${item.label}（${importActionableCount}件の確認対象）`) : `${text(item.label)} (${importActionableCount})` : item.id === 'capture' && capturePendingCount > 0 ? locale === 'ja' ? localize(`${item.label}（${capturePendingCount}件の確認対象）`) : `${text(item.label)} (${capturePendingCount})` : text(item.label)}
        ...
        <div className="sidebar-foot">
          <div className={`sync-status ${bootstrap?.database.healthy ? '' : 'sync-status--offline'}`}><span /><div><strong>{text('ローカル · デスクトップ版')}</strong><small>{bootstrap?.database.healthy ? `Schema v${bootstrap.database.schemaVersion}` : text(platformClient.runtime === 'web' ? localize("ブラウザプレビュー") : localize("データベース確認中"))}</small></div></div>
...
function DesktopTitleBar({ householdName }: { householdName: string }) {
  ...
  return <div className={`desktop-titlebar desktop-titlebar--${os}`} data-tauri-drag-region>
    ...
    <div className="windows-app-title" aria-hidden="true"><span>{localize("家")}</span><b>KakeFlow — {householdName}</b></div>
...
function Topbar({ page, openMenu, onGlobalSearch, onManualEntry, onImport, month, setMonth, accountGroups, accountGroupId, setAccountGroupId, attributionScope, setAttributionScope, members, showAccountScope, showBasis, basis, setBasis }: { page: PageId; openMenu: () => void; onGlobalSearch: (query: string) => void; onManualEntry: () => void; onImport: () => void; month: string; setMonth: (month: string) => void; accountGroups: readonly AccountGroupDto[]; accountGroupId: string | null; setAccountGroupId: (groupId: string | null) => void; attributionScope: AttributionScopeDto; setAttributionScope: (scope: AttributionScopeDto) => void; members: readonly HouseholdMemberDto[]; showAccountScope: boolean; showBasis: boolean; basis: WorkspaceBasis; setBasis: (basis: WorkspaceBasis) => void }) {
  ...
  const monthLabel = new Intl.DateTimeFormat(localeCode, { year: 'numeric', month: 'long' }).format(new Date(`${month}-01T12:00:00`))
  const pickerYearLabel = new Intl.DateTimeFormat(localeCode, { year: 'numeric' }).format(new Date(`${pickerYear}-01-01T12:00:00`))
  const pickerMonthLabel = (index: number) => new Intl.DateTimeFormat(localeCode, { month: 'short' }).format(new Date(`${pickerYear}-${String(index + 1).padStart(2, '0')}-01T12:00:00`))
  const accountCount = accountGroups.reduce((sum, group) => sum + group.accountIds.length, 0)
  const accountCountLabel = locale === 'ja'
    ? `${accountCount || localize("全")}${localize("口座")}`
    : accountCount > 0 ? `${accountCount} ${text("口座")}` : text("すべての口座")
  useEffect(() => {
  ...
  return (
    <header className={`topbar topbar--${page}`}>
      ...
      <GlobalLedgerSearch onSearch={onGlobalSearch} />
      <div className="gemini-quick-actions"><button className="primary-btn" onClick={onManualEntry}><Plus size={14} /> {localize('手入力取引')}</button><button className="secondary-btn" onClick={onImport}><Import size={14} /> {localize('ファイル取り込み')}</button></div>
      <div className="top-actions" ref={controlsRef}>
        {showAccountScope && <div className="workspace-popover-control"><button className="scope-trigger" aria-haspopup="dialog" aria-expanded={openPopover === 'scope'} onClick={() => setOpenPopover((value) => value === 'scope' ? null : 'scope')}>{localize("範囲:")} <strong>{selectedScopeLabel}</strong><ChevronDown size={13} /></button>{openPopover === 'scope' && <div className="scope-popover" role="dialog" aria-label={localize("集計範囲")}><section><p>{localize("口座グループ")}</p><button className={accountGroupId === null ? 'selected' : ''} onClick={() => { setAccountGroupId(null); setOpenPopover(null) }}><span>{accountGroupId === null && <Check size={13} />}{localize("すべての口座")}</span><small>{accountCountLabel}</small></button>{accountGroups.map((group) => <button key={group.id} className={accountGroupId === group.id ? 'selected' : ''} onClick={() => { setAccountGroupId(group.id); setOpenPopover(null) }}><span>{accountGroupId === group.id && <Check size={13} />}{group.name}</span><small>{locale === 'ja' ? `${group.accountIds.length}${localize("口座")}` : `${group.accountIds.length} ${text("口座")}`}</small></button>)}</section><section><p>{localize("帰属")}</p><button className={attributionScope.kind === 'ALL' ? 'selected' : ''} onClick={() => { setAttributionScope(ALL_ATTRIBUTION_SCOPE); setOpenPopover(null) }}><span>{attributionScope.kind === 'ALL' && <Check size={13} />}{localize("世帯全体")}</span></button>{members.filter((member) => member.status === 'ACTIVE').map((member) => <button key={member.id} className={attributionScope.kind === 'MEMBER' && attributionScope.memberId === member.id ? 'selected' : ''} onClick={() => { setAttributionScope({ kind: 'MEMBER', memberId: member.id }); setOpenPopover(null) }}><span>{attributionScope.kind === 'MEMBER' && attributionScope.memberId === member.id && <Check size={13} />}{member.displayName}{localize("のみ")}</span></button>)}</section></div>}</div>}
        <div className="period-stepper"><button aria-label={localize("前月")} onClick={() => selectPeriod(shiftMonth(month, -1))}><ChevronLeft size={14} /></button><button aria-haspopup="dialog" aria-expanded={openPopover === 'period'} onClick={() => { setPickerYear(Number(month.slice(0, 4))); setOpenPopover((value) => value === 'period' ? null : 'period') }}>{monthLabel}</button><button aria-label={localize("翌月")} disabled={month >= currentMonth} onClick={() => selectPeriod(shiftMonth(month, 1))}><ChevronRight size={14} /></button>{openPopover === 'period' && <div className="period-popover" role="dialog" aria-label={localize("対象月を選択")}><header><button aria-label={localize("前年")} onClick={() => setPickerYear((year) => year - 1)}><ChevronLeft size={14} /></button><strong>{pickerYearLabel}</strong><button aria-label={localize("翌年")} disabled={pickerYear >= Number(currentMonth.slice(0, 4))} onClick={() => setPickerYear((year) => year + 1)}><ChevronRight size={14} /></button></header><div className="month-grid">{Array.from({ length: 12 }, (_, index) => { const value = `${pickerYear}-${String(index + 1).padStart(2, '0')}`; const future = value > currentMonth; const partial = value === month || value === currentMonth; return <button key={value} disabled={future} className={value === month ? 'selected' : ''} onClick={() => selectPeriod(value)}><span>{pickerMonthLabel(index)}</span>{future ? <Minus size={11} /> : partial ? <CircleDotDashed size={11} /> : <Check size={11} />}</button> })}</div><footer><span><Check size={11} /> {localize("完全")}</span><span><CircleDotDashed size={11} /> {localize("一部")}</span><span><Minus size={11} /> {localize("なし")}</span><button onClick={() => selectPeriod(currentMonth)}>{localize("今月")}</button></footer></div>}</div>
        {showBasis && <div className="workspace-basis" role="group" aria-label={localize("計上基準")}>{([['ACCRUAL', localize("発生")], ['CASH', localize("資金移動")], ['BALANCE', localize("残高")]] as const).map(([value, label]) => <button key={value} className={basis === value ? 'active' : ''} aria-pressed={basis === value} disabled={value === 'BALANCE'} title={value === 'BALANCE' ? localize("残高ベースは読み取りモデル対応後に有効になります") : undefined} onClick={() => setBasis(value)}>{localize(label)}</button>)}</div>}
      </div>
      <div className="topbar-native-fallback" hidden aria-hidden="true"><label>{localize("対象月")}<input tabIndex={-1} type="month" value={month} onChange={(event) => selectPeriod(event.target.value)} /></label>{showAccountScope && <><label>{localize("口座スコープ")}<select tabIndex={-1} value={accountGroupId ?? ''} onChange={(event) => setAccountGroupId(event.target.value || null)}><option value="">{localize("すべての口座")}</option>{accountGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label><label>{localize("家族集計範囲")}<select tabIndex={-1} value={attributionScope.kind === 'MEMBER' ? `MEMBER:${attributionScope.memberId}` : attributionScope.kind} onChange={(event) => { const value = event.target.value; setAttributionScope(value === 'HOUSEHOLD_COMMON' ? { kind: 'HOUSEHOLD_COMMON' } : value.startsWith('MEMBER:') ? { kind: 'MEMBER', memberId: value.slice(7) } : ALL_ATTRIBUTION_SCOPE) }}><option value="ALL">{localize("世帯全体")}</option><option value="HOUSEHOLD_COMMON">{localize("世帯共通")}</option>{members.map((member) => <option key={member.id} value={`MEMBER:${member.id}`}>{member.displayName}</option>)}</select></label></>}</div>
    </header>
  )
}
...
function KpiCard({ label, value, meta, trend, icon: Icon, accent }: { label: string; value: string; meta: string; trend?: string; icon: typeof TrendingUp; accent: string }) {
  ...
  return (
    <article className="kpi-card">
      <div className="kpi-head"><div className="kpi-icon" style={{ background: accent }}><Icon size={18} /></div><span>{text(label)}</span><small>{text(meta.includes(localize("資金移動")) || label.includes(localize("入金")) || label.includes(localize("出金")) ? localize("資金移動") : label.includes(localize("純資産")) || label === localize("資産") || label === localize("負債") ? localize("残高") : localize("発生"))}</small></div>
      <strong>{value}</strong>
      <div className="kpi-meta">{trend && <em aria-label={locale === 'ja' ? localize(`${trend} 増加`) : trend}><ArrowUpRight aria-hidden="true" size={13} /><span aria-hidden="true">{trend}</span></em>}<span>{text(meta)}</span></div>
    </article>
  )
}

function TrendChart({ data = spendingTrend.slice(-6).map((point) => ({ month: point.month, income: point.income * 1000, expense: point.expense * 1000 })), incomeLabel = localize("収入"), expenseLabel = localize("支出") }: { data?: readonly { month: string; income: number; expense: number }[]; incomeLabel?: string; expenseLabel?: string }) {
  const { locale, localeCode, text } = useI18n()
  if (data.length === 0) return <p className="empty-state">{localize("トレンドを表示する取引はまだありません。")}</p>
  const max = Math.max(1, ...data.flatMap((point) => [point.income, point.expense]))
  const periodLabel = locale === 'ja' ? localize(`直近${data.length}か月`) : locale === 'vi' ? `${data.length} tháng` : `${data.length} months`
  const chartLabel = locale === 'ja' ? `${periodLabel}の${text(incomeLabel)}と${text(expenseLabel)}` : locale === 'vi' ? `${periodLabel}: ${text(incomeLabel)} và ${text(expenseLabel)}` : `${periodLabel}: ${text(incomeLabel)} and ${text(expenseLabel)}`
  const displayMonth = (value: string) => value.includes('-') ? new Intl.DateTimeFormat(localeCode, { month: 'short' }).format(new Date(`${value}-01T12:00:00`)) : value
  return (
    <div className="chart-wrap chart-bars" role="img" aria-label={chartLabel}>
      <div className="bar-grid" aria-hidden="true">{[1, .75, .5, .25].map((line) => <i key={line} style={{ bottom: `${line * 100}%` }} />)}</div>
      <div className="bar-columns">{data.map((point) => <div className="bar-month" key={point.month} aria-label={`${point.month}: ${text(incomeLabel)} ${yen(point.income)}, ${text(expenseLabel)} ${yen(point.expense)}`}><div className="bar-pair"><span className="bar-income" style={{ height: `${Math.max(3, point.income / max * 100)}%` }} /><span className="bar-expense" style={{ height: `${Math.max(3, point.expense / max * 100)}%` }} /></div><small>{displayMonth(point.month)}</small></div>)}</div>
      <table className="visually-hidden"><caption>{chartLabel}</caption><thead><tr><th>{locale === 'ja' ? localize("月") : text('対象月')}</th><th>{text(incomeLabel)}</th><th>{text(expenseLabel)}</th></tr></thead><tbody>{data.map((point) => <tr key={`table-${point.month}`}><th>{point.month}</th><td>{point.income}{locale === 'ja' ? localize("円") : ' JPY'}</td><td>{point.expense}{locale === 'ja' ? localize("円") : ' JPY'}</td></tr>)}</tbody></table>
    </div>
  )
}
...
function TransactionRows({ rows = transactions, onSelect, selectedIds, onToggleSelection, compact = !onSelect }: { rows?: readonly Transaction[]; onSelect?: (id: string, trigger: HTMLButtonElement) => void; selectedIds?: ReadonlySet<string>; onToggleSelection?: (id: string, selected: boolean) => void; compact?: boolean }) {
  const { text } = useI18n()
  if (!compact) return <div className="transaction-ledger" role="table" aria-label={localize("確定取引")}>
    <div className="transaction-ledger-head" role="row">{onToggleSelection && <span aria-hidden="true" />}<span>{localize("日付")}</span><span>{localize("摘要")}</span><span>{localize("カテゴリ")}</span><span>{localize("口座")}</span><span>{localize("種別")}</span><span>{localize("金額")}</span><span aria-label={localize("証跡")}>{localize("証跡")}</span></div>
    {rows.map((tx) => {
      const txType = tx.transactionType ?? (tx.amount > 0 ? 'INCOME' : tx.accountingEffect === 'CASH_ONLY' ? 'CARD_PAYMENT' : 'EXPENSE')
      const row = <button type="button" className="transaction-ledger-row" onClick={(event) => onSelect?.(tx.id, event.currentTarget)} role="row"><span className="tx-date">{tx.date}</span><span className="tx-description"><strong>{tx.merchant}</strong><small>{tx.detail}</small>{tx.calculationTarget === false && <em className="calculation-badge excluded">{text('集計対象外')}</em>}</span><span><b className="category-pill">{text(tx.category)}</b></span><span className="tx-account">{tx.account}</span><span><b className={`transaction-type type-${txType.toLowerCase()}`}>{localize(transactionTypeLabels[txType])}</b></span><strong className={`tx-amount ${txType === 'INCOME' || txType === 'REFUND' ? 'amount-positive' : txType === 'EXPENSE' || txType === 'CARD_PURCHASE' ? 'amount-expense' : ''}`}>{yen(tx.amount)}</strong><span className="tx-evidence" aria-label={localize("原本証跡あり")}><FileCheck2 size={14} /></span></button>
      return <div className="transaction-ledger-selection" role="rowgroup" key={tx.id}>{onToggleSelection && <label className="transaction-selector"><input type="checkbox" aria-label={localize(`${tx.merchant}を一括編集対象に選択`)} checked={selectedIds?.has(tx.id) ?? false} onChange={(event) => onToggleSelection(tx.id, event.target.checked)} /></label>}{row}</div>
    })}
  </div>
  return <div className="transaction-list">{rows.map((tx) => {
    ...
    return onToggleSelection
      ? <div className="transaction-selection-row" key={tx.id}><label className="transaction-selector"><input type="checkbox" aria-label={localize(`${tx.merchant}を一括編集対象に選択`)} checked={selectedIds?.has(tx.id) ?? false} onChange={(event) => onToggleSelection(tx.id, event.target.checked)} /></label>{row}</div>
...
function ReconciliationMini({ liveCards, desktop, onOpen }: { liveCards: readonly CardSettlementDto[]; desktop: boolean; onOpen: () => void }) {
  ...
  const cards = desktop ? liveCards.map((card) => ({
    name: card.cardName, mask: card.maskedIdentifier ?? localize("番号未設定"), dueDate: card.paymentDueOn ?? card.periodEnd,
  ...
  return (
    <article className="panel reconciliation">
      ...
      <div className="card-stack">{cards.length > 0 ? cards.map((card) => <div className="settlement" key={card.name}>
        <div className="settlement-title"><i style={{ background: card.color }} /><div><strong>{card.name}</strong><span>{card.mask} ・ {card.dueDate}</span></div><b className={card.status}>{text(card.status === 'reconciled' ? localize("全額照合") : card.status === 'possible' ? localize("一部・候補あり") : localize("引落待ち"))}</b></div>
      ...
      </div>) : <p className="empty-state">{localize("カード明細はまだありません。")}</p>}</div>
    </article>
  )
}
...
function DashboardDataQuality({ counts, desktop, onOpenImport }: { counts: ImportRunCountsDto | null; desktop: boolean; onOpenImport: () => void }) {
  ...
  const state = !data || data.sourceDocuments === 0 ? localize("原本データなし") : data.failed > 0 ? localize("取込エラーあり") : reviewCount > 0 ? localize("確認待ちあり") : localize("確認済みデータを反映")
  ...
  return <section className="panel dashboard-data-quality" aria-labelledby="dashboard-data-quality-title">
    <div className="panel-head"><div><h2 id="dashboard-data-quality-title">{text('データ品質')}</h2><p>{text(desktop ? localize("この端末の原本・取込・確認状態") : localize("ブラウザプレビュー用のサンプル状態"))}</p></div><b className={data?.failed ? 'error' : reviewCount ? 'review' : 'ready'}>{text(state)}</b></div>
...
function DashboardControls({ preferences, disabled, onChange }: { preferences: DashboardPreferencesDto; disabled: boolean; onChange: (change: DashboardPreferenceChange) => void }) {
  ...
  const move = (widget: DashboardWidgetIdDto, target: DashboardWidgetIdDto) => {
    ...
    setAnnouncement(localize(`${localize(dashboardWidgetLabels[widget])}を${position}/${available.length}へ移動しました`))
  }
  ...
  const toggleVisible = (widget: DashboardWidgetIdDto) => {
    ...
    setAnnouncement(localize(`${localize(dashboardWidgetLabels[widget])}を${hidden ? localize("表示") : localize("非表示に")}しました`))
  }
  return <div className="dashboard-controls-shell">
    <div className="dashboard-controls" aria-label={localize("ダッシュボード表示設定")}>
      <span className="dashboard-control-label">{localize("テンプレート:")}</span>
      <div className="dashboard-template-chips" role="group" aria-label={localize("ダッシュボードテンプレート")}>{Object.entries(dashboardTemplateLabels).map(([value, label]) => <button key={value} type="button" aria-label={localize(`${localize("テンプレート:")} ${localize(label)}`)} className={preferences.template === value ? 'active' : ''} aria-pressed={preferences.template === value} disabled={disabled} onClick={() => { setDragging(null); setAnnouncement(''); onChange({ template: value as DashboardPreferencesDto['template'] }) }}>{localize(label)}</button>)}</div>
      <div className="visually-hidden"><label>{localize("ホームの表示テンプレート")}<select aria-label={localize("ホームの表示テンプレート")} disabled={disabled} value={preferences.template} onChange={(event) => { setDragging(null); setAnnouncement(''); onChange({ template: event.target.value as DashboardPreferencesDto['template'] }) }}>{Object.entries(dashboardTemplateLabels).map(([value, label]) => <option key={value} value={value}>{localize(label)}</option>)}</select></label><label>{localize("アプリのテーマ")}<select aria-label={localize("アプリのテーマ")} disabled={disabled} value={preferences.theme} onChange={(event) => onChange({ theme: event.target.value as DashboardPreferencesDto['theme'] })}><option value="SYSTEM">{localize("システム")}</option><option value="LIGHT">{localize("ライト")}</option><option value="DARK">{localize("ダーク")}</option></select></label><label>{localize("画面の表示密度")}<select aria-label={localize("画面の表示密度")} disabled={disabled} value={preferences.density} onChange={(event) => onChange({ density: event.target.value as DashboardPreferencesDto['density'] })}><option value="COMFORTABLE">{localize("標準")}</option><option value="COMPACT">{localize("コンパクト")}</option></select></label></div>
      <button className="secondary-btn dashboard-layout-toggle" type="button" disabled={disabled} aria-expanded={editorOpen} onClick={() => { if (editorOpen) { setEditorOpen(false); setEditorBaseline(null) } else { setEditorBaseline({ widgetOrder: [...layout.widgetOrder], hiddenWidgets: [...layout.hiddenWidgets] }); setEditorOpen(true) } }}><LayoutDashboard size={15} /> {localize("レイアウト編集")}</button>
    </div>
    {editorOpen && <section className="dashboard-layout-editor" aria-labelledby="dashboard-layout-title">
      <div className="dashboard-layout-head"><div><strong id="dashboard-layout-title">{localize("ウィジェットの並びと表示")}</strong><span>{localize(dashboardTemplateLabels[preferences.template])}{localize("に適用")}</span></div><div><button type="button" disabled={disabled} onClick={() => onChange({ widgetOrder: exhaustiveWidgetOrder(preferences.template), hiddenWidgets: [] })}>{localize("テンプレートに戻す")}</button><button type="button" disabled={disabled || !editorBaseline} onClick={() => { if (editorBaseline) onChange(editorBaseline); setEditorOpen(false); setEditorBaseline(null) }}>{localize("キャンセル")}</button><button type="button" className="primary-btn" disabled={disabled} onClick={() => { setEditorOpen(false); setEditorBaseline(null); showToast(localize("ホームのレイアウトを保存しました。")) }}>{localize("完了")}</button></div></div>
      <div className="dashboard-layout-list">{visibleWidgets.map((widget, index) => {
        ...
        return <div className={`dashboard-layout-row${dragging === widget ? ' is-dragging' : ''}`} key={widget} draggable={!disabled} onDragStart={(event) => { setDragging(widget); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', widget) }} onDragEnd={() => setDragging(null)} onDragOver={(event) => { if (dragging) event.preventDefault() }} onDrop={(event) => { event.preventDefault(); if (dragging) move(dragging, widget); setDragging(null) }}>
          <GripVertical size={15} aria-hidden="true" /><strong>{localize(dashboardWidgetLabels[widget])}</strong>
          <button type="button" className="layout-visibility" aria-label={localize(`${localize(dashboardWidgetLabels[widget])}を非表示`)} aria-pressed="true" aria-describedby={lastVisible ? 'last-widget-note' : undefined} disabled={disabled || lastVisible} onClick={() => toggleVisible(widget)}>{localize("非表示")}</button>
          <button type="button" aria-label={localize(`${localize(dashboardWidgetLabels[widget])}を上へ移動`)} disabled={disabled || index === 0} onClick={() => moveBy(widget, -1)}><ChevronUp size={15} /></button>
          <button type="button" aria-label={localize(`${localize(dashboardWidgetLabels[widget])}を下へ移動`)} disabled={disabled || index === visibleWidgets.length - 1} onClick={() => moveBy(widget, 1)}><ChevronDown size={15} /></button>
        </div>
      })}</div>
      <div className="dashboard-hidden-tray" aria-label={localize("非表示のウィジェット")}><strong>{localize("非表示のウィジェット")}</strong>{hiddenWidgets.length > 0 ? <div>{hiddenWidgets.map((widget) => <button type="button" key={widget} disabled={disabled} onClick={() => toggleVisible(widget)}>{localize(dashboardWidgetLabels[widget])} <span>{localize("復元")}</span></button>)}</div> : <span>{localize("非表示のウィジェットはありません。")}</span>}</div>
      <small id="last-widget-note">{localize("少なくとも1つのウィジェットを表示します。")}</small>
...
function Overview({ setPage, openAllActions, householdId, accountGroupId, attributionScope, revision, liveDashboard, liveTransactions, liveCards, importCounts, dashboardLoading, desktop, householdName, month, preferences, preferencesBusy, updatePreferences }: { setPage: (page: PageId) => void; openAllActions: () => void; householdId: string | null; accountGroupId: string | null; attributionScope: AttributionScopeDto; revision: number; liveDashboard: DashboardMonthlyTotalsDto | null; liveTransactions: readonly TransactionRowDto[]; liveCards: readonly CardSettlementDto[]; importCounts: ImportRunCountsDto | null; dashboardLoading: boolean; desktop: boolean; householdName: string; month: string; preferences: DashboardPreferencesDto; preferencesBusy: boolean; updatePreferences: (change: DashboardPreferenceChange) => void }) {
  ...
  const kpis = cashFlow
    ? [<KpiCard key="cash-in" label={localize("今月の入金")} value={yen(income)} meta={localize("資金移動ベース")} icon={ArrowDownLeft} accent="#dce9e6" />, <KpiCard key="cash-out" label={localize("今月の出金")} value={yen(expense)} meta={localize("カード購入ではなく銀行引落時に計上")} icon={ArrowUpRight} accent="#f7e3d9" />, <KpiCard key="net-cash" label={localize("差引キャッシュフロー")} value={yen(projectedSavings)} meta={localize("入金 − 出金")} icon={CircleDollarSign} accent="#eee5cf" />, <KpiCard key="assets" label={localize("月末資産")} value={yen(assets)} meta={localize(`${liveDashboard?.netWorthAsOf ?? localize("月末")} 現在`)} icon={WalletCards} accent="#e4edda" />]
    : preferences.template === 'HOUSEHOLD_LEDGER'
    ? [<KpiCard key="income" label={localize("今月の収入")} value={yen(income)} meta={localize("発生ベース")} icon={ArrowDownLeft} accent="#dce9e6" />, <KpiCard key="expense" label={localize("今月の支出")} value={yen(expense)} meta={localize("カード引落は二重計上しません")} icon={ArrowUpRight} accent="#f7e3d9" />, <KpiCard key="savings" label={localize("貯蓄見込み")} value={yen(projectedSavings)} meta={localize("収入 − 支出")} icon={CircleDollarSign} accent="#eee5cf" />, <KpiCard key="net-worth" label={localize("純資産")} value={yen(netWorth)} meta={localize(`${liveDashboard?.netWorthAsOf ?? localize("月末")} 現在`)} icon={TrendingUp} accent="#e4edda" />]
    : preferences.template === 'ASSETS_LIABILITIES'
      ? [<KpiCard key="assets" label={localize("資産")} value={yen(assets)} meta={localize(`${liveDashboard?.netWorthAsOf ?? localize("月末")} 現在`)} icon={WalletCards} accent="#dce9e6" />, <KpiCard key="liabilities" label={localize("負債")} value={yen(liabilities)} meta={localize("カードを含む台帳残高")} icon={CreditCard} accent="#f7e3d9" />, <KpiCard key="net-worth" label={localize("純資産")} value={yen(netWorth)} meta={localize("資産 − 負債")} icon={TrendingUp} accent="#e4edda" />, <KpiCard key="savings" label={localize("今月の貯蓄")} value={yen(projectedSavings)} meta={localize("収入 − 支出")} icon={CircleDollarSign} accent="#eee5cf" />]
      : preferences.template === 'CARD_RECONCILIATION'
        ? [<KpiCard key="liabilities" label={localize("カードを含む負債")} value={yen(liabilities)} meta={localize(`${liveDashboard?.netWorthAsOf ?? localize("月末")} 現在`)} icon={CreditCard} accent="#f7e3d9" />, <KpiCard key="expense" label={localize("今月の支出")} value={yen(expense)} meta={localize("カード購入は利用日に計上")} icon={ArrowUpRight} accent="#f7e3d9" />, <KpiCard key="assets" label={localize("支払原資を含む資産")} value={yen(assets)} meta={localize("台帳上の資産残高")} icon={WalletCards} accent="#dce9e6" />, <KpiCard key="net-worth" label={localize("純資産")} value={yen(netWorth)} meta={localize("支払い後も二重計上しません")} icon={TrendingUp} accent="#e4edda" />]
        : [<KpiCard key="net-worth" label={localize("純資産")} value={yen(netWorth)} meta={desktop ? localize(`${liveDashboard?.netWorthAsOf ?? localize("月末")} 現在`) : localize("前月比")} trend={desktop ? undefined : '2.8%'} icon={TrendingUp} accent="#e4edda" />, <KpiCard key="income" label={localize("今月の収入")} value={yen(income)} meta={desktop ? localize("発生ベース") : localize("予定の 104%")} trend={desktop ? undefined : '4.2%'} icon={ArrowDownLeft} accent="#dce9e6" />, <KpiCard key="expense" label={localize("今月の支出")} value={yen(expense)} meta={desktop ? localize("カード引落は二重計上しません") : localize(`予算 ${yen(currentMonthMetrics.budget)}`)} icon={ArrowUpRight} accent="#f7e3d9" />, <KpiCard key="savings" label={localize("貯蓄見込み")} value={yen(projectedSavings)} meta={desktop ? localize("収入 − 支出") : localize(`貯蓄率 ${(savingsRate * 100).toFixed(1)}%`)} trend={desktop ? undefined : '6.1%'} icon={CircleDollarSign} accent="#eee5cf" />]
  const panels: Record<DashboardWidgetIdDto, React.ReactNode> = {
    TREND: <article key="trend" className="panel trend-panel dashboard-widget dashboard-widget--trend"><div className="panel-head"><div><h2>{text(cashFlow ? localize("入出金の推移") : localize("収支の推移"))}</h2><p>{text(cashFlow ? localize("資金移動") : localize("発生ベース"))} · {locale === 'ja' ? localize(`${desktop ? trend?.length ?? 0 : 6}か月`) : locale === 'vi' ? `${desktop ? trend?.length ?? 0 : 6} tháng` : `${desktop ? trend?.length ?? 0 : 6} months`}</p></div><div className="chart-legend"><span className="income">{text(cashFlow ? localize("入金") : localize("収入"))}</span><span className="expense">{text(cashFlow ? localize("出金") : localize("支出"))}</span></div></div><TrendChart data={trend} incomeLabel={cashFlow ? localize("入金") : localize("収入")} expenseLabel={cashFlow ? localize("出金") : localize("支出")} /></article>,
  ...
  const overviewDescription = locale === 'ja'
    ? (desktop ? localize(`選択月の計算対象の確定取引 ${liveDashboard?.postedTransactionCount ?? 0}件を${cashFlow ? localize("資金移動") : localize("発生")}ベースで集計しています（集計対象外を除く）。`) : localize(`家計は順調です。予算の ${(budgetUsage * 100).toFixed(1)}% を使いました。`))
  ...
  if (desktop && dashboardLoading) return <div className="overview dashboard-loading" aria-busy="true" aria-label={localize("ホームを読み込み中")}><PageHeader eyebrow={monthLabel} title={householdName === localize("家計") ? localize("家計の概要") : localize(`${householdName}の家計`)} description={localize("確定済み台帳を読み込んでいます。")}><DashboardControls preferences={preferences} disabled={preferencesBusy} onChange={updatePreferences} /></PageHeader><div className="skeleton-kpi-grid">{[0, 1, 2, 3].map((item) => <div className="skeleton skeleton-kpi" key={item} />)}</div><div className="skeleton-dashboard-grid"><div className="skeleton skeleton-panel" /><div className="skeleton skeleton-panel" /></div></div>
  const firstRunEmpty = desktop && liveDashboard?.postedTransactionCount === 0 && (importCounts?.sourceDocuments ?? 0) === 0
  if (firstRunEmpty) return <div className="overview"><PageHeader eyebrow={monthLabel} title={localize(`${householdName}の家計`)} description={localize("最初のデータを追加すると、収支・予算・カード照合がここにまとまります。")} /><section className="panel dashboard-first-run"><div className="first-run-icon"><Home size={24} /></div><h2>{localize("新しい世帯へようこそ")}</h2><p>{localize("ダッシュボードに表示されるのは、レビューして確定した取引だけです。まず原本、レシート、または口座を設定してください。")}</p><div><button className="primary-btn" onClick={() => setPage('import')}><Import size={16} /> {localize("ファイルを取り込む")}</button><button className="secondary-btn" onClick={() => setPage('capture')}><Camera size={16} /> {localize("レシートを追加")}</button><button className="secondary-btn" onClick={() => setPage('settings')}><Settings size={16} /> {localize("口座を設定")}</button></div></section></div>
  return <div className={`overview overview--${preferences.template.toLowerCase().replaceAll('_', '-')}`}>
    <PageHeader eyebrow={monthLabel} title={householdName === localize("家計") ? localize("家計の概要") : locale === 'ja' ? localize(`${householdName}の家計`) : localize(`${householdName} · ${text('家計の概要')}`)} description={overviewDescription}>
    ...
    <SecondaryCurrencySummary netWorth={netWorth} income={income} expense={expense} />
    ...
    <div className="data-footnote"><FileCheck2 size={15} /> {localize("確定済み台帳から")}{cashFlow ? localize("実際の資産入出金を") : ''}{localize("集計 ・ 未確認の候補は含みません")}</div>
  </div>
}
...
function TransactionSplitEditor({ total, side, categoryAccounts, initialRows, onApply, onCancel }: { total: number; side: 'DEBIT' | 'CREDIT'; categoryAccounts: readonly AccountDto[]; initialRows: readonly TransactionEntryDraft[]; onApply: (rows: readonly TransactionEntryDraft[]) => void; onCancel: () => void }) {
  ...
  return <section className="transaction-split-editor" aria-label={localize("取引の分割")}><header><div><strong>{localize("取引を分割")}</strong><span>{localize("カテゴリごとの金額を入力してください。仕訳の合計は変わりません。")}</span></div><button className="icon-btn" aria-label={localize("分割編集を閉じる")} onClick={onCancel}><X size={16} /></button></header><div className="split-row-list">{rows.map((row, index) => <div className="split-row" key={row.id}><span>{index + 1}</span><select aria-label={localize(`分割${index + 1}のカテゴリ`)} value={row.accountId} onChange={(event) => update(row.id, { accountId: event.target.value })}><option value="">{localize("カテゴリを選択")}</option>{categoryAccounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select><input aria-label={localize(`分割${index + 1}の金額`)} inputMode="numeric" value={row.amountJpy} onChange={(event) => update(row.id, { amountJpy: event.target.value.replace(/\D/g, '') })} placeholder="0" /><button className="text-btn" disabled={rows.length <= 2} onClick={() => setRows((current) => current.filter((item) => item.id !== row.id))}>{localize("削除")}</button></div>)}</div><button className="secondary-btn split-add" onClick={() => setRows((current) => [...current, { id: crypto.randomUUID(), accountId: '', side, amountJpy: '' }])}>{localize("＋ 分割行を追加")}</button><footer><div className={remainder === 0 ? 'split-remainder complete' : 'split-remainder'}><span>{localize("残り")}</span><strong>{yen(remainder)}</strong>{remainder === 0 && <Check size={15} aria-label={localize("分割金額一致")} />}</div><button className="secondary-btn" onClick={onCancel}>{localize("キャンセル")}</button><button className="primary-btn" disabled={!valid} onClick={() => onApply(rows)}>{localize("分割を適用")}</button></footer></section>
}
...
function CardStatementSourceFields({ records }: { records: readonly SourceRecordViewDto[] }) {
  ...
  return <section className="card-statement-source-fields" aria-label={localize("カード明細情報")}><header><h3>{localize("カード明細情報")}</h3><span>{localize("原本の列名と値")}</span></header><dl>{entries.map(([name, value]) => <div key={name}><dt>{name}</dt><dd>{value || '—'}</dd></div>)}</dl></section>
}
...
function TransactionDetailPanel({ detail, accounts, members, returnFocus, onClose, onSave, onChanged }: { detail: TransactionDetailDto; accounts: readonly AccountDto[]; members: readonly HouseholdMemberDto[]; returnFocus: HTMLElement | null; onClose: () => void; onSave: (input: UpdatePostedTransactionInputDto) => Promise<void>; onChanged: () => void }) {
  ...
  const applySplit = (rows: readonly TransactionEntryDraft[]) => {
    ...
    setSplitOpen(false)
    setNotice(localize("分割を仕訳へ反映しました。「変更を保存」で確定してください。"))
  }
    ...
    try {
      ...
      if (evidence?.mediaType.startsWith('image/')) {
        try {
          ...
          setNotice(localize("原本行は読み込みましたが、画像プレビューを表示できませんでした。"))
        }
      }
    }
    catch { setNotice(localize("原本レコードを読み込めませんでした。")) }
  ...
  const applyBestRule = async () => {
    ...
    try {
      ...
      if (!winner) { setNotice(localize("この取引に一致する有効な分類ルールはありません。")); return }
      ...
      setNotice(localize(`${winner.name} を適用し、${winner.categoryName} に分類しました。`)); onChanged()
    } catch { setNotice(localize("分類ルールを適用できませんでした。取引が更新されている可能性があります。")) }
  ...
  const save = async () => {
    if (entries.length < 2 || debitTotal <= 0 || debitTotal !== creditTotal || entries.some((entry) => !entry.accountId || !/^\d+$/.test(entry.amountJpy) || Number(entry.amountJpy) <= 0)) {
      setNotice(localize("借方と貸方を同額にし、すべての口座と金額を入力してください。")); return
    }
    ...
    } catch { setNotice(localize("変更を保存できませんでした。入力内容を確認してください。")) }
  ...
  const updateSourceAudience = async (sourceDocumentId: string) => {
    ...
    try {
      ...
      setNotice(localize("原本の表示区分を更新しました。リンク先取引の帰属・表示区分は変更していません。"))
    } catch { setNotice(localize("原本の表示区分を更新できませんでした。")) }
    finally { setSourceBusy(false) }
  }
  const assignmentMembers = (currentId: string | null) => members.filter((member) => member.status === 'ACTIVE' || member.id === currentId)
  const pageImages = sourceImagePreview && sourceImageSize ? { 1: { src: sourceImagePreview.dataUrl, width: sourceImageSize.width, height: sourceImageSize.height, alt: localize(`${sourceImagePreview.filename} 原本`) } } : undefined
  ...
  return <div className="detail-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose() }}><section ref={dialogRef} className="transaction-detail-panel" role="dialog" aria-modal="true" aria-labelledby="transaction-detail-title" onKeyDown={handleKeyDown}><div className="panel-head"><div><p>{localize("取引詳細")}</p><h2 ref={headingRef} tabIndex={-1} id="transaction-detail-title">{detail.payee ?? detail.description ?? detail.id}</h2><span className={calculationTarget ? 'calculation-badge included' : 'calculation-badge excluded'}>{calculationTarget ? localize("計算対象") : localize("集計対象外")}</span></div><button className="icon-btn" aria-label={localize("取引詳細を閉じる")} disabled={busy} onClick={onClose}><X size={18} /></button></div>{transactionType === 'CARD_PAYMENT' && <aside className="card-payment-info"><Repeat2 size={16} /><span><strong>{localize("カード支払・振替")}</strong><small>{localize("支出ではありません — 購入時に計上済み")}</small></span></aside>}<div className="detail-fields"><label>{localize("取引日")}<input type="date" value={occurredOn} onChange={(event) => setOccurredOn(event.target.value)} /></label><label>{localize("取引種別")}<select value={transactionType} onChange={(event) => { setTransactionType(event.target.value as ManualTransactionTypeDto); setSplitOpen(false) }}>{['EXPENSE', 'INCOME', 'TRANSFER', 'CARD_PURCHASE', 'CARD_PAYMENT', 'REFUND', 'FEE', 'INTEREST', 'ADJUSTMENT'].map((type) => <option key={type} value={type}>{transactionTypeLabel(type, text)}</option>)}</select></label><label>{localize("支払先")}<input value={payee} onChange={(event) => setPayee(event.target.value)} /></label><label>{localize("メモ")}<input value={description} onChange={(event) => setDescription(event.target.value)} /></label></div><div className="family-assignment-fields"><fieldset><legend>{localize("家族内の帰属")}</legend><p>{localize("誰の支出・収入として集計するかを設定します。支払口座の所有者とは別です。")}</p><select aria-label={localize("取引の家族内の帰属")} value={attribution} onChange={(event) => setAttribution(event.target.value)}><option value="HOUSEHOLD">{localize("世帯共通")}</option>{assignmentMembers(detail.attributedMemberId).map((member) => <option key={member.id} value={member.id}>{member.displayName}{member.status === 'ARCHIVED' ? localize("（アーカイブ済み）") : ''}</option>)}</select></fieldset><fieldset><legend>{localize("表示区分")}</legend><p>{localize("「個人」もこの端末内の整理用ラベルであり、閲覧制限ではありません。")}</p><select aria-label={localize("取引の表示区分")} value={audience} onChange={(event) => setAudience(event.target.value)}><option value="SHARED">{localize("共有")}</option>{assignmentMembers(detail.audienceMemberId).map((member) => <option key={member.id} value={member.id}>{localize("個人・")}{member.displayName}{member.status === 'ARCHIVED' ? localize("（アーカイブ済み）") : ''}</option>)}</select></fieldset></div><label className="calculation-target-control"><input type="checkbox" checked={calculationTarget} onChange={(event) => setCalculationTarget(event.target.checked)} /><span><strong>{localize("家計の集計に含める")}</strong><small>{localize("オフにするとダッシュボード、予算、レポート、予測の集計から外れます。台帳の仕訳は削除されず、口座・カード残高も変わりません。")}</small></span></label><div className="detail-section-head"><div><h3>{localize("仕訳")}</h3><span>{localize("借方")} {yen(debitTotal)} {localize("/ 貸方")} {yen(creditTotal)}</span></div><div><button className="secondary-btn" disabled={ruleBusy} onClick={() => void applyBestRule()}>{ruleBusy ? localize("照合中…") : localize("分類ルールを適用")}</button>{splitShape && splitEntries.length > 0 && <button className="secondary-btn" aria-expanded={splitOpen} onClick={() => setSplitOpen((value) => !value)}>{localize("取引を分割…")}</button>}<button className="secondary-btn" onClick={() => setEntries((current) => [...current, { id: crypto.randomUUID(), accountId: '', side: 'DEBIT' as const, amountJpy: '' }])}>{localize("仕訳行を追加")}</button></div></div>{splitOpen && splitShape && <TransactionSplitEditor total={splitTotal} side={splitShape.side} categoryAccounts={splitCategoryAccounts} initialRows={splitEntries} onApply={applySplit} onCancel={() => setSplitOpen(false)} />}<div className="journal-editor">{entries.map((entry, index) => <div className="journal-line" key={entry.id}><select aria-label={localize(`仕訳${index + 1}の借貸`)} value={entry.side} onChange={(event) => updateEntry(index, { side: event.target.value as 'DEBIT' | 'CREDIT' })}><option value="DEBIT">{localize("借方")}</option><option value="CREDIT">{localize("貸方")}</option></select><select aria-label={localize(`仕訳${index + 1}の口座`)} value={entry.accountId} onChange={(event) => updateEntry(index, { accountId: event.target.value })}><option value="">{localize("口座を選択")}</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select><input aria-label={localize(`仕訳${index + 1}の金額`)} inputMode="numeric" value={entry.amountJpy} onChange={(event) => updateEntry(index, { amountJpy: event.target.value })} /><button className="text-btn" aria-label={localize(`仕訳${index + 1}を削除`)} disabled={entries.length <= 2} onClick={() => setEntries((current) => current.filter((_, currentIndex) => currentIndex !== index))}>{localize("削除")}</button></div>)}</div><CardStatementSourceFields records={sourceRecords} /><div className="evidence-list"><h3>{localize("原本・証跡")}</h3><p className="source-audience-help">{localize("原本自体の整理ラベルです。リンク先取引の帰属・表示区分とは別に管理します。")}</p>{detail.sourceEvidence.length === 0 ? <p>{localize("手動入力のため原本はありません。")}</p> : detail.sourceEvidence.map((evidence) => { const sourceAudience = sourceAudiences[evidence.sourceDocumentId] ?? (evidence.audienceMemberId ?? 'SHARED'); return <div className="source-evidence-item" key={evidence.sourceDocumentId + '-' + evidence.sourceRecordId}><button type="button" className={'source-evidence-button ' + (selectedSourceRecordId === evidence.sourceRecordId ? 'active' : '')} onClick={() => void showSourceRecord(evidence.sourceRecordId)}><FileCheck2 size={16} /><span><strong>{evidence.originalFilename}</strong><small>{sourceTypeLabel(evidence.sourceType, text)} {localize("・ 行")} {evidence.rowNumber} ・ {evidenceRoleLabel(evidence.evidenceRole, text)}</small><small>{localize("原本:")} {sourceAudience === 'SHARED' ? localize("共有") : localize("個人・") + (members.find((member) => member.id === sourceAudience)?.displayName ?? evidence.audienceMemberName ?? localize("メンバー"))}</small></span><em>{sourceBusy && selectedSourceRecordId === evidence.sourceRecordId ? localize("読込中…") : localize("原本行を表示")}</em></button><div className="source-audience-editor"><label>{localize("原本の表示区分")}<select aria-label={evidence.originalFilename + localize("の原本表示区分")} value={sourceAudience} onChange={(event) => setSourceAudiences((current) => ({ ...current, [evidence.sourceDocumentId]: event.target.value }))}><option value="SHARED">{localize("共有")}</option>{assignmentMembers(evidence.audienceMemberId).map((member) => <option key={member.id} value={member.id}>{localize("個人・")}{member.displayName}{member.status === 'ARCHIVED' ? localize("（アーカイブ済み）") : ''}</option>)}</select></label><button className="secondary-btn" aria-label={evidence.originalFilename + localize("の原本区分を保存")} disabled={sourceBusy} onClick={() => void updateSourceAudience(evidence.sourceDocumentId)}>{localize("原本区分を保存")}</button></div></div> })}</div>{selectedSource && (documentEvidence ? <DocumentEvidenceViewer evidence={documentEvidence} filename={selectedSourceFilename} pageImages={pageImages} pdfSource={selectedSourceEvidence?.mediaType === 'application/pdf' ? { householdId: detail.householdId, sourceDocumentId: selectedSourceEvidence.sourceDocumentId } : undefined} /> : <section className="source-record-viewer" aria-label={localize("原本レコード")}><div><strong>{evidenceRoleLabel(selectedSource.evidenceRole ?? 'PRIMARY', text)} {localize("・ 行")} {selectedSource.rowNumber}</strong><small>{localize("改変されていない取込時の値")}</small></div><pre>{formattedSourcePayload}</pre></section>)}{notice && <p role="status">{notice}</p>}<div className="detail-actions"><span>{detail.sourceEvidence.length > 0 ? localize("原本とのリンクを保持したまま修正します。") : localize("手動取引")}</span><button className="secondary-btn" disabled={busy} onClick={onClose}>{localize("キャンセル")}</button><button className="primary-btn" disabled={busy || debitTotal !== creditTotal} onClick={() => void save()}>{busy ? localize("保存中…") : localize("変更を保存")}</button></div></section></div>
}
...
function TransactionsPage({ householdId, accountGroupId, accountGroups, onAccountGroupChange, attributionScope, revision, month, basis, onBasisChange, accounts, members, onChanged }: { householdId: string | null; accountGroupId: string | null; accountGroups: readonly AccountGroupDto[]; onAccountGroupChange: (groupId: string | null) => void; attributionScope: AttributionScopeDto; revision: number; month: string; basis: 'ACCRUAL' | 'CASH'; onBasisChange: (basis: 'ACCRUAL' | 'CASH') => void; accounts: readonly AccountDto[]; members: readonly HouseholdMemberDto[]; onChanged: () => void }) {
  ...
  const [globalAmountRange, setGlobalAmountRange] = useState<{ min: number | null; max: number | null }>({ min: null, max: null })
  ...
  useEffect(() => {
    const receiveGlobalSearch = (event: Event) => {
      ...
      if (typeof detail?.query !== 'string' || !detail.query.trim()) return
      const raw = detail.query.trim()
      const amountPattern = /(?:¥|￥)?([\d,]+)\s*円?\s*(以上|超|以下|未満|over|under)/i
      const amount = raw.match(amountPattern)
      const value = amount ? Number(amount[1].replaceAll(',', '')) : null
      const direction = amount?.[2]?.toLowerCase()
      setGlobalAmountRange({ min: value !== null && ['以上', '超', 'over'].includes(direction ?? '') ? value : null, max: value !== null && ['以下', '未満', 'under'].includes(direction ?? '') ? value : null })
      setQuery(raw.replace(amountPattern, '').replace(/\s+/g, ' ').trim())
    }
    globalThis.addEventListener('kakeflow:global-search', receiveGlobalSearch)
    const receiveManualEntry = () => setShowManual(true)
    globalThis.addEventListener('kakeflow:manual-entry', receiveManualEntry)
    return () => { globalThis.removeEventListener('kakeflow:global-search', receiveGlobalSearch); globalThis.removeEventListener('kakeflow:manual-entry', receiveManualEntry) }
  }, [])
  ...
  const searchedRows = (desktop ? displayRows : displayRows.filter((t) => `${t.merchant}${t.category}${t.account}`.toLowerCase().includes(query.toLowerCase()))).filter((transaction) => {
    const amount = Math.abs(transaction.amount)
    return (globalAmountRange.min === null || amount >= globalAmountRange.min) && (globalAmountRange.max === null || amount <= globalAmountRange.max)
  })
  ...
  const openDetail = async (transactionId: string, trigger: HTMLButtonElement) => {
    ...
    catch { setDetailNotice(localize("取引詳細を読み込めませんでした。")) }
  }
  const saveDetail = async (input: UpdatePostedTransactionInputDto) => {
    await platformClient.updateTransaction(input)
    setSelectedDetail(null); onChanged(); setDetailNotice(localize("取引と仕訳を更新しました。"))
  }
  const toggleTransactionSelection = (transactionId: string, selected: boolean) => {
    setSelectedTransactionIds((current) => {
      const next = new Set(current)
      if (selected) {
        if (next.size >= 200) { setMetadataNotice(localize("一度に編集できる取引は200件までです。")); return current }
  ...
  const applyBulkMetadata = async () => {
    if (!householdId || selectedTransactionIds.size === 0) { setMetadataNotice(localize("編集する取引を選択してください。")); return }
    const tags = [...new Set(metadataTags.split(',').map((tag) => tag.trim().replace(/^#/, '')).filter(Boolean))]
    if (!metadataLabel && tags.length === 0) { setMetadataNotice(localize("ラベルまたはタグを入力してください。")); return }
    if (tags.length > 20 || tags.some((tag) => tag.length > 64)) { setMetadataNotice(localize("タグは一度に20個、1個64文字以内で入力してください。")); return }
    ...
    try {
      ...
      setMetadataNotice(localize(`${result.updatedCount}件のラベル・タグを${metadataOperation === 'ADD' ? localize("追加") : localize("削除")}しました。カテゴリーと仕訳は変更していません。`))
      showToast(localize(`${result.updatedCount}件を更新しました。`))
    } catch { setMetadataNotice(localize("ラベル・タグを更新できませんでした。選択内容を確認してください。")); showToast(localize("一括更新に失敗しました。"), 'error') }
  ...
  const applyBulkLedgerChange = async (kind: 'CATEGORY' | 'CALCULATION' | 'ATTRIBUTION') => {
    ...
    if (kind === 'CATEGORY' && !categoryAccount) { setMetadataNotice(localize("変更先のカテゴリーを選択してください。")); return }
    if (kind === 'CALCULATION' && !bulkCalculationTarget) { setMetadataNotice(localize("計算対象の状態を選択してください。")); return }
    if (kind === 'ATTRIBUTION' && !bulkAttribution) { setMetadataNotice(localize("変更先の帰属を選択してください。")); return }
    setMetadataBusy(true); setMetadataNotice('')
    let updated = 0
    let skipped = 0
    ...
    const action = kind === 'CATEGORY' ? localize("カテゴリー") : kind === 'CALCULATION' ? localize("計算対象") : localize("家族内の帰属")
    setMetadataNotice(localize(`${updated}件の${action}を更新しました。${skipped ? `${skipped}件は取引種別または照合状態により変更できませんでした。` : ''}`))
    showToast(localize(`${updated}件の${action}を更新しました。`), skipped ? 'info' : 'success')
  ...
  const createManual = async () => {
    ...
    if (!householdId || !/^\d+$/.test(manualAmount) || !Number.isSafeInteger(amount) || amount <= 0 || !manualDebit || !manualCredit || manualDebit === manualCredit) {
      setManualNotice(localize("金額と異なる借方・貸方口座を正しく入力してください。")); return
    }
    ...
    try {
      ...
      setManualPayee(''); setManualDescription(''); setManualAmount(''); setManualAttribution('HOUSEHOLD'); setManualAudience('SHARED'); setShowManual(false); setLedgerPage(1); onChanged(); setManualNotice(localize("手動取引を台帳に記録しました。")); showToast(localize("手動取引を台帳に記録しました。"))
    } catch { setManualNotice(localize("取引を記録できませんでした。日付と口座を確認してください。")) }
    finally { setManualBusy(false) }
  }
  const exportLedger = async (format: 'CSV' | 'XLSX' | 'PDF') => {
    if (!desktop || !householdId) { setExportNotice(localize("保存はデスクトップ版で利用できます。")); return }
    ...
    try {
      ...
      setExportNotice(saved ? localize(`${format}を保存しました。`) : localize("保存をキャンセルしました。"))
      if (saved) showToast(localize(`${format}を保存しました。`))
    } catch { setExportNotice(localize(`${format}を書き出せませんでした。`)); showToast(localize(`${format}を書き出せませんでした。`), 'error') }
  ...
  return <>
    <PageHeader eyebrow={localize("取引台帳")} title={localize("すべての取引")} description={localize("確定した取引と元データを一か所で管理します。")}>
      {desktop && <button className="primary-btn" onClick={() => setShowManual((value) => !value)}>{showManual ? localize("入力を閉じる") : localize("＋ 手入力取引")}</button>}
    </PageHeader>
    {showManual && <div className="manual-transaction-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !manualBusy) setShowManual(false) }}><section className="panel manual-transaction-dialog" role="dialog" aria-modal="true" aria-labelledby="manual-transaction-title"><div className="panel-head"><div><h2 id="manual-transaction-title">{localize("手入力取引（複式）")}</h2><p>{localize("借方と貸方を明示し、貸借差額が ¥0 の取引だけを確定台帳へ転記します。")}</p></div><button className="icon-btn" aria-label={localize("手動取引を閉じる")} disabled={manualBusy} onClick={() => setShowManual(false)}><X size={18} /></button></div><div className="manual-transaction-grid"><label>{localize("日付")}<input aria-label={localize("取引日")} type="date" value={manualDate} onChange={(event) => setManualDate(event.target.value)} /></label><label>{localize("取引種別")}<select aria-label={localize("手動取引種別")} value={manualType} onChange={(event) => setManualType(event.target.value as ManualTransactionTypeDto)}>{['EXPENSE', 'INCOME', 'TRANSFER', 'CARD_PURCHASE', 'CARD_PAYMENT', 'REFUND', 'FEE', 'INTEREST', 'ADJUSTMENT'].map((type) => <option key={type} value={type}>{transactionTypeLabel(type, text)}</option>)}</select></label><label>{localize("支払先")}<input aria-label={localize("手動取引の支払先")} value={manualPayee} onChange={(event) => setManualPayee(event.target.value)} placeholder={localize("店舗・支払先")} /></label><label>{localize("摘要")}<input aria-label={localize("手動取引のメモ")} value={manualDescription} onChange={(event) => setManualDescription(event.target.value)} placeholder={localize("任意")} /></label><label className="manual-amount-field">{localize("金額 (JPY)")}<input aria-label={localize("手動取引の金額")} inputMode="numeric" value={manualAmount} onChange={(event) => setManualAmount(event.target.value)} placeholder="1500" /></label><fieldset className="compact-assignment"><legend>{localize("家族内の帰属")}</legend><select aria-label={localize("手動取引の家族内の帰属")} value={manualAttribution} onChange={(event) => setManualAttribution(event.target.value)}><option value="HOUSEHOLD">{localize("世帯共通")}</option>{members.filter((member) => member.status === 'ACTIVE').map((member) => <option key={member.id} value={member.id}>{member.displayName}</option>)}</select></fieldset><label className="manual-journal-side debit"><span>{localize("借方")}</span><select aria-label={localize("手動取引の借方口座")} value={manualDebit} onChange={(event) => setManualDebit(event.target.value)}><option value="">{localize("口座を選択")}</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select><b>{manualAmountValue > 0 ? yen(manualAmountValue) : '—'}</b></label><label className="manual-journal-side credit"><span>{localize("貸方")}</span><select aria-label={localize("手動取引の貸方口座")} value={manualCredit} onChange={(event) => setManualCredit(event.target.value)}><option value="">{localize("口座を選択")}</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select><b>{manualAmountValue > 0 ? yen(manualAmountValue) : '—'}</b></label><fieldset className="compact-assignment"><legend>{localize("表示区分")}</legend><select aria-label={localize("手動取引の表示区分")} value={manualAudience} onChange={(event) => setManualAudience(event.target.value)}><option value="SHARED">{localize("共有")}</option>{members.filter((member) => member.status === 'ACTIVE').map((member) => <option key={member.id} value={member.id}>{localize("個人・")}{member.displayName}</option>)}</select></fieldset></div><p className="local-label-help">{localize("家族内帰属は分析対象、表示区分は端末内の整理ラベルです。「個人」も閲覧制限ではありません。")}</p><div className={`manual-balance-check${manualBalanced ? ' balanced' : ''}`}><span>{localize("貸借差額")}</span><strong>{manualBalanced ? '¥0 ✓' : localize("入力を確認")}</strong></div>{manualNotice && <p role="status">{manualNotice}</p>}<div className="manual-dialog-actions"><button className="secondary-btn" disabled={manualBusy} onClick={() => setShowManual(false)}>{localize("キャンセル")}</button><button className="primary-btn" disabled={manualBusy || !manualBalanced} onClick={() => void createManual()}>{manualBusy ? localize("転記中…") : localize("転記")}</button></div></section></div>}
    ...
    <section className="panel table-panel">
      <div className="table-toolbar"><div className="search table-search"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={text('店舗、カテゴリー、口座を検索')} /></div><div className="transaction-export-actions" aria-label={localize("取引を書き出す")}><button className="secondary-btn" disabled={exportBusy} onClick={() => void exportLedger('CSV')}>CSV</button><button className="secondary-btn" disabled={exportBusy} onClick={() => void exportLedger('XLSX')}>Excel</button><button className="secondary-btn" disabled={exportBusy} onClick={() => void exportLedger('PDF')}><Download size={15} /> PDF</button></div></div>
      <div className="transaction-filter-row"><div className="transaction-type-filters" aria-label={localize("取引種別フィルター")}>{([['ALL', localize("すべて")], ['INCOME', localize("収入")], ['EXPENSE', localize("支出")], ['TRANSFER_PAYMENT', localize("振替・カード支払")], ['REFUND', localize("返金")]] as const).map(([value, label]) => <button key={value} className={transactionTypeFilter === value ? 'active' : ''} aria-pressed={transactionTypeFilter === value} onClick={() => setTransactionTypeFilter(value)}>{localize(label)}</button>)}</div><button className="secondary-btn advanced-filter-trigger" aria-expanded={showAdvancedFilters} onClick={() => setShowAdvancedFilters((value) => !value)}><Layers size={15} />{localize("詳細フィルタ")}</button></div>
      {showAdvancedFilters && <div className="metadata-filters advanced-filter-panel" role="group" aria-label={localize("詳細フィルター")}><div className="basis-toggle" aria-label={text('計上基準')}><button className={basis === 'ACCRUAL' ? 'active' : ''} onClick={() => onBasisChange('ACCRUAL')}>{text('発生ベース')}</button><button className={basis === 'CASH' ? 'active' : ''} onClick={() => onBasisChange('CASH')}>{text('資金移動')}</button><button disabled title={localize("残高ベースは読み取りモデル対応後に有効になります")}>{localize("残高")}</button></div><label>{localize("計算対象")}<select aria-label={localize("計算対象で絞り込み")} value={calculationFilter} onChange={(event) => setCalculationFilter(event.target.value as typeof calculationFilter)}><option value="ALL">{localize("すべて")}</option><option value="INCLUDED">{localize("対象のみ")}</option><option value="EXCLUDED">{localize("対象外のみ")}</option></select></label>{desktop && <><label>{localize("ラベル")}<select aria-label={localize("ラベルで絞り込み")} value={labelFilter} onChange={(event) => setLabelFilter(event.target.value as TransactionLabelDto | '')}><option value="">{localize("すべて")}</option>{Object.entries(transactionLabelNames).map(([value, label]) => <option key={value} value={value}>{localize(label)}</option>)}</select></label><label>{localize("タグ")}<input aria-label={localize("タグで絞り込み")} value={tagFilter} onChange={(event) => setTagFilter(event.target.value.replace(/^#/, ''))} placeholder={localize("旅行")} /></label><label>{localize("口座グループ")}<select aria-label={localize("口座グループで絞り込み")} value={accountGroupId ?? ''} onChange={(event) => onAccountGroupChange(event.target.value || null)}><option value="">{localize("すべての口座")}</option>{accountGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label></>}<button className="text-btn" onClick={() => { setCalculationFilter('ALL'); setLabelFilter(''); setTagFilter(''); onAccountGroupChange(null) }}>{localize("クリア")}</button></div>}
      {(calculationFilter !== 'ALL' || labelFilter || tagFilter || accountGroupId) && <div className="active-filter-chips" aria-label={localize("適用中のフィルター")}>{calculationFilter !== 'ALL' && <button onClick={() => setCalculationFilter('ALL')}>{calculationFilter === 'INCLUDED' ? localize("計算対象") : localize("集計対象外")} <X size={12} /></button>}{labelFilter && <button onClick={() => setLabelFilter('')}>{localize("ラベル:")} {localize(transactionLabelNames[labelFilter])} <X size={12} /></button>}{tagFilter && <button onClick={() => setTagFilter('')}>#{tagFilter} <X size={12} /></button>}{accountGroupId && <button onClick={() => onAccountGroupChange(null)}>{localize("口座:")} {accountGroups.find((group) => group.id === accountGroupId)?.name ?? accountGroupId} <X size={12} /></button>}<button className="clear" onClick={() => { setCalculationFilter('ALL'); setLabelFilter(''); setTagFilter(''); onAccountGroupChange(null) }}>{localize("すべてクリア")}</button></div>}
      {desktop && selectedTransactionIds.size > 0 && <section className="bulk-metadata-toolbar" aria-label={localize("選択した取引の一括編集")}><div className="bulk-selection"><button type="button" className="secondary-btn" onClick={toggleVisibleTransactions}>{visible.length > 0 && visible.every((row) => selectedTransactionIds.has(row.id)) ? localize("表示中の選択を解除") : localize("表示中を選択")}</button><strong>{selectedTransactionIds.size}{localize("件選択")}</strong><small>{localize("最大200件")}</small><button type="button" className="text-btn" onClick={() => setSelectedTransactionIds(new Set())}>{localize("すべて解除")}</button></div><div className="bulk-edit-group"><strong>{localize("カテゴリー")}</strong><select aria-label={localize("一括変更するカテゴリー")} value={bulkCategoryAccountId} onChange={(event) => setBulkCategoryAccountId(event.target.value)}><option value="">{localize("変更先を選択")}</option><optgroup label={localize("支出")}>{accounts.filter((account) => account.accountKind === 'EXPENSE').map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</optgroup><optgroup label={localize("収入")}>{accounts.filter((account) => account.accountKind === 'INCOME').map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</optgroup></select><button type="button" className="secondary-btn" disabled={metadataBusy || !bulkCategoryAccountId} onClick={() => void applyBulkLedgerChange('CATEGORY')}>{localize("一括変更")}</button></div><div className="bulk-edit-group"><strong>{localize("集計")}</strong><select aria-label={localize("一括変更する計算対象")} value={bulkCalculationTarget} onChange={(event) => setBulkCalculationTarget(event.target.value as 'INCLUDED' | 'EXCLUDED' | '')}><option value="">{localize("状態を選択")}</option><option value="INCLUDED">{localize("計算対象")}</option><option value="EXCLUDED">{localize("集計対象外")}</option></select><button type="button" className="secondary-btn" disabled={metadataBusy || !bulkCalculationTarget} onClick={() => void applyBulkLedgerChange('CALCULATION')}>{localize("一括変更")}</button></div><div className="bulk-edit-group"><strong>{localize("帰属")}</strong><select aria-label={localize("一括変更する家族内の帰属")} value={bulkAttribution} onChange={(event) => setBulkAttribution(event.target.value)}><option value="">{localize("変更先を選択")}</option><option value="HOUSEHOLD">{localize("世帯共通")}</option>{members.filter((member) => member.status === 'ACTIVE').map((member) => <option key={member.id} value={member.id}>{member.displayName}</option>)}</select><button type="button" className="secondary-btn" disabled={metadataBusy || !bulkAttribution} onClick={() => void applyBulkLedgerChange('ATTRIBUTION')}>{localize("帰属を変更")}</button></div><div className="bulk-edit-group bulk-label-group"><strong>{localize("ラベル・タグ")}</strong><select aria-label={localize("ラベルとタグの操作")} value={metadataOperation} onChange={(event) => setMetadataOperation(event.target.value as 'ADD' | 'REMOVE')}><option value="ADD">{localize("追加")}</option><option value="REMOVE">{localize("削除")}</option></select><select aria-label={localize("一括編集するラベル")} value={metadataLabel} onChange={(event) => setMetadataLabel(event.target.value as TransactionLabelDto | '')}><option value="">{localize("ラベル変更なし")}</option>{Object.entries(transactionLabelNames).map(([value, label]) => <option key={value} value={value}>{localize(label)}</option>)}</select><input aria-label={localize("一括編集するタグ")} value={metadataTags} onChange={(event) => setMetadataTags(event.target.value)} placeholder={localize("タグ（カンマ区切り）")} /><button type="button" className="primary-btn" disabled={metadataBusy} onClick={() => void applyBulkMetadata()}>{metadataBusy ? localize("更新中…") : localize(`${metadataOperation === 'ADD' ? localize("追加") : localize("削除")}を適用`)}</button></div><p>{localize("帰属の変更は原本の表示区分を変更しません。カテゴリー変更できないカード支払・振替は安全にスキップします。")}</p></section>}
      ...
      <div className="table-summary"><span>{month}・{basis === 'ACCRUAL' ? localize("発生ベース") : localize("資金移動ベース")}{localize("・家計集計は計算対象のみ")}</span><strong>{localize("収入")} {yen(basisIncome)}</strong><strong>{basis === 'ACCRUAL' ? localize("支出") : localize("現金流出")} {yen(basisExpense)}</strong><em>{desktop ? localize(`${totalItems}件中 ${visible.length}件`) : localize(`${visible.length}件を表示`)}</em></div>
      {loadError ? <p className="empty-state">{text('台帳を読み込めませんでした。')}</p> : visible.length > 0 ? <TransactionRows compact={false} rows={visible} onSelect={desktop ? (id, trigger) => void openDetail(id, trigger) : undefined} selectedIds={selectedTransactionIds} onToggleSelection={toggleTransactionSelection} /> : <p className="empty-state">{localize("該当する取引がありません")}</p>}
...
function applyLocalCategorySuggestionToDecision(decision: PostingDecisionDto, suggestion: LocalCategorySuggestion, accounts: readonly AccountDto[]): PostingDecisionDto | null {
  if (!eligibleClassificationDecision(decision, accounts) || !accounts.some((account) => account.id === suggestion.categoryAccountId && account.accountKind === 'EXPENSE')) return null
  const expenseEntries = decision.entries.filter((entry) => accounts.find((account) => account.id === entry.accountId)?.accountKind === 'EXPENSE')
  if (expenseEntries.length !== 1) return null
  return clearClassificationProvenance({
    ...decision,
    entries: decision.entries.map((entry) => entry.id === expenseEntries[0].id ? { ...entry, accountId: suggestion.categoryAccountId } : entry),
  })
}
...
function decisionUsesCategory(decision: PostingDecisionDto, categoryAccountId: string, accounts: readonly AccountDto[]): boolean {
  const expectedSide = decision.transactionType === 'REFUND' ? 'CREDIT' : 'DEBIT'
  return decision.entries.some((entry) => entry.side === expectedSide && entry.accountId === categoryAccountId && accounts.find((account) => account.id === entry.accountId)?.accountKind === 'EXPENSE')
}

function LocalCategorySuggestionView({ suggestion, merchantLabel, selected, onApply }: { suggestion: LocalCategorySuggestion; merchantLabel: string; selected: boolean; onApply: () => void }) {
  const { locale, text } = useI18n()
  const confidence = Math.round(suggestion.confidenceBps / 100)
  const source = text(suggestion.source === 'HISTORY' ? "確定済み履歴から提案" : "ローカルキーワードから提案")
  const reason = suggestion.source === 'HISTORY'
    ? locale === 'ja' ? `${suggestion.sampleCount}件の類似取引・${suggestion.historySharePercent ?? 0}%がこのカテゴリー${suggestion.amountConsistent === false ? '・金額差を考慮済み' : ''}` : locale === 'vi' ? `${suggestion.sampleCount} giao dịch tương tự; ${suggestion.historySharePercent ?? 0}% thuộc danh mục này${suggestion.amountConsistent === false ? '; đã giảm độ tin cậy do chênh lệch số tiền' : ''}` : `${suggestion.sampleCount} similar transactions; ${suggestion.historySharePercent ?? 0}% used this category${suggestion.amountConsistent === false ? '; confidence reduced for the amount difference' : ''}`
    : locale === 'ja' ? `キーワード「${suggestion.matchedKeyword ?? ''}」に一致` : locale === 'vi' ? `Khớp từ khóa “${suggestion.matchedKeyword ?? ''}”` : `Matched keyword “${suggestion.matchedKeyword ?? ''}”`
  return <div className="classification-rule-suggestion classification-local-suggestion"><span><strong>{source}</strong><small>{suggestion.categoryName} ・ {text("信頼度")} {confidence}%</small><small>{reason} ・ {text("端末内だけで判定")}</small></span><button type="button" className="mini-btn" aria-label={`${merchantLabel} — ${text("ローカルカテゴリー候補を適用")}`} disabled={selected} onClick={onApply}>{selected ? text("選択済み") : text("提案を適用")}</button></div>
}
...
function initialPostingDraft(candidate: PreviewCandidateDto, accounts: readonly AccountDto[], householdId: string): PostingDraft {
  try {
    ...
    return {
      approved: false,
      touched: false,
      error: localize("取込先または相手勘定が見つかりません。口座設定を確認するか、このインポートを取り消してください。"),
      decision: {
        candidateId: candidate.id,
        transactionId: globalThis.crypto.randomUUID(),
...
export function DuplicateReviewGate({ preview, onResolution }: { preview: ImportPreviewDto; onResolution: (candidateId: string, resolution: 'LINK' | 'KEEP_BOTH' | 'EXCLUDE') => void | Promise<void> }) {
  ...
  const reasonLabel: Record<string, string> = { SAME_ACCOUNT: localize("同じ口座"), SAME_CURRENCY: localize("同じ通貨"), SAME_DIRECTION: localize("同じ入出金方向"), SAME_AMOUNT: localize("同じ金額"), SAME_EFFECTIVE_DATE: localize("同じ取引日"), SAME_NORMALIZED_TEXT: localize("支払先・摘要が一致"), OTHER_ACTIVE_REVIEW: localize("別の確認待ちにも存在") }
  return <section className="panel duplicate-review-gate" aria-label={localize(`${preview.source.originalFilename}の重複レビュー`)}>
    <div className="duplicate-file-summary"><strong>{localize("重複チェック")}</strong><span>{preview.candidates.length}{localize("候補:")} {summary.confirmedReplays}{localize("件は取込済み・")}{summary.likelyDuplicates}{localize("件は重複の可能性が高い・")}{summary.possibleDuplicates}{localize("件は要確認")}</span>{summary.overlapStart && <small>{localize("既存データとの重複期間:")} {summary.overlapStart} → {summary.overlapEnd}</small>}</div>
    {affected.map((candidate) => { const match = candidate.duplicateMatch!; const selected = match.decision === 'UNRESOLVED' ? undefined : match.decision; return <article className="duplicate-comparison" key={candidate.id}>
      <header><strong>{match.confidence === 'LIKELY' ? localize("重複の可能性が高い") : localize("重複の可能性あり")}</strong><span>{match.reasons.map((reason) => reasonLabel[reason] ?? reason).join(' + ')}</span></header>
      <div><section><small>{localize("今回の候補")}</small><b>{candidate.occurredOn} ・ {yen(candidate.amountJpy)}</b><span>{candidate.merchantRaw ?? candidate.descriptionRaw ?? localize("支払先なし")}</span><em>{preview.source.originalFilename}</em></section><section><small>{localize("既存または確認待ち")}</small><b>{match.occurredOn} ・ {yen(match.amountJpy)}</b><span>{match.payee ?? match.description ?? localize("支払先なし")}</span><em>{match.sourceFilename ?? match.matchedTransactionId ?? match.matchedCandidateId ?? localize("確認待ち")}</em></section></div>
      <footer role="group" aria-label={localize(`${candidate.id}の重複判定`)}><button type="button" aria-label={localize("追加証憑として紐付け")} aria-pressed={selected === 'LINK'} disabled={!match.matchedTransactionId} onClick={() => void onResolution(candidate.id, 'LINK')}>{localize("リンク")}</button><button type="button" aria-label={localize("別取引として両方残す")} aria-pressed={selected === 'KEEP_BOTH'} onClick={() => void onResolution(candidate.id, 'KEEP_BOTH')}>{localize("両方保持")}</button><button type="button" aria-label={localize("このソース行を除外")} aria-pressed={selected === 'EXCLUDE'} onClick={() => void onResolution(candidate.id, 'EXCLUDE')}>{localize("除外")}</button></footer>
    </article> })}
  </section>
}
...
function ImportReviewSection({ stagedImport, accounts, householdId, busy, isReceipt, recovered, sourceCompletionBlocked, bindingValid, onRollback, onCommit, onReceiptLinked }: { stagedImport: ImportPreviewDto; accounts: readonly AccountDto[]; householdId: string; busy: boolean; isReceipt: boolean; recovered: boolean; sourceCompletionBlocked: boolean; bindingValid: boolean; onRollback: () => void; onCommit: (decisions: readonly PostingDecisionDto[]) => void; onReceiptLinked: () => Promise<void> }) {
  ...
  const [classificationHistory, setClassificationHistory] = useState<readonly TransactionRowDto[]>([])
  const [classificationLoadFailed, setClassificationLoadFailed] = useState(false)
  const [classificationHistoryLoadFailed, setClassificationHistoryLoadFailed] = useState(false)
  ...
  useEffect(() => {
    if (!hasEligibleClassificationCandidates) return
    let active = true
    void platformClient.queryTransactions({ householdId, attributionScope: ALL_ATTRIBUTION_SCOPE, accountingBasis: 'ACCRUAL', calculationTargetFilter: 'INCLUDED', page: 1, pageSize: 100 })
      .then((page) => { if (active) { setClassificationHistory(page.items); setClassificationHistoryLoadFailed(false) } })
      .catch(() => { if (active) { setClassificationHistory([]); setClassificationHistoryLoadFailed(true) } })
    return () => { active = false }
  }, [hasEligibleClassificationCandidates, householdId])
  useEffect(() => {
    let active = true
  ...
  const localSuggestionFor = (candidateId: string, draft: PostingDraft): LocalCategorySuggestion | null => {
    const candidate = stagedImport.candidates.find((item) => item.id === candidateId)
    return suggestLocalCategory({ merchant: draft.decision.payee, description: draft.decision.description, transactionType: draft.decision.transactionType, amountJpy: candidate?.amountJpy }, classificationHistory, accounts)
  }
  const applyLocalSuggestion = (candidateId: string, suggestion: LocalCategorySuggestion) => {
    const latest = draftsRef.current[candidateId]
    if (!latest || !eligibleClassificationDecision(latest.decision, accounts)) return
    const decision = applyLocalCategorySuggestionToDecision(latest.decision, suggestion, accounts)
    if (!decision) return
    setDrafts((current) => current[candidateId] !== latest ? current : { ...current, [candidateId]: { ...latest, approved: false, touched: true, decision } })
    setClassificationNotices((current) => ({ ...current, [candidateId]: localize("ローカル候補を適用しました。承認はまだ行われていません。") }))
  }
  const applyRuleSuggestion = async (candidateId: string) => {
    ...
    try {
      ...
      setClassificationNotices((current) => ({ ...current, [candidateId]: localize(`${rule.name} を適用しました。承認はまだ行われていません。`) }))
    } catch {
      setClassificationNotices((current) => ({ ...current, [candidateId]: localize("ルールを再確認できませんでした。候補は変更せず、手動で確認できます。") }))
    } finally { setApplyingClassification(null) }
  }
  const applyAllClassificationSuggestions = () => {
    let appliedCount = 0
    ...
    const nextDrafts = Object.fromEntries(Object.entries(draftsRef.current).map(([candidateId, draft]) => {
      ...
      const localSuggestion = rule ? null : localSuggestionFor(candidateId, draft)
      if (!rule && (!localSuggestion || localSuggestion.confidenceBps < HIGH_CONFIDENCE_CATEGORY_BPS)) return [candidateId, draft]
      const decision = rule ? applyClassificationRuleToDecision(draft.decision, rule, accounts) : applyLocalCategorySuggestionToDecision(draft.decision, localSuggestion!, accounts)
      if (!decision) return [candidateId, draft]
      appliedCount += 1
      if (!rule) localAppliedCount += 1
      return [candidateId, { ...draft, approved: false, touched: true, decision }]
    }))
    if (appliedCount > 0) {
      setDrafts(nextDrafts)
      const notice = localAppliedCount === 0
        ? `${appliedCount}件にルール候補を適用しました。承認はまだ行われていません。`
        : `${appliedCount}件にカテゴリー候補を適用しました。承認はまだ行われていません。`
      setClassificationNotices((current) => ({ ...current, all: localize(notice) }))
    }
  }
  ...
  const untouchedSuggestionCount = Object.entries(drafts).filter(([candidateId, draft]) => {
    if (draft.touched || !eligibleClassificationDecision(draft.decision, accounts)) return false
    if (matchingClassificationRule(classificationRules, draft.decision.payee, draft.decision.description)) return true
    return (localSuggestionFor(candidateId, draft)?.confidenceBps ?? 0) >= HIGH_CONFIDENCE_CATEGORY_BPS
  }).length
  ...
  if (sourceOnly && sourceCompletionBlocked) return <section className="panel review-panel"><div className="panel-head"><div><h2>{stagedImport.source.originalFilename}</h2><p>{localize("投資・資産データ")}{recovered ? localize("・再起動後に復元") : ''}</p></div><b>{recovered ? localize("復元済み") : localize("確認")}</b></div><p className="empty-state" role="alert">{localize("専用データの保存が完了する前に中断されました。この取込を取り消し、原本ファイルを再度取り込んでください。")}</p><div className="review-actions"><span>{localize("未保存の投資データを完了済みとして扱いません。")}</span><button className="secondary-btn" disabled={busy} onClick={onRollback}>{busy ? localize("処理中…") : localize("取り消して再取込")}</button></div></section>

  return <section className="panel review-panel"><div className="panel-head"><div><h2>{stagedImport.source.originalFilename}</h2><p>{stagedImport.candidates.length}{localize("件の候補・原本は暗号化済み")}{recovered ? localize("・再起動後に復元") : ''}</p></div><b>{recovered ? localize("復元済み") : localize("確認")}</b></div>{hasEligibleClassificationCandidates && <div className="classification-review-toolbar"><span><strong>{localize("分類ルール候補")}</strong><small>{localize("保存済みルール、確定済み履歴、端末内キーワードの順で提案します。承認や台帳反映は自動で行いません。")}</small></span><button type="button" className="secondary-btn" disabled={untouchedSuggestionCount === 0} onClick={applyAllClassificationSuggestions}>{localize("未編集の候補に一括適用（")}{untouchedSuggestionCount}{localize("件）")}</button>{classificationLoadFailed && <small role="status">{localize("保存済みルールを読み込めませんでした。履歴とキーワードによる候補は引き続き利用できます。")}</small>}{classificationHistoryLoadFailed && <small role="status">{localize("取引履歴を読み込めませんでした。保存済みルールとキーワードで提案します。")}</small>}{classificationNotices.all && <small role="status">{classificationNotices.all}</small>}</div>}{!sourceOnly && <div className="candidate-approval-toolbar"><label><input aria-label={localize("すべての承認可能な候補を承認")} type="checkbox" checked={allEligibleApproved} disabled={bulkApprovalEligibleIds.size === 0 || busy} onChange={(event) => setAllEligibleApprovals(event.target.checked)} /><span>{localize("すべて承認")}</span></label><small>{bulkApprovedCount}/{bulkApprovalEligibleIds.size}{localize("件承認済み")}{bulkApprovalEligibleIds.size < stagedImport.candidates.length ? localize(`・未解決の${stagedImport.candidates.length - bulkApprovalEligibleIds.size}件は対象外`) : ''}</small></div>}<div className="candidate-review-list">{stagedImport.candidates.map((candidate) => { const draft = drafts[candidate.id]; const draftValid = validatePostingDecision(draft.decision, { candidateAmountJpy: candidate.amountJpy, accountIds, expectedCandidateId: candidate.id }).valid; return <div className="candidate-review-row candidate-review-edit" key={candidate.id}><ReceiptReviewPanel candidate={candidate} decision={draft.decision} accounts={accounts} onDecisionChange={(decision) => updateDecision(candidate.id, () => decision)} /><label><input aria-label={localize(`${candidate.merchantRaw ?? candidate.descriptionRaw ?? candidate.id}を承認`)} type="checkbox" checked={draft.approved} disabled={!draftValid} onChange={(event) => setDrafts((current) => ({ ...current, [candidate.id]: { ...current[candidate.id], approved: event.target.checked } }))} /><span>{localize("承認")}</span></label><div><input aria-label={localize(`${candidate.id}の支払先`)} value={draft.decision.payee ?? ''} onChange={(event) => updateDecision(candidate.id, (decision) => ({ ...decision, payee: event.target.value || null }))} /><span>{candidate.occurredOn} ・ {directionLabel(candidate.direction, text)} ・ {yen(candidate.amountJpy)}</span>{candidate.institutionRaw && <small>{candidate.institutionRaw} ・ {[candidate.categoryMajorRaw, candidate.categoryMinorRaw].filter(Boolean).join(' / ')}{candidate.externalTransactionId ? ` ・ ID ${candidate.externalTransactionId}` : ''}</small>}</div><select aria-label={localize(`${candidate.id}の取引種別`)} value={draft.decision.transactionType} onChange={(event) => updateDecision(candidate.id, (decision) => ({ ...decision, transactionType: event.target.value }))}>{['EXPENSE', 'CARD_PURCHASE', 'CARD_PAYMENT', 'INCOME', 'REFUND', 'ADJUSTMENT', 'TRANSFER'].map((type) => <option key={type} value={type}>{transactionTypeLabel(type, text)}</option>)}</select><PostingEntryEditor candidateId={candidate.id} candidateAmountJpy={candidate.amountJpy} decision={draft.decision} accounts={accounts} onChange={(decision) => updateDecision(candidate.id, () => decision)} /><label><input type="checkbox" checked={draft.decision.calculationTarget} disabled={candidate.suggestedTransactionType === 'TRANSFER'} onChange={(event) => updateDecision(candidate.id, (decision) => ({ ...decision, calculationTarget: event.target.checked }))} /><span>{localize("家計集計に含める")}</span></label>{eligibleClassificationDecision(draft.decision, accounts) && (() => { const rule = matchingClassificationRule(classificationRules, draft.decision.payee, draft.decision.description); const localSuggestion = rule ? null : localSuggestionFor(candidate.id, draft); const ruleNotice = classificationNotices[candidate.id]; return rule ? <div className="classification-rule-suggestion"><span><strong>{rule.name}</strong><small>{rule.categoryName}{rule.labels.length > 0 ? ` ・ ${rule.labels.join(' / ')}` : ''}{rule.tags.length > 0 ? ` ・ #${rule.tags.join(' #')}` : ''}</small></span><button type="button" className="mini-btn" aria-label={localize(`${candidate.merchantRaw ?? candidate.descriptionRaw ?? candidate.id}の分類ルール候補を適用`)} disabled={applyingClassification === candidate.id} onClick={() => void applyRuleSuggestion(candidate.id)}>{applyingClassification === candidate.id ? localize("再確認中…") : draft.decision.classificationRuleId === rule.id ? localize("適用済み・再確認") : localize("提案を適用")}</button>{ruleNotice && <small role="status">{ruleNotice}</small>}</div> : localSuggestion ? <><LocalCategorySuggestionView suggestion={localSuggestion} merchantLabel={candidate.merchantRaw ?? candidate.descriptionRaw ?? candidate.id} selected={decisionUsesCategory(draft.decision, localSuggestion.categoryAccountId, accounts) && draft.touched} onApply={() => applyLocalSuggestion(candidate.id, localSuggestion)} />{ruleNotice && <small className="classification-rule-notice" role="status">{ruleNotice}</small>}</> : ruleNotice ? <small className="classification-rule-notice" role="status">{ruleNotice}</small> : null })()}{isReceipt && (receiptMatches[candidate.id]?.length ?? 0) > 0 && <div className="receipt-match-review"><strong>{localize("既存取引の候補")}</strong><select aria-label={localize(`${candidate.id}のレシート紐付け候補`)} value={receiptSelections[candidate.id] ?? ''} onChange={(event) => setReceiptSelections((current) => ({ ...current, [candidate.id]: event.target.value }))}>{receiptMatches[candidate.id].map((match) => <option key={match.transactionId} value={match.transactionId}>{match.occurredOn} ・ {match.payee ?? match.description ?? match.transactionId} ・ {yen(match.amountJpy)} ・ {Math.round(match.scoreBps / 100)}%</option>)}</select><button className="mini-btn" disabled={matchingCandidate === candidate.id} onClick={() => void linkReceipt(candidate.id)}>{matchingCandidate === candidate.id ? localize("紐付け中…") : localize("新規支出を作らず証憑として紐付け")}</button><small>{localize("金額一致・日付差3日以内の確定済み支出だけを表示します。自動紐付けはしません。")}</small></div>}{candidate.issues.length > 0 && <small>{candidate.issues.map((issue) => localize(issue)).join(', ')}</small>}</div> })}{sourceOnly && <p className="empty-state">{localize("台帳候補のない原本処理です。内容を確認して完了するか、取り消してください。")}</p>}</div><div className="review-actions"><span>{sourceOnly ? localize("台帳へ取引は追加されません") : approved ? localize("全候補を承認済み") : localize("各候補の口座と種別を確認して承認してください")}</span><button className="secondary-btn" disabled={busy} onClick={onRollback}>{localize("取り消す")}</button><button className="primary-btn" disabled={busy || !approved || decisions.length !== stagedImport.candidates.length} onClick={() => onCommit(decisions)}>{busy ? localize("処理中…") : sourceOnly ? localize("原本処理を完了") : localize("承認済みを台帳へ反映")}</button></div></section>
}
...
export function ImportPage({ previews, setPreviews, householdId, accounts, members, summary, onChanged, folderInbox, initialView = 'LOCAL' }: { previews: ImportPreview[]; setPreviews: React.Dispatch<React.SetStateAction<ImportPreview[]>>; householdId: string | null; accounts: readonly AccountDto[]; members: readonly HouseholdMemberDto[]; summary: ImportRunCountsDto | null; onChanged: () => void; folderInbox: DurableFolderInboxView; initialView?: 'LOCAL' | 'CONNECTORS' }) {
  ...
  useEffect(() => {
    ...
    void platformClient.listPendingReviews(householdId).then(async (result) => {
      ...
      if (failedCount > 0) setRecoveryError(localize(`${failedCount}件の確認待ちを復元できませんでした。他の候補は表示しています。`))
    }).catch(() => { if (active) setRecoveryError(localize("確認待ちのインポートを復元できませんでした。再試行してください。")) })
  ...
  const processFiles = async (files: FileList | readonly File[], sourceType: 'MANUAL_UPLOAD' | 'LOCAL_FOLDER' | 'ICLOUD_PICKER' = 'MANUAL_UPLOAD') => {
    ...
    try {
      ...
      setNotice(localize(`${next.length}件のファイルを読み取りました。状態と次の操作を確認してください。`))
    } catch {
      setNotice(localize("ファイルを読み取れませんでした。形式、サイズ、アクセス権を確認して再試行してください。"))
    } finally {
      setBusy(false)
    }
  }
    ...
    try {
      ...
      for (const expected of previewable) {
        ...
        try {
          ...
          setNotice(localize(`Google Drive の「${expected.fileName}」を安全にプレビューできませんでした。同期後に再試行してください。`))
        }
      }
    } catch {
      setNotice(localize("Google Drive Inbox を読み込めませんでした。接続状態を確認してください。"))
    } finally {
  ...
  const retryGoogleDriveInboxItem = async (itemId: string) => {
    ...
    } catch { setNotice(localize("Google Drive のファイルを再試行できませんでした。")) }
  ...
  const ignoreGoogleDriveInboxItem = async (itemId: string) => {
    ...
    } catch { setNotice(localize("Google Drive のファイルを無視できませんでした。")) }
    finally { setDriveInboxBusy(false) }
  }
    ...
    try {
      ...
      for (const expected of items.filter((item) => isGmailInboxPreviewable(item) && !hydratedGmailItemsRef.current.has(item.id)).slice(0, 20)) {
        ...
        try {
          ...
          setNotice(localize(`Gmail の「${expected.fileName}」をプレビューできませんでした。同期後に再試行してください。`))
        }
      }
    } catch { setNotice(localize("Gmail Inbox を読み込めませんでした。接続状態を確認してください。")) }
    finally { setGmailInboxBusy(false) }
  }, [householdId, setPreviews])

  const retryGmailInboxItem = async (itemId: string) => { if (!householdId) return; setGmailInboxBusy(true); try { await platformClient.retryGmailInboxItem(householdId, itemId); hydratedGmailItemsRef.current.delete(itemId); await refreshGmailInbox(true) } catch { setNotice(localize("Gmail のメールを再試行できませんでした。")) } finally { setGmailInboxBusy(false) } }
  const ignoreGmailInboxItem = async (itemId: string) => { if (!householdId) return; setGmailInboxBusy(true); try { await platformClient.ignoreGmailInboxItem(householdId, itemId); hydratedGmailItemsRef.current.add(itemId); setPreviews((current) => current.filter((preview) => preview.gmailInboxItemId !== itemId)); await refreshGmailInbox(false) } catch { setNotice(localize("Gmail のメールを無視できませんでした。")) } finally { setGmailInboxBusy(false) } }
  ...
  useEffect(() => {
    ...
    void platformClient.listWatchedFolders(householdId).then(setWatchedFolders).catch(() => setNotice(localize("監視フォルダーを読み込めませんでした。")))
  }, [householdId])
  ...
  const applyCustomParserProfile = (item: ImportPreview, explicitProfile?: DelimitedParserProfileDto, explicitAccountId?: string) => {
    ...
    setNotice(result.preview.candidateCount > 0 && !hasErrors ? localize(`${profile.name} で ${result.preview.candidateCount}件を候補化しました。台帳への反映前に内容を確認してください。`) : localize("エラーを含むため取込を開始できません。ヘッダーの一致、除外行と問題を確認してください。"))
  }
  ...
  const selectCustomParserProfile = (item: ImportPreview, profileId: string) => {
    ...
    setNotice(localize("プロファイルを変更しました。「適用してプレビュー」をもう一度実行してください。"))
  }
  ...
  const addWatchedFolder = async () => {
    ...
    try {
      const selected = await platformClient.selectWatchedFolder(householdId, localize("家計簿 Inbox"))
    ...
    } catch { setNotice(localize("フォルダーを登録できませんでした。シンボリックリンクではないローカルフォルダーを選択してください。")) }
  ...
  const connectIcloudFolder = async () => {
    ...
    try {
      ...
      if (!selected) { setNotice(localize("iCloud Drive フォルダーの接続をキャンセルしました。")); return }
      ...
      await folderInbox.refresh(true)
      setNotice(localize("iCloud Drive の同期済みローカルフォルダーを永続 Inbox に接続しました。台帳へは自動反映しません。"))
    } catch {
  ...
  const scanWatchedFolder = async (folder: WatchedFolderDto) => {
    ...
    try { await scanForNewFiles([folder]); setNotice(localize("同期フォルダーを確認し、永続 Inbox を更新しました。")) }
    catch { setNotice(localize("フォルダーを安全にスキャンできませんでした。同期状態とアクセス権を確認してください。")) }
    finally { setFolderBusy(null) }
  }
  const removeWatchedFolder = async (folder: WatchedFolderDto) => {
    ...
    catch { setNotice(localize("監視フォルダーを解除できませんでした。")) }
    finally { setFolderBusy(null) }
  }
  ...
  const stageImport = async (item: ImportPreview) => {
    ...
    try {
      ...
      if (!reviewOptionsLoaded || !isStagedReviewBindingValid({
        binding,
      ...
      onChanged()
      setNotice(summary.reusedExisting ? localize("同じファイルの既存インポートを開きました。") : localize("原本を暗号化し、取引候補をステージングしました。"))
    } catch {
      setNotice(localize("インポートを開始できませんでした。データベースの状態を確認してください。"))
    } finally {
      setActiveRun(null)
    }
  }
  ...
  const extractDocument = async (item: ImportPreview, password?: string, requestedOperation?: 'EXTRACT' | 'OCR') => {
    ...
    try {
      ...
      if (isImage) extracted = await paddleOcrDocument(item.fileBytes, item.mediaType)
        ...
        if (attempt.status !== 'SUCCESS') {
          if (attempt.status === 'PASSWORD_REQUIRED' || attempt.status === 'PASSWORD_INVALID' || attempt.status === 'PASSWORD_UNSUPPORTED') {
            ...
            setNotice(attempt.status === 'PASSWORD_UNSUPPORTED' ? localize("このPDFの暗号方式には対応していません。保護を解除したコピーを取り込んでください。") : localize("スキャンPDFをOCRするためのパスワードを入力してください。"))
          } else {
            const messages = {
              LIMIT_EXCEEDED: localize("このPDFはOCRのページ数または処理上限を超えています。32ページ以下のファイルに分割してください。"),
              FAILED: localize("スキャンPDFを画像へ変換できませんでした。原本は台帳へ反映されていません。"),
            } as const
        ...
        if (attempt.status !== 'SUCCESS') {
          ...
          setNotice(attempt.status === 'PASSWORD_UNSUPPORTED' ? localize("このPDFの暗号方式には対応していません。保護を解除したコピーを取り込んでください。") : localize("PDFを開くためのパスワードを入力してください。"))
          return
        }
        ...
        if (extracted.issues.includes('OCR_REQUIRED')) {
          ...
          setNotice(extracted.issues.includes('EMBEDDED_TEXT_UNSUPPORTED')
            ? localize(`このPDFの埋め込み文字コードには直接対応できません。${extracted.pageCount ?? localize("全")}ページを端末内OCRで読み取ってください。`)
            : localize(`画像として保存されたPDFです。${extracted.pageCount ?? localize("全")}ページを端末内OCRで読み取る操作が必要です。`))
          return
        }
      }
      if (!isImage) {
        ...
        if (rakutenStatement) {
          ...
          const needsManualReview = rakutenStatement.parsed.issues.some((issue) => issue.code === 'RAKUTEN_PDF_TOTAL_MISMATCH')
          setPreviews((current) => current.map((preview) => preview.id === item.id ? {
            ...preview,
          ...
          if (hasErrors) {
            setNotice(localize("楽天カードPDFは読み取れましたが、請求額と明細合計を照合できません。原本内容を確認してください。"))
          } else if (needsManualReview) {
            setNotice(localize("楽天カードPDFを読み取りました。請求額と明細合計に差がありますが、取込先カード口座を選択してレビューへ進み、返金・調整行を手動で修正できます。原本PDFは変更しません。"))
          } else {
            const statement = rakutenStatement.parsed.records[0]
            setNotice(localize(`楽天カードPDF ${extracted.pageCount ?? 1}ページから${rakutenStatement.detailCount}件を抽出し、請求額${yen(statement.statementTotal ?? 0)}と照合しました。取込先カード口座を選択してからレビューへ進んでください。`))
          }
          return
        }
      }
      ...
      if (!normalized.request) {
        const missingDate = normalized.fields.issues.includes('DATE_MISSING')
        const missingTotal = normalized.fields.issues.includes('TOTAL_MISSING')
        setNotice(normalized.fields.issues.includes('STATEMENT_LIKELY')
          ? localize("明細書形式のPDFは、1件の支出として取り込みません。原本内容を確認してください。")
          : missingDate && !missingTotal
            ? localize("OCRで合計金額は読み取れましたが、日付が画像にありません。日付が写った原本を選択してください。")
            : missingTotal && !missingDate
              ? localize("OCRで日付は読み取れましたが、合計金額を確定できません。合計欄が鮮明な画像を選択してください。")
              : localize("日付または合計金額を読み取れませんでした。内容を確認してください。"))
        return
      }
      ...
      if ((extracted.pageCount ?? 1) > 1) {
        setNotice(candidatePages.length > 0
          ? localize(`スキャンPDF ${extracted.pageCount}ページの原本を保存し、独立したレシートとして読めた${candidatePages.length}ページだけを支出候補にしました（最低信頼度 ${Math.round(confidence / 100)}%）。明細書ページは集約しません。`)
          : localize(`スキャンPDF ${extracted.pageCount}ページの原本とOCR結果を保存しました。独立したレシートとして確定できるページがないため、支出候補は作成していません。`))
      } else {
        setNotice(localize(`${isImage ? localize("PP-OCRv5によるレシート画像OCR") : extracted.method === 'OCR' ? localize("スキャンPDF 1ページのOCR") : localize("PDFの埋め込みテキスト")}から支出候補を抽出しました（信頼度 ${Math.round(confidence / 100)}%）。台帳への反映にはレビューと承認が必要です。`))
      }
    } catch {
      setNotice(item.mediaType.startsWith('image/') ? localize("PP-OCRv5で画像を読み取れませんでした。モデル資源、対応形式、画質を確認してください。") : localize("PDFを解析できませんでした。原本、パスワード、端末内OCRの状態を確認してください。"))
  ...
  const importPortfolioSnapshot = async (item: ImportPreview) => {
    ...
    if (!securitiesAccount) { setNotice(localize("先に設定で証券口座を追加してください。")); return }
    ...
    if (!snapshot) { setNotice(localize("資産スナップショットを正規化できませんでした。")); return }
    setActiveRun(item.id); setNotice('')
    try {
  ...
  const importAggregateAssetHistory = async (item: ImportPreview) => {
    ...
    if (snapshots.length === 0) { setNotice(localize("Money Forwardの総資産履歴を正規化できませんでした。")); return }
    setActiveRun(item.id); setNotice('')
    ...
    try {
      ...
      setNotice(localize("Money Forward総資産履歴を保存できませんでした。同じ日付の値と原本行を確認してください。"))
    }
    finally { setActiveRun(null) }
  }
  ...
  const rollbackRun = async (previewId: string, runId: string) => {
    ...
    try {
      ...
      setNotice(gmailInboxItemId ? localize("未確定のインポートを取り消し、Gmail Inbox に戻しました。") : driveInboxItemId ? localize("未確定のインポートを取り消し、Google Drive Inbox に戻しました。") : localize("未確定のインポートを取り消しました。"))
    } catch {
      setNotice(localize("インポートを取り消せませんでした。"))
    } finally {
  ...
  const refreshAfterReceiptLink = async (previewId: string, runId: string) => {
    ...
    onChanged(); setRecoveryRevision((value) => value + 1); setNotice(localize("既存取引にレシート証憑を紐付けました。新しい支出は作成していません。"))
  }
  ...
  const setDuplicateResolution = async (previewId: string, candidateId: string, resolution: 'LINK' | 'KEEP_BOTH' | 'EXCLUDE') => {
    ...
    try {
      ...
      setNotice(localize("重複判定を保存できませんでした。既存取引と候補の状態を再確認してください。"))
    }
  }
  ...
  return <>
    <PageHeader eyebrow={localize("データ取り込み")} title={localize("インポート Inbox")} description={localize("ファイルから読み取った候補を確認して台帳へ反映します。")} headingId={importView === 'CONNECTORS' ? 'connector-settings-watched-folder' : 'connector-import-inbox'}>
      ...
      <input ref={inputRef} aria-label={localize("CSV、TSV、Excel、PDF、レシート画像、ゆうちょ公式ZIP、EMLを選択")} className="visually-hidden" type="file" accept=".csv,.tsv,.xlsx,.pdf,.png,.jpg,.jpeg,.zip,.eml,text/csv,text/tab-separated-values,application/zip,application/x-zip-compressed,application/pdf,image/png,image/jpeg,message/rfc822,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" multiple onChange={(event) => { const files = takeFileInputSelection(event.currentTarget); void processFiles(files) }} />
    </PageHeader>
    <div className="workspace-tabs import-source-tabs" role="tablist" aria-label={localize("インポート元")}><button role="tab" aria-selected={importView === 'LOCAL'} className={importView === 'LOCAL' ? 'active' : ''} onClick={() => setImportView('LOCAL')}>{localize("ローカル")}</button><button role="tab" aria-selected={importView === 'CONNECTORS'} className={importView === 'CONNECTORS' ? 'active' : ''} onClick={() => setImportView('CONNECTORS')}>{localize("コネクタ")}</button><span>{localize("どの経路もレビュー確定まで台帳へ反映しません")}</span></div>
    <div className="import-stage-strip" aria-label={localize("インポート処理段階")} hidden={importView !== 'LOCAL'}>
      <strong>{localize("処理段階:")}</strong>
      {[localize("検出"), localize("抽出中"), localize("プレビュー"), localize("マッピング必要"), localize("レビュー必要"), localize("転記可能"), localize("転記済み")].map((label, index) => <span className={index === 6 ? 'complete' : ''} key={label}>{label}{index < 6 && <ArrowRight size={11} aria-hidden="true" />}</span>)}
    </div>
    ...
    <section className="status-grid import-status-summary" hidden={importView !== 'LOCAL'}>
      {[
        [localize("取込済み"), String(summary?.posted ?? (platformClient.runtime === 'web' ? 79 : 0)), localize(`${summary?.sourceDocuments ?? 0}原本`)],
        [localize("確認待ち"), String(summary?.reviewRequired ?? (platformClient.runtime === 'web' ? 6 : 0)), localize(`${summary?.readyCandidates ?? 0}候補`)],
        [localize("処理失敗"), String(summary?.failed ?? (platformClient.runtime === 'web' ? 2 : 0)), localize("再実行可能")],
        [localize("ソース行"), String(summary?.sourceRecords ?? (platformClient.runtime === 'web' ? 4 : 0)), localize("監査証跡")],
    ...
    {platformClient.runtime === 'tauri' && gmailInboxItems.length > 0 && <section className="panel watched-folders" aria-labelledby="gmail-inbox-title"><div className="panel-head"><div><h2 id="gmail-inbox-title">Gmail Inbox</h2><p>{localize("ラベル同期したEML原本と添付をプレビューします。レビューで承認するまで台帳へ反映しません。")}</p></div><button className="secondary-btn" disabled={gmailInboxBusy} onClick={() => void refreshGmailInbox(true)}>{gmailInboxBusy ? localize("更新中…") : localize("Inbox を更新")}</button></div><div className="watched-folder"><div><strong>{localize("添付メールの原本")}</strong><span>{gmailInboxItems.length}{localize("件 ・ Gmail 読み取り専用")}</span></div>{gmailInboxItems.filter((item) => item.state !== 'REMOVED').map((item) => { const emailPreview = previews.find((preview) => preview.gmailInboxItemId === item.id); const previewFailed = emailPreview?.status === 'error' || emailPreview?.status === 'unsupported'; return <div className="watched-file" key={item.id}><FileCheck2 size={15} /><span><strong>{item.fileName}</strong><small>Gmail ・ {item.estimatedByteSize === null ? localize("サイズ未取得") : `${(item.estimatedByteSize / 1024).toFixed(1)} KB`} ・ {new Date(item.internalDateMs).toLocaleDateString('ja-JP')}</small></span><b className={!previewFailed && ['READY', 'STAGED'].includes(item.state) ? 'ready' : 'review'}>{previewFailed ? localize("プレビューで確認が必要") : gmailInboxStateLabel(item.state)}</b><span className="folder-inbox-actions">{previewFailed && <button className="mini-btn" disabled={gmailInboxBusy} onClick={() => void repreviewGmailInboxItem(item.id)}>{localize("再プレビュー")}</button>}{item.state === 'FAILED' && <button className="mini-btn" disabled={gmailInboxBusy} onClick={() => void retryGmailInboxItem(item.id)}>{localize("再試行")}</button>}{['DISCOVERED', 'READY', 'NEEDS_MAPPING', 'FAILED'].includes(item.state) && <button className="text-btn" disabled={gmailInboxBusy} onClick={() => void ignoreGmailInboxItem(item.id)}>{localize("無視")}</button>}</span>{item.lastErrorCode && <small className="folder-inbox-error">{item.lastErrorCode}</small>}</div> })}</div></section>}
    {platformClient.runtime === 'tauri' && driveInboxItems.length > 0 && <section className="panel watched-folders" aria-labelledby="google-drive-inbox-title"><div className="panel-head"><div><h2 id="google-drive-inbox-title">Google Drive Inbox</h2><p>{localize("同期済みの原本世代を端末内でプレビューします。取引はレビューで承認するまで台帳へ反映しません。")}</p></div><button className="secondary-btn" disabled={driveInboxBusy} onClick={() => void refreshGoogleDriveInbox(true)}>{driveInboxBusy ? localize("更新中…") : localize("Inbox を更新")}</button></div><div className="watched-folder"><div><strong>{localize("接続フォルダーの原本")}</strong><span>{driveInboxItems.length}{localize("件 ・ Google Drive 読み取り専用")}</span></div>{driveInboxItems.filter((item) => item.state !== 'REMOVED').map((item) => {
      ...
      const displayState = previewFailed ? localize("プレビューで確認が必要") : googleDriveInboxStateLabel(item.state)
      return <div className="watched-file" key={item.id}><FileCheck2 size={15} /><span><strong>{item.fileName}</strong><small>Google Drive ・ {item.remoteByteSize === null ? localize("サイズ未取得") : `${(item.remoteByteSize / 1024).toFixed(1)} KB`} {localize("・ 世代")} {item.driveVersion ?? '—'}</small></span><b className={!previewFailed && ['READY', 'STAGED'].includes(item.state) ? 'ready' : 'review'}>{displayState}</b><span className="folder-inbox-actions">{previewFailed && <button className="mini-btn" disabled={driveInboxBusy} onClick={() => void repreviewGoogleDriveInboxItem(item.id)}>{localize("再プレビュー")}</button>}{item.state === 'FAILED' && <button className="mini-btn" disabled={driveInboxBusy} onClick={() => void retryGoogleDriveInboxItem(item.id)}>{localize("再試行")}</button>}{['DISCOVERED', 'READY', 'NEEDS_MAPPING', 'FAILED'].includes(item.state) && <button className="text-btn" disabled={driveInboxBusy} onClick={() => void ignoreGoogleDriveInboxItem(item.id)}>{localize("無視")}</button>}</span>{item.lastErrorCode && <small className="folder-inbox-error">{item.lastErrorCode}</small>}</div>
    })}</div></section>}
    {platformClient.runtime === 'tauri' && watchedFolders.length > 0 && <section className="panel watched-folders"><div className="panel-head"><div><h2>{localize("同期フォルダー")}</h2><p>{localize("変更履歴と処理状態は端末内データベースに保持され、再起動後も復元されます。")}</p></div><label className="auto-scan-toggle"><input type="checkbox" checked={folderInbox.autoScan} onChange={(event) => folderInbox.setAutoScan(event.target.checked)} /><span>{localize("自動プレビュー")}</span></label></div>{watchedFolders.map((folder) => <div className="watched-folder" key={folder.id}><div><strong>{folder.label}</strong><span>{folder.provider === 'ICLOUD' ? 'iCloud Drive' : localize("ローカル同期")} ・ {folder.displayName}</span></div><button className="secondary-btn" disabled={folderBusy === folder.id} onClick={() => void scanWatchedFolder(folder)}>{folderBusy === folder.id ? localize("スキャン中…") : localize("新しいファイルを確認")}</button><button className="text-btn" disabled={folderBusy === folder.id} onClick={() => void removeWatchedFolder(folder)}>{localize("解除")}</button>{folderInbox.items.filter((item) => item.watchedFolderId === folder.id && item.state !== 'REMOVED').map((item) => { const stateLabel = { DISCOVERED: localize("検出済み"), PROCESSING: localize("解析中"), READY: localize("プレビュー完了"), NEEDS_MAPPING: localize("形式の対応付けが必要"), STAGED: localize("取込処理に接続済み"), FAILED: localize("失敗"), IGNORED: localize("無視"), REMOVED: localize("削除済み") }[item.state]; return <div className="watched-file" key={item.id}><FileCheck2 size={15} /><span><strong>{item.fileName}</strong><small>{item.provider === 'ICLOUD' ? 'iCloud Drive' : localize("ローカル同期")} ・ {(item.byteSize / 1024).toFixed(1)} {localize("KB ・ 試行")} {item.attemptCount}</small></span><b className={item.state === 'READY' || item.state === 'STAGED' ? 'ready' : 'review'}>{stateLabel}</b><span className="folder-inbox-actions">{(item.state === 'FAILED' || item.state === 'IGNORED') && <button className="mini-btn" onClick={() => void folderInbox.retry(item.id)}>{localize("再試行")}</button>}{['DISCOVERED', 'READY', 'NEEDS_MAPPING', 'FAILED'].includes(item.state) && <button className="text-btn" onClick={() => void folderInbox.ignore(item.id)}>{localize("無視")}</button>}</span>{item.lastErrorCode && <small className="folder-inbox-error">{item.lastErrorCode}</small>}</div> })}</div>)}</section>}
    </div>
    ...
    <section className="panel import-panel">
      ...
      {parserProfiles.length > 0 && previews.some((item) => /\.(?:csv|tsv)$/i.test(item.filename) && item.fileBytes) && <div className="custom-parser-files">
        <div><strong>{localize("保存済みプロファイルを明示的に適用")}</strong><small>{localize("組み込み判定を上書きする場合、ファイルとプロファイルを選んで実データをプレビューします。")}</small></div>
      ...
      {previews.some((item) => dedicatedBrokerageImport(item.detectedAdapterId) != null && item.status === 'ready') && <div className="custom-parser-files">{previews.filter((item) => dedicatedBrokerageImport(item.detectedAdapterId) != null && item.status === 'ready').map((item) => {
        ...
        const eligibleAccounts = reviewAccountOptionsFor(item).filter((account) => account.accountKind === 'ASSET' && account.accountSubtype === 'SECURITIES')
        const descriptionId = eligibleAccounts.length === 0 ? `investment-account-empty-${item.id}` : undefined
        return <div className="custom-parser-file" key={item.id}><span><strong>{config.title}</strong><small>{item.filename} ・ {config.description}</small><small>{config.accountHint}</small>{eligibleAccounts.length === 0 && <small id={descriptionId} role="status">{localize("設定ページで先に証券口座を追加してください。追加するまで保存できません。")}</small>}</span><select aria-label={localize(`${item.filename}の取込先証券口座`)} aria-describedby={descriptionId} disabled={eligibleAccounts.length === 0} value={effectiveReviewAccountId(item, investmentImportAccounts[item.id])} onChange={(event) => setInvestmentImportAccounts((current) => ({ ...current, [item.id]: event.target.value }))}><option value="">{localize("証券口座を選択")}</option>{eligibleAccounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></div>
      })}</div>}
...
function CardsPage({ cards, householdId, accounts, revision, onChanged, month }: { cards: readonly CardSettlementDto[]; householdId: string | null; accounts: readonly AccountDto[]; revision: number; onChanged: () => void; month: string }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setMappingDrafts(Object.fromEntries(nextMappings.map((mapping) => [mapping.cardAccountId, mapping.bankAccountId])))
    }).catch(() => { if (active) setNotice(localize("引落口座と支払余力を読み込めませんでした。")) })
    return () => { active = false }
  ...
  const confirm = async (card: CardSettlementDto, paymentId: string) => {
    ...
    try { await platformClient.confirmCardPaymentLink(householdId, card.id, paymentId); await reloadSettlementPlan(); onChanged(); setNotice(localize("選択した口座引落を請求に紐付けました。仕訳や支払いは変更していません。")) }
    catch { setNotice(localize("照合を確定できませんでした。金額とカード口座を確認してください。")) }
    finally { setBusyId(null) }
  }
  const unlink = async (card: CardSettlementDto, paymentId: string) => {
    if (!householdId) return
    if (unlinkConfirmId !== paymentId) { setUnlinkConfirmId(paymentId); setNotice(localize("解除すると請求の照合合計を再計算します。もう一度押して確定してください。")); return }
    setBusyId(paymentId); setNotice('')
    try {
      ...
      setNotice(localize("誤って紐付けた口座引落を解除しました。銀行取引と仕訳は変更していません。"))
    } catch { setNotice(localize("紐付けを解除できませんでした。最新の照合状態を確認してください。")) }
    finally { setBusyId(null) }
  }
  const saveMapping = async (cardAccountId: string) => {
    ...
    try {
      ...
      await reloadSettlementPlan(); onChanged(); setNotice(localize("明示したカード引落口座を保存しました。"))
    } catch { setNotice(localize("カード引落口座を保存できませんでした。")) }
    finally { setBusyId(null) }
  }
  const removeMapping = async (cardAccountId: string) => {
    ...
    try {
      ...
      await reloadSettlementPlan(); onChanged(); setNotice(localize("カード引落口座の設定を解除しました。"))
    } catch { setNotice(localize("カード引落口座の設定を解除できませんでした。")) }
    finally { setBusyId(null) }
  }
  const saveDueDate = async (statementId: string, paymentDueOn: string | null) => {
    ...
    try {
      ...
      setNotice(paymentDueOn == null ? localize("ユーザー登録の支払期日を解除しました。予測から除外されます。") : localize("ユーザー確認済みの支払期日を保存し、支払余力と予測を更新しました。"))
    } catch { setNotice(localize("支払期日を保存できませんでした。明細期間以降の正しい日付を入力してください。")) }
    finally { setBusyId(null) }
  }
  const coverageLabel = (status: 'COVERED' | 'SHORTFALL' | 'OVERDUE') => status === 'COVERED' ? localize("支払可能") : status === 'SHORTFALL' ? localize("残高不足") : localize("期限超過")
  ...
  const reconciliationPresentation: Record<CardPresentationStatus, { readonly label: string; readonly tone: string }> = {
    UNMATCHED: { label: localize("⚠ 未照合"), tone: 'pending' }, PAYMENT_PENDING: { label: localize("◐ 支払待ち"), tone: 'possible' }, POSSIBLE_MATCH: { label: localize("◐ 照合候補あり"), tone: 'candidate' },
    FULLY_RECONCILED: { label: localize("✓ 全額照合"), tone: 'reconciled' }, PARTIALLY_RECONCILED: { label: localize("◐ 一部支払済み"), tone: 'possible' },
    OVERPAID: { label: localize("⚠ 過払い"), tone: 'overpaid' }, UNDERPAID: { label: localize("⚠ 支払不足"), tone: 'underpaid' },
    MANUAL_OVERRIDE: { label: localize("✓ 手動確認済み"), tone: 'manual' },
  }
  ...
  return <>
    <PageHeader eyebrow={localize("カード管理")} title={localize("カード引落・支払余力")} description={localize("請求照合に加え、明示した銀行口座で今後のカード引落を支払えるか確認します。")}>
    </PageHeader>
    {desktop && <aside className="card-coverage-disclosure"><strong>{localize("引落口座はユーザーが明示した設定だけを使用し、取引名から推測しません。")}</strong><span>{localize("残高と予測は「集計対象外」を含むすべての確定済み仕訳を反映します。この画面は確認専用で、振込・カード支払いは実行しません。")}</span></aside>}
    {notice && <div className="import-notice" role="status">{notice}</div>}
    {desktop && <section className="panel card-bank-mappings" aria-label={localize("カード引落口座設定")}><div className="panel-head"><div><h2>{localize("カードごとの引落銀行口座")}</h2><p>{mappings.length}/{cardAccounts.length}{localize("枚を明示設定済み")}</p></div><CreditCard size={19} /></div><div className="card-mapping-list">{cardAccounts.map((card) => { const mapped = mappings.some((mapping) => mapping.cardAccountId === card.id); return <div key={card.id}><span><strong>{card.name}</strong><small>{mapped ? localize("明示設定済み") : localize("未設定・銀行口座を推測しません")}</small></span><select id={`card-bank-${card.id}`} aria-label={localize(`${card.name}の引落銀行口座`)} value={mappingDrafts[card.id] ?? ''} onChange={(event) => setMappingDrafts((current) => ({ ...current, [card.id]: event.target.value }))}><option value="">{localize("銀行口座を選択")}</option>{bankAccounts.map((bank) => <option key={bank.id} value={bank.id}>{bank.name}</option>)}</select><button className="secondary-btn" disabled={busyId === card.id || !mappingDrafts[card.id]} onClick={() => void saveMapping(card.id)}>{busyId === card.id ? localize("保存中…") : localize("保存")}</button>{mapped && <button className="text-btn" disabled={busyId === card.id} onClick={() => void removeMapping(card.id)}>{localize("解除")}</button>}</div> })}{cardAccounts.length === 0 && <p className="empty-state">{localize("クレジットカード口座を登録すると引落銀行口座を設定できます。")}</p>}</div></section>}
    {desktop && coverage && <section className="card-coverage-section" aria-label={localize("カード支払余力")}><div className="card-coverage-heading"><div><h2>{localize("支払余力")}</h2><p>{coverage.asOf} {localize("現在 →")} {coverage.horizonThrough}（{coverage.horizonDays}{localize("日）")}</p></div><small>{localize("残高基準: 全確定仕訳")}</small></div>{coverage.banks.map((bank) => <article className={`panel bank-coverage-card${bank.maxShortfallJpy > 0 ? ' has-shortfall' : ''}`} key={bank.bankAccountId}><header><div><strong>{bank.bankAccountName}</strong><span>{localize("現在残高")} {yen(bank.balanceAsOfJpy)}</span></div><div><small>{localize("引落後の見込み")}</small><b>{yen(bank.projectedEndingBalanceJpy)}</b></div></header><div className="coverage-statement-list">{bank.statements.map((statement) => <div key={statement.statementId}><span><strong>{statement.cardAccountName}</strong><small>{localize("支払期日")} {statement.paymentDueOn} {localize("・ 未払")} {yen(statement.outstandingAmountJpy)}</small></span><span><small>{localize("累積見込残高")}</small><b>{yen(statement.projectedBankBalanceJpy)}</b></span><em className={`coverage-status ${statement.status.toLowerCase()}`}>{coverageLabel(statement.status)}</em></div>)}{bank.statements.length === 0 && <p className="empty-state">{localize("期間内の未払請求はありません。")}</p>}</div>{bank.maxShortfallJpy > 0 && <footer>{localize("最大不足額")} <strong>{yen(bank.maxShortfallJpy)}</strong></footer>}</article>)}{coverage.banks.length === 0 && <p className="empty-state">{localize("明示された引落口座に、期日付きの未払請求はありません。")}</p>}{coverage.unmappedStatements.length > 0 && <aside className="unmapped-statements" role="status"><div><strong>{localize("引落口座が未設定の請求")}</strong><span>{localize("銀行口座は推測せず、支払余力の計算にも含めません。")}</span></div>{coverage.unmappedStatements.map((statement) => <div key={statement.statementId}><span><strong>{statement.cardAccountName}</strong><small>{localize("支払期日")} {statement.paymentDueOn}</small></span><b>{yen(statement.outstandingAmountJpy)}</b><em className={statement.status === 'OVERDUE' ? 'overdue' : ''}>{statement.status === 'OVERDUE' ? localize("期限超過") : localize("未設定")}</em></div>)}</aside>}{coverage.missingDueStatements.length > 0 && <aside className="unmapped-statements missing-due-statements" role="status"><div><strong>{localize("支払期日が未登録の請求")}</strong><span>{localize("支払期日がないため予測から除外しています。明細で確認した日付だけを登録してください。")}</span></div>{coverage.missingDueStatements.map((statement) => <div key={statement.statementId}><span><strong>{statement.cardAccountName}</strong><small>{statement.mappingConfigured ? localize("引落口座は設定済み") : localize("引落口座も未設定")}</small></span><b>{yen(statement.outstandingAmountJpy)}</b><label className="due-date-editor compact"><span>{localize("ユーザー確認日")}</span><input aria-label={localize(`${statement.cardAccountName}の未登録支払期日`)} type="date" value={dueDateDrafts[statement.statementId] ?? ''} onChange={(event) => setDueDateDrafts((current) => ({ ...current, [statement.statementId]: event.target.value }))} /><button className="secondary-btn" disabled={!dueDateDrafts[statement.statementId] || busyId === `due:${statement.statementId}`} onClick={() => void saveDueDate(statement.statementId, dueDateDrafts[statement.statementId])}>{busyId === `due:${statement.statementId}` ? localize("保存中…") : localize("保存")}</button></label></div>)}</aside>}</section>}
    <div className="section-divider"><span>{localize("請求・口座引落の照合")}</span></div>
    <section className="cards-page-grid">{displayCards.map((card) => <article className="panel card-detail" aria-labelledby={`card-statement-${card.id}`} key={card.id}>
      <header className="card-statement-identity"><div><h2 id={`card-statement-${card.id}`}>{card.cardName}</h2><code>{card.maskedIdentifier ?? localize("番号未設定")}</code></div><p>{card.periodStart} – {card.periodEnd} ・ {card.lineCount}{localize("件")}{card.paymentDueOn ? localize(` ・ 支払期日 ${card.paymentDueOn}`) : ''}</p></header>
      <div className="card-detail-head"><div><span>{localize("請求額")}</span><strong>{yen(card.statementAmountJpy)}</strong></div><b className={reconciliationPresentation[presentationStatus(card)].tone}>{reconciliationPresentation[presentationStatus(card)].label}</b></div>
      <dl className="card-settlement-totals"><div><dt>{localize("支払済み")}</dt><dd>{yen(card.paidAmountJpy)}</dd></div><div><dt>{localize("未払い")}</dt><dd>{yen(card.outstandingAmountJpy)}</dd></div><div><dt>{localize("過払い")}</dt><dd>{yen(card.overpaidAmountJpy)}</dd></div></dl>
      <div className="card-payment-history"><h3>{localize("紐付け済みの口座引落")}</h3>{card.payments.map((payment) => <div className="card-payment-row confirmed" key={payment.paymentId}><span><strong>{payment.paymentOn}</strong><small>{localize("銀行取引")} {payment.bankTransactionId}</small></span><b>{yen(payment.paymentAmountJpy)}</b><em>{localize("確認済み")}</em><button className={unlinkConfirmId === payment.paymentId ? 'secondary-btn card-unlink-confirm' : 'text-btn'} disabled={busyId === payment.paymentId} onClick={() => void unlink(card, payment.paymentId)}>{busyId === payment.paymentId ? localize("解除中…") : unlinkConfirmId === payment.paymentId ? localize("解除を確定") : localize("紐付けを解除")}</button></div>)}{card.payments.length === 0 && <p className="empty-state">{localize("紐付け済みの口座引落はありません。")}</p>}</div>
      {card.eligiblePayments.length > 0 && <div className="card-payment-candidates"><h3>{localize("照合候補")}</h3><p>{localize("候補ごとに金額と日付を確認してください。自動確定や支払い処理は行いません。")}</p>{card.eligiblePayments.map((payment) => <div className="card-payment-row" key={payment.paymentId}><span><strong>{payment.paymentOn}</strong><small>{payment.matchScoreBps == null ? localize("一致度未算出") : localize(`一致度 ${Math.round(payment.matchScoreBps / 100)}%`)} {localize("・ 銀行取引")} {payment.bankTransactionId}</small></span><b>{yen(payment.paymentAmountJpy)}</b><button className="secondary-btn" disabled={busyId === payment.paymentId} onClick={() => void confirm(card, payment.paymentId)}>{busyId === payment.paymentId ? localize("確定中…") : localize("この引落を確認して紐付け")}</button></div>)}</div>}
      <dl className="card-statement-meta"><div><dt>{localize("期間")}</dt><dd>{card.periodStart} – {card.periodEnd}</dd></div><div><dt>{localize("利用明細")}</dt><dd>{card.lineCount}{localize("件")}</dd></div><div><dt>{localize("支払期日")}</dt><dd>{card.paymentDueOn ?? localize("未登録")} <small>{localize("ユーザー確認値")}</small></dd></div></dl>
      {desktop && <><div className="due-date-editor"><label><span>{localize("支払期日を登録・訂正")}</span><input id={`card-due-${card.id}`} aria-label={localize(`${card.cardName}の支払期日`)} type="date" min={card.periodEnd} value={dueDateDrafts[card.id] ?? ''} onChange={(event) => setDueDateDrafts((current) => ({ ...current, [card.id]: event.target.value }))} /></label><button className="secondary-btn" disabled={!dueDateDrafts[card.id] || busyId === `due:${card.id}`} onClick={() => void saveDueDate(card.id, dueDateDrafts[card.id])}>{busyId === `due:${card.id}` ? localize("保存中…") : localize("保存")}</button>{card.paymentDueOn && <button className="text-btn" disabled={busyId === `due:${card.id}`} onClick={() => void saveDueDate(card.id, null)}>{localize("解除")}</button>}<small>{localize("発行会社から自動推測せず、確認した日付だけを使用します。")}</small></div><footer className="card-detail-actions" aria-label={localize(`${card.cardName}のカード操作`)}><button type="button" onClick={() => document.getElementById(`card-bank-${card.cardAccountId}`)?.focus()}>{localize("引落口座を変更…")}</button><button type="button" onClick={() => document.getElementById(`card-due-${card.id}`)?.focus()}>{localize("支払日を上書き…")}</button><button type="button" disabled={card.payments.length === 0 || busyId === card.payments[0]?.paymentId} onClick={() => { const payment = card.payments[0]; if (payment) void unlink(card, payment.paymentId) }}>{unlinkConfirmId === card.payments[0]?.paymentId ? localize("照合解除を確定") : localize("照合を解除…")}</button></footer></>}
    </article>)}{desktop && displayCards.length === 0 && <p className="empty-state">{localize("カードCSVを取り込むと、ここに請求と照合状況が表示されます。")}</p>}</section>
  </>
}
...
function InvestmentsPage({ householdId, revision, openImport }: { householdId: string | null; revision: number; openImport: () => void }) {
  ...
  useEffect(() => {
    ...
    void portfolioPlatform.listSnapshots(householdId).then(async (items) => {
      ...
      setDetail(null)
    }).catch(() => { if (active) { setSnapshots([]); setDetail(null); setNotice(localize("投資データを読み込めませんでした。")) } })
    ...
    void aggregateAssetHistoryPlatform.listHistory({ householdId, limit: 240 }).then((items) => { if (active) setAggregateAssets(items) }).catch(() => { if (active) { setAggregateAssets([]); setNotice(localize("Money Forward総資産履歴を読み込めませんでした。")) } })
    return () => { active = false }
  ...
  const selectSnapshot = async (snapshotId: string) => {
    ...
    catch { setNotice(localize("選択した資産スナップショットを読み込めませんでした。")) }
  }
  const saveSelectedSnapshotXlsx = async () => {
    ...
    try {
      ...
      setSnapshotExportNotice(saved === null ? localize("資産スナップショットExcelエクスポートをキャンセルしました。") : localize(`${saved.fileName}（${saved.rowCount.toLocaleString('ja-JP')}行）を保存しました。`))
      if (saved) showToast(localize("資産スナップショットExcelを保存しました。"))
    } catch { setSnapshotExportNotice(localize("資産スナップショットExcelを書き出せませんでした。選択中のスナップショットを確認してください。")) }
  ...
  const saveSelectedSnapshotCsv = async () => {
    ...
    try {
      ...
      setSnapshotExportNotice(saved === null ? localize("資産スナップショットCSVエクスポートをキャンセルしました。") : localize(`${saved.fileName}（${saved.rowCount.toLocaleString('ja-JP')}行）を保存しました。`))
      if (saved) showToast(localize("資産スナップショットCSVを保存しました。"))
    } catch { setSnapshotExportNotice(localize("資産スナップショットCSVを書き出せませんでした。選択中のスナップショットを確認してください。")) }
  ...
  const saveSelectedSnapshotPdf = async () => {
    ...
    try {
      ...
      setSnapshotExportNotice(saved === null ? localize("資産スナップショットPDFエクスポートをキャンセルしました。") : localize(`${saved.fileName}（${saved.pageCount.toLocaleString('ja-JP')}ページ）を保存しました。`))
      if (saved) showToast(localize("資産スナップショットPDFを保存しました。"))
    } catch { setSnapshotExportNotice(localize("資産スナップショットPDFを書き出せませんでした。選択中のスナップショットを確認してください。")) }
  ...
  const applyAggregateRange = async (from: string | null, to: string | null) => {
    ...
    catch { setNotice(localize("総資産履歴の期間を読み込めませんでした。開始日と終了日を確認してください。")) }
  }
  return <><PageHeader eyebrow={localize("資産形成")} title={localize("資産・投資")} description={localize("証券会社の資産残高ファイルを、家計取引とは分離した時点スナップショットとして管理します。")}><button className="primary-btn" onClick={openImport}><Import size={17} /> {localize("残高ファイルを取り込む")}</button></PageHeader>
    <div className="workspace-tabs investment-tabs" role="tablist" aria-label={localize("投資表示")}><button role="tab" aria-selected={investmentView === 'SNAPSHOT'} className={investmentView === 'SNAPSHOT' ? 'active' : ''} onClick={() => setInvestmentView('SNAPSHOT')}>{localize("スナップショット")}</button><button role="tab" aria-selected={investmentView === 'PERFORMANCE'} className={investmentView === 'PERFORMANCE' ? 'active' : ''} onClick={() => setInvestmentView('PERFORMANCE')}>{localize("実現損益（FIFO）")}</button><button role="tab" aria-selected={investmentView === 'VALUATION'} className={investmentView === 'VALUATION' ? 'active' : ''} onClick={() => setInvestmentView('VALUATION')}>{localize("推移・評価")}</button><span>{localize("家計の収支とは分離されたワークスペースです")}</span></div>
    {notice && <div className="import-notice" role="status">{notice}</div>}
    {snapshotExportNotice && <div className="import-notice" role="status">{snapshotExportNotice}</div>}
    <div hidden={investmentView !== 'SNAPSHOT'}>
    {snapshots.length > 0 && <section className="panel snapshot-selector"><div><strong>{localize("表示するスナップショット")}</strong><small>{localize("最新への自動切替は行いません。評価日を明示して表示します。")}</small></div><select aria-label={localize("資産スナップショット")} value={detail?.id ?? ''} onChange={(event) => { if (event.target.value) void selectSnapshot(event.target.value) }}><option value="">{localize("スナップショットを選択")}</option>{snapshots.map((snapshot) => <option key={snapshot.id} value={snapshot.id}>{snapshot.asOf.slice(0, 10)} ・ {snapshot.accountName}</option>)}</select></section>}
    {detail ? <><section className="kpi-grid investment-kpis"><KpiCard label={localize("時価総額")} value={yen(detail.marketValueJpy)} meta={localize(`${detail.asOf.slice(0, 10)} 残高`)} icon={TrendingUp} accent="#e4edda" /><KpiCard label={localize("現金")} value={yen(detail.cashValueJpy)} meta={localize(`${detail.accountName}・残高`)} icon={CircleDollarSign} accent="#dce9e6" /><KpiCard label={localize("評価損益")} value={yen(detail.unrealizedPnlJpy ?? 0)} meta={localize("未実現・残高")} icon={ArrowUpRight} accent="#eee5cf" /><KpiCard label={localize("実現損益 2026")} value={yen(performance?.totalsByCurrency.find((total) => total.currency === 'JPY')?.realizedPnl ?? 0)} meta="FIFO・JPY" icon={WalletCards} accent="#f7e3d9" /></section>
      <section className="investment-grid"><article className="panel"><div className="panel-head"><div><h2>{localize("資産配分")}</h2><p>{localize("評価額ベース")}</p></div></div><div className="asset-allocation">{detail.assetClasses.map((item) => <div key={item.id}><span><strong>{item.name}</strong><em>{yen(item.marketValueJpy)}</em></span><div className="progress"><span style={{ width: `${item.marketValueJpy / maxAssetClass * 100}%` }} /></div></div>)}</div></article><article className="panel snapshot-history"><div className="panel-head"><div><h2>{localize("スナップショット履歴")}</h2><p>{snapshots.length}{localize("件")}</p></div></div>{snapshots.map((snapshot) => <button key={snapshot.id} className={snapshot.id === detail.id ? 'active' : ''} onClick={() => void selectSnapshot(snapshot.id)}><span>{snapshot.asOf.slice(0, 10)} ・ {snapshot.accountName}</span><strong>{yen(snapshot.marketValueJpy)}</strong></button>)}</article></section>
      <section className="panel positions-table"><div className="panel-head"><div><h2>{localize("保有商品")}</h2><p>{localize("原本の行番号まで追跡可能・銘柄を選ぶと取引履歴を表示")}</p></div><div className="snapshot-export-actions"><button className="secondary-btn" disabled={savingSnapshotCsv || savingSnapshotXlsx || savingSnapshotPdf} onClick={() => void saveSelectedSnapshotCsv()}>{savingSnapshotCsv ? localize("CSVを作成中…") : localize("選択中の残高CSVを保存")}</button><button className="secondary-btn" disabled={savingSnapshotCsv || savingSnapshotXlsx || savingSnapshotPdf} onClick={() => void saveSelectedSnapshotXlsx()}>{savingSnapshotXlsx ? localize("Excelを作成中…") : localize("選択中の残高Excelを保存")}</button><button className="secondary-btn" disabled={savingSnapshotCsv || savingSnapshotXlsx || savingSnapshotPdf} onClick={() => void saveSelectedSnapshotPdf()}>{savingSnapshotPdf ? localize("PDFを作成中…") : localize("選択中の残高PDFを保存")}</button></div></div><div className="position-row position-head"><span>{localize("銘柄")}</span><span>{localize("口座")}</span><span>{localize("数量")}</span><span>{localize("現在値")}</span><span>{localize("評価額")}</span><span>{localize("評価損益")}</span></div>{detail.positions.map((position) => <button type="button" className={`position-row position-selectable${selectedInstrumentCode === position.instrumentCode ? ' active' : ''}`} key={position.id} onClick={() => setSelectedInstrumentCode(position.instrumentCode || position.instrumentName)}><span><strong>{position.instrumentName}</strong><small>{position.instrumentCode || position.productType} {localize("・ 行")} {position.sourceRow}</small></span><span>{position.accountType}</span><span>{position.quantity?.toLocaleString('ja-JP') ?? '—'}</span><span>{position.marketPrice == null ? '—' : `${position.currency} ${position.marketPrice.toLocaleString('ja-JP')}`}</span><strong>{position.marketValueJpy == null ? '—' : yen(position.marketValueJpy)}</strong><em className={(position.unrealizedPnlJpy ?? 0) >= 0 ? 'amount-positive' : ''}>{position.unrealizedPnlJpy == null ? '—' : yen(position.unrealizedPnlJpy)}</em></button>)}</section>{selectedInstrumentCode && <section className="panel brokerage-instrument-history"><div className="panel-head"><div><h2>{selectedInstrumentCode} {localize("の取引履歴")}</h2><p>{localize("証券会社原本の約定日・受渡日を保持")}</p></div><button className="text-btn" onClick={() => setSelectedInstrumentCode(null)}>{localize("閉じる")}</button></div>{brokerage?.events.filter((event) => event.instrumentCode === selectedInstrumentCode || event.instrumentName === selectedInstrumentCode).slice(0, 20).map((event) => <div key={event.id}><span><strong>{event.rawTransactionType}</strong><small>{localize("約定")} {event.tradeDate ?? '—'} {localize("・ 受渡")} {event.settlementDate ?? '—'} {localize("・ 行")} {event.sourceRow}</small></span><b>{brokerageEventTypeLabel(event.eventType, text)}</b><em>{event.currency} {event.settlementAmount.toLocaleString('ja-JP')}</em></div>)}{!brokerage?.events.some((event) => event.instrumentCode === selectedInstrumentCode || event.instrumentName === selectedInstrumentCode) && <p className="empty-state">{localize("この銘柄に対応する取引履歴はありません。")}</p>}</section>}</> : <section className="panel investment-empty"><TrendingUp size={32} /><h2>{localize("資産スナップショットはまだありません")}</h2><p>{localize("設定で証券口座を追加し、`assetbalance(all)_*.csv` をインポートしてください。")}</p><button className="primary-btn" onClick={openImport}>{localize("インポート Inboxを開く")}</button></section>}
    ...
    {brokerage && brokerage.events.length > 0 && <section className="panel brokerage-history"><div className="panel-head"><div><h2>{localize("証券取引履歴")}</h2><p>{localize("売買・配当・手数料・税金・入出金（家計支出には含めません）")}</p></div><strong>{brokerage.events.length}{localize("件")}</strong></div><div className="brokerage-totals">{brokerage.totalsByCurrency.map((total) => <article key={total.currency}><span>{total.currency} {localize("純資金移動")}</span><strong>{total.netCashMovement.toLocaleString('ja-JP')}</strong><small>{localize("配当")} {total.dividendGross.toLocaleString('ja-JP')} {localize("・ 手数料")} {total.fees.toLocaleString('ja-JP')} {localize("・ 税")} {total.taxes.toLocaleString('ja-JP')}</small></article>)}</div><div className="brokerage-event-list">{brokerage.events.slice(0, 20).map((event) => <div key={event.id}><span><strong>{event.instrumentName || event.rawTransactionType}</strong><small>{event.tradeDate ?? event.settlementDate} ・ {event.accountName} {localize("・ 行")} {event.sourceRow}</small></span><b>{brokerageEventTypeLabel(event.eventType, text)}</b><em>{event.currency} {event.settlementAmount.toLocaleString('ja-JP')}</em></div>)}</div></section>}
    {holdings && (holdings.positions.length > 0 || (performance?.totalsByCurrency.length ?? 0) > 0) && <section className="panel investment-performance"><div className="panel-head"><div><h2>{localize("投資パフォーマンス")}</h2><p>{holdings.costBasisMethod} {localize("原価法・通貨ごとに集計（自動換算なし）")}</p></div><span>{holdings.asOf} {localize("現在")}</span></div>{performance && <div className="performance-currency-grid">{performance.totalsByCurrency.map((total) => <article key={total.currency}><span>{total.currency}</span><strong className={total.realizedPnl >= 0 ? 'amount-positive' : ''}>{total.realizedPnl.toLocaleString('ja-JP')} {localize("実現損益")}</strong><small>{localize("配当")} {total.dividendGross.toLocaleString('ja-JP')} {localize("・ 手数料")} {total.fees.toLocaleString('ja-JP')} {localize("・ 税")} {total.taxes.toLocaleString('ja-JP')}</small></article>)}</div>}<div className="performance-position-list">{holdings.positions.map((position) => <div key={`${position.accountId}-${position.instrumentCode}-${position.currency}`}><span><strong>{position.instrumentName}</strong><small>{position.instrumentCode} ・ {position.accountName} ・ {position.openLotCount}{localize("ロット")}</small></span><em>{position.quantity.toLocaleString('ja-JP')} {localize("株")}</em><b>{position.currency} {position.costBasis.toLocaleString('ja-JP')} {localize("原価")}</b></div>)}</div>{(holdings.uncoveredSales.length > 0 || holdings.skippedEventIds.length > 0) && <p className="performance-warning">{localize("原価未確認の売却")} {holdings.uncoveredSales.length}{localize("件・計算対象外")} {holdings.skippedEventIds.length}{localize("件。原本取引を確認してください。")}</p>}</section>}
    ...
    <div hidden={investmentView !== 'VALUATION'}>
      <section className="panel investment-history-disclosure"><strong>{localize("スナップショット時点だけを表示")}</strong><span>{localize("点と点の間は補間しません。各値の基準日とソースを分けて確認できます。")}</span></section>
...
function FinancialIntelligencePanel({ householdId, accountGroupId, attributionScope, month, revision, preferenceRevision, openTransactions, onDecisionChanged }: { householdId: string | null; accountGroupId: string | null; attributionScope: AttributionScopeDto; month: string; revision: number; preferenceRevision: number; openTransactions: () => void; onDecisionChanged: () => void }) {
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([
      ...
      if (intelligenceResult.status === 'rejected') messages.push(localize("定期支出と異常支出を分析できませんでした。"))
      if (preferenceResult.status === 'rejected') messages.push(localize("確認状態を読み込めないため、変更操作は一時停止しています。"))
  ...
  const changeDecision = async (normalizedPayee: string, decision: RecurringDecision | 'RESTORE') => {
    ...
    try {
      ...
      setDecisionNotice(decision === 'CONFIRMED' ? localize("定期支出として確認しました。関連する分析を更新しています。") : decision === 'IGNORED' ? localize("対象外にしました。関連する分析を更新しています。") : localize("自動検出へ戻しました。関連する分析を更新しています。"))
      onDecisionChanged()
    } catch {
      setDecisionNotice(localize("確認状態を更新できませんでした。表示中の状態は変更していません。再読み込み後にもう一度お試しください。"))
    } finally { setBusyPayee(null) }
  }
  if (notice && !intelligence) return <section className="panel"><p className="empty-state">{notice}</p></section>
  if (!intelligence) return <section className="panel"><p className="empty-state">{localize("家計履歴を分析しています…")}</p></section>
  const cadenceLabel = { WEEKLY: localize("毎週"), BIWEEKLY: localize("隔週"), MONTHLY: localize("毎月"), QUARTERLY: localize("四半期"), ANNUAL: localize("毎年") } as const
  const allRecurringItems = [...intelligence.recurringItems, ...intelligence.ignoredRecurringItems]
  const statusLabel = { AUTO_DETECTED: localize("自動検出"), CONFIRMED: localize("確認済み"), IGNORED: localize("対象外") } as const
  return <><section className="intelligence-grid"><article className="panel recurring-panel"><div className="panel-head"><div><h2>{localize("定期支出・サブスクリプション")}</h2><p>{intelligence.historyFrom} {localize("以降の計算対象の確定取引から推定（集計対象外を除く）")}</p></div><Repeat2 size={19} /></div>{notice && <p className="recurring-decision-notice" role="status">{notice}</p>}{decisionNotice && <p className="recurring-decision-notice" role="status">{decisionNotice}</p>}{allRecurringItems.length === 0 ? <p className="empty-state">{localize("十分な反復履歴はまだありません。")}</p> : allRecurringItems.map((item) => { const preference = preferences?.find((candidate) => candidate.normalizedPayee === item.normalizedPayee); const requiresPreference = item.decisionStatus !== 'AUTO_DETECTED'; const disabled = busyPayee !== null || preferences === null || (requiresPreference && !preference); return <div className={`recurring-row recurring-${item.decisionStatus.toLowerCase()}`} key={item.normalizedPayee}><div><span className={`recurring-status recurring-status-${item.decisionStatus.toLowerCase()}`}>{statusLabel[item.decisionStatus]}</span><strong>{item.displayPayee}</strong><span>{cadenceLabel[item.cadence]} ・ {item.occurrenceCount}{localize("回 ・ 信頼度")} {Math.round(item.confidenceBps / 100)}%</span></div><div><small>{localize("次回見込み")}</small><strong>{item.nextExpectedOn}</strong></div><div><small>{localize("標準金額")}</small><strong>{yen(item.typicalAmountJpy)}</strong>{item.priceChangeBps != null && item.priceChangeBps !== 0 && <em>{item.priceChangeBps > 0 ? '+' : ''}{(item.priceChangeBps / 100).toFixed(1)}%</em>}</div><div className="recurring-decision-actions">{item.decisionStatus === 'AUTO_DETECTED' && <button className="mini-btn" disabled={disabled} onClick={() => void changeDecision(item.normalizedPayee, 'CONFIRMED')}>{localize("定期支出として確認")}</button>}{item.decisionStatus !== 'IGNORED' && <button className="text-btn" disabled={disabled} onClick={() => void changeDecision(item.normalizedPayee, 'IGNORED')}>{localize("対象外にする")}</button>}{item.decisionStatus === 'IGNORED' && <button className="secondary-btn" disabled={disabled} onClick={() => void changeDecision(item.normalizedPayee, 'RESTORE')}>{localize("自動検出へ戻す")}</button>}{busyPayee === item.normalizedPayee && <small>{localize("更新中…")}</small>}</div></div> })}</article><article className="panel anomaly-panel"><div className="panel-head"><div><h2>{localize("異常支出")}</h2><p>{localize("同じ支払先の計算対象の過去実績と比較")}</p></div><Bell size={19} /></div>{intelligence.anomalies.length === 0 ? <p className="empty-state">{localize("確認が必要な異常支出はありません。")}</p> : intelligence.anomalies.map((item) => <button key={item.transactionId} onClick={openTransactions}><span><strong>{item.displayPayee}</strong><small>{item.occurredOn} {localize("・ 基準")} {yen(item.baselineAmountJpy)} ({item.baselineSampleCount}{localize("件)")}</small></span><strong>{yen(item.amountJpy)}</strong><em>{localize("スコア")} {Math.round(item.scoreBps / 100)}</em></button>)}</article></section></>
}
...
function FixedCostReviewPanel({ householdId, accountGroupId, attributionScope, month, revision, preferenceRevision, openTransactions }: { householdId: string | null; accountGroupId: string | null; attributionScope: AttributionScopeDto; month: string; revision: number; preferenceRevision: number; openTransactions: () => void }) {
  ...
  useEffect(() => {
    ...
    void queryFixedCostReview(tauriInvoke, { householdId, accountGroupId, attributionScope, asOf: periodFromMonth(month).toDate })
      ...
      .catch(() => { if (active) setNotice(localize("固定費レビューを読み込めませんでした。6か月分の確定取引を確認してください。")) })
    return () => { active = false }
  }, [accountGroupId, attributionScope, householdId, month, preferenceRevision, revision])
  if (notice) return <section className="panel"><p className="empty-state">{notice}</p></section>
  if (!review) return <section className="panel report-loading"><Repeat2 size={28} /><p>{localize("完了済み6か月の固定費を比較しています…")}</p></section>
  return <FixedCostReviewView data={review} onOpenTransactions={openTransactions} />
}

function RecurringPage({ householdId, accountGroupId, attributionScope, month, revision, openTransactions }: { householdId: string | null; accountGroupId: string | null; attributionScope: AttributionScopeDto; month: string; revision: number; openTransactions: () => void }) {
  const [preferenceRevision, setPreferenceRevision] = useState(0)
  if (platformClient.runtime === 'web') {
    const previewItems = [
      { payee: 'Netflix', cadence: localize('毎月'), amount: 1_490, next: '2026-08-12', status: localize('確認済み') },
      { payee: 'Amazon Prime', cadence: localize('毎年'), amount: 5_900, next: '2026-09-03', status: localize('自動検出') },
      { payee: '東京電力', cadence: localize('毎月'), amount: 12_840, next: '2026-08-18', status: localize('確認済み') },
    ]
    return <><PageHeader eyebrow={localize('Planning & Intelligence')} title={localize('定期取引・固定費')} description={localize('サブスクリプション、固定費、価格変化、異常支出をまとめて確認します。')}><button className="secondary-btn" onClick={openTransactions}><WalletCards size={16} /> {localize('関連取引を開く')}</button></PageHeader><section className="intelligence-grid"><article className="panel recurring-panel"><div className="panel-head"><div><h2>{localize('定期支出・サブスクリプション')}</h2><p>{localize('確定取引の反復履歴から検出')}</p></div><Repeat2 size={19} /></div>{previewItems.map((item) => <div className="recurring-row" key={item.payee}><div><span className="recurring-status recurring-status-confirmed">{item.status}</span><strong>{item.payee}</strong><span>{item.cadence} · {localize('信頼度')} 94%</span></div><div><small>{localize('次回見込み')}</small><strong>{item.next}</strong></div><div><small>{localize('標準金額')}</small><strong>{yen(item.amount)}</strong></div></div>)}</article><article className="panel anomaly-panel"><div className="panel-head"><div><h2>{localize('固定費サマリー')}</h2><p>{localize('直近3か月平均')}</p></div><Clock3 size={19} /></div><div className="gemini-result"><strong>{yen(68_430)} / {localize('月')}</strong><span>{localize('前3か月比')} +2.1% · 8{localize('件')}</span></div></article></section></>
  }
  return <>
    <PageHeader eyebrow={localize('Planning & Intelligence')} title={localize('定期取引・固定費')} description={localize('サブスクリプション、固定費、価格変化、異常支出をまとめて確認します。')}><button className="secondary-btn" onClick={openTransactions}><WalletCards size={16} /> {localize('関連取引を開く')}</button></PageHeader>
    <FinancialIntelligencePanel householdId={householdId} accountGroupId={accountGroupId} attributionScope={attributionScope} month={month} revision={revision} preferenceRevision={preferenceRevision} openTransactions={openTransactions} onDecisionChanged={() => setPreferenceRevision((value) => value + 1)} />
    <FixedCostReviewPanel householdId={householdId} accountGroupId={accountGroupId} attributionScope={attributionScope} month={month} revision={revision} preferenceRevision={preferenceRevision} openTransactions={openTransactions} />
  </>
}
...
function AccountGroupsExportPanel({ householdId, accounts, month, groups, selectedAccountGroupId, attributionScope, onGroupsChanged }: { householdId: string | null; accounts: readonly AccountDto[]; month: string; groups: readonly AccountGroupDto[]; selectedAccountGroupId: string | null; attributionScope: AttributionScopeDto; onGroupsChanged: (groups: readonly AccountGroupDto[]) => void }) {
  ...
  const createGroup = async () => {
    if (!householdId || !name.trim() || selectedAccounts.size === 0) { setNotice(localize("グループ名と1つ以上の口座を選択してください。")); return }
    setBusy(true); setNotice('')
    try { await accountGroupExportPlatform.createGroup({ id: crypto.randomUUID(), householdId, name: name.trim(), groupKind: kind, accountIds: [...selectedAccounts] }); setName(''); setSelectedAccounts(new Set()); await reload(); setNotice(localize("口座グループを保存しました。")) }
    catch { setNotice(localize("口座グループを保存できませんでした。")) }
    finally { setBusy(false) }
  }
  const deleteGroup = async (group: AccountGroupDto) => {
    if (!householdId) return
    setBusy(true)
    try { await accountGroupExportPlatform.deleteGroup(householdId, group.id); if (groupId === group.id) setGroupId(''); await reload(); setNotice(localize("口座グループを削除しました。")) }
    catch { setNotice(localize("口座グループを削除できませんでした。")) }
    finally { setBusy(false) }
  }
  const exportCsv = async () => {
    ...
    try {
      ...
      setNotice(saved ? localize(`${saved.fileName}（${saved.rowCount}行）を保存しました。`) : localize("エクスポートをキャンセルしました。"))
      if (saved) showToast(localize("CSVを保存しました。"))
    } catch { setNotice(localize("CSVを書き出せませんでした。対象期間とグループを確認してください。")) }
    finally { setBusy(false) }
  }
  const exportXlsx = async () => {
    ...
    try {
      ...
      setNotice(saved ? localize(`${saved.fileName}（${saved.rowCount}行）を保存しました。`) : localize("Excelエクスポートをキャンセルしました。"))
      if (saved) showToast(localize("取引台帳Excelを保存しました。"))
    } catch { setNotice(localize("取引台帳Excelを書き出せませんでした。対象期間とグループを確認してください。")) }
    finally { setBusy(false) }
  }
  const exportPdf = async () => {
    ...
    try {
      ...
      setNotice(saved ? localize(`${saved.fileName}（${saved.rowCount}行・${saved.pageCount}ページ）を保存しました。`) : localize("PDFエクスポートをキャンセルしました。"))
      if (saved) showToast(localize("取引台帳PDFを保存しました。"))
    } catch { setNotice(localize("取引台帳PDFを書き出せませんでした。対象期間とグループを確認してください。")) }
    finally { setBusy(false) }
  }
  const kindLabels: Record<AccountGroupKindDto, string> = { FAMILY: localize("家族"), PERSONAL: localize("個人"), DAILY_SPENDING: localize("日常支出"), INVESTMENT: localize("投資"), BUSINESS: localize("事業"), TAX: localize("税務"), EDUCATION: localize("教育"), CUSTOM: localize("カスタム") }
  return <section className="groups-export-grid"><article className="panel account-group-panel"><div className="panel-head"><div><h2>{localize("口座グループ")}</h2><p>{localize("ダッシュボードと出力で再利用する保存済みスコープ")}</p></div><Layers size={19} /></div><div className="group-form"><input aria-label={localize("グループ名")} value={name} onChange={(event) => setName(event.target.value)} placeholder={localize("家族の生活費")} /><select aria-label={localize("グループ種別")} value={kind} onChange={(event) => setKind(event.target.value as AccountGroupKindDto)}>{Object.entries(kindLabels).map(([value, label]) => <option key={value} value={value}>{localize(label)}</option>)}</select><div className="group-account-choices">{accounts.map((account) => <label key={account.id}><input type="checkbox" checked={selectedAccounts.has(account.id)} onChange={(event) => setSelectedAccounts((current) => { const next = new Set(current); if (event.target.checked) next.add(account.id); else next.delete(account.id); return next })} /><span>{account.name}</span></label>)}</div><button className="primary-btn" disabled={busy} onClick={() => void createGroup()}>{localize("グループを保存")}</button></div><div className="saved-groups">{groups.map((group) => <div key={group.id}><span><strong>{group.name}</strong><small>{kindLabels[group.groupKind]} ・ {group.accountIds.length}{localize("口座")}</small></span><button className="text-btn" disabled={busy} onClick={() => void deleteGroup(group)}>{localize("削除")}</button></div>)}{groups.length === 0 && <p className="empty-state">{localize("保存済みグループはありません。")}</p>}</div></article><article className="panel export-panel"><div className="panel-head"><div><h2>{localize("台帳エクスポート")}</h2><p>{localize("同じ確定データをCSV・Excel・PDFで保存")}</p></div><Download size={19} /></div><label>{localize("データ")}<select aria-label={localize("エクスポートデータ")} value={exportKind} onChange={(event) => setExportKind(event.target.value as ExportKindDto)}><option value="TRANSACTIONS">{localize("取引台帳")}</option><option value="PORTFOLIO_SNAPSHOTS">{localize("資産スナップショット")}</option></select></label><label>{localize("計上基準")}<select aria-label={localize("エクスポート計上基準")} value={basis} onChange={(event) => setBasis(event.target.value as ExportAccountingBasisDto)}><option value="ACCRUAL">{localize("発生ベース")}</option><option value="CASH">{localize("資金移動")}</option></select></label><label>{localize("口座スコープ")}<select aria-label={localize("エクスポートグループ")} value={groupId} onChange={(event) => setGroupId(event.target.value)}><option value="">{localize("すべての口座")}</option>{groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label><div className="export-period"><span>{localize("対象期間")}</span><strong>{period.fromDate} → {period.toDate}</strong></div><div className="monthly-review-export-actions"><button className="primary-btn" disabled={busy} onClick={() => void exportCsv()}>{busy ? localize("処理中…") : localize("CSVを保存")}</button>{exportKind === 'TRANSACTIONS' && <><button className="secondary-btn" disabled={busy} onClick={() => void exportXlsx()}>{busy ? localize("処理中…") : localize("取引台帳Excelを保存")}</button><button className="secondary-btn" disabled={busy} onClick={() => void exportPdf()}>{busy ? localize("処理中…") : localize("取引台帳PDFを保存")}</button></>}</div>{exportKind === 'PORTFOLIO_SNAPSHOTS' && <small>{localize("資産スナップショットのExcel/PDFは投資画面から保存できます。")}</small>}{notice && <p role="status">{notice}</p>}</article></section>
}
...
function ReportTabs({ view, onChange }: { view: ReportView; onChange: (view: ReportView) => void }) {
  ...
  const items = [
    { id: 'report-tab-calendar', label: localize("カレンダー"), target: 'CALENDAR' as const, active: view === 'CALENDAR', icon: CalendarDays },
    { id: 'report-tab-review', label: localize("月次・年次レビュー"), target: 'MONTHLY' as const, active: reviewActive, icon: FileText },
    { id: 'report-tab-analysis', label: localize("分析・予測"), target: 'FORECAST' as const, active: analysisActive, icon: TrendingUp },
  ]
  ...
  return <div className="report-tabs report-tabs-primary" role="tablist" aria-label={localize("レポート表示")}>{items.map((item, index) => <button key={item.id} id={item.id} role="tab" aria-controls="report-tabpanel" aria-selected={item.active} tabIndex={index === activeIndex ? 0 : -1} className={item.active ? 'active' : ''} onClick={() => onChange(item.target)} onKeyDown={(event) => handleKeyDown(event, index)}><item.icon size={15} /> {item.label}</button>)}</div>
}

function ReportSubTabs({ view, onChange }: { view: ReportView; onChange: (view: ReportView) => void }) {
  const items = view === 'MONTHLY' || view === 'ANNUAL'
    ? [['MONTHLY', localize("月次レポート")], ['ANNUAL', localize("年次レビュー")]] as const
    : view === 'FORECAST' || view === 'INTELLIGENCE' || view === 'FIXED_COST'
      ? [['FORECAST', localize("予測とアクション")], ['INTELLIGENCE', localize("定期・異常レビュー")], ['FIXED_COST', localize("固定費レビュー")]] as const
      : []
  if (items.length === 0) return null
  return <div className="report-subtabs" role="tablist" aria-label={localize("レポート詳細")}>{items.map(([itemView, label]) => <button key={itemView} id={`report-subtab-${itemView.toLowerCase()}`} role="tab" aria-selected={view === itemView} className={view === itemView ? 'active' : ''} onClick={() => onChange(itemView)}>{localize(label)}</button>)}</div>
}
...
function MonthlyReviewMemo({ householdId, month }: { householdId: string | null; month: string }) {
  ...
  useEffect(() => {
    ...
    if (platformClient.runtime === 'tauri' && householdId) {
      void platformClient.getMonthlyReviewMemo(householdId, month)
        ...
        .catch(() => { if (active) { setMemo(''); setSavedMemo(''); setNotice(localize("締めメモを読み込めませんでした。")) } })
    } else {
  ...
  const save = async () => {
    ...
    try {
      ...
      setMemo(normalized); setSavedMemo(normalized); setNotice(normalized ? localize("この月の締めメモを保存しました。") : localize("締めメモを削除しました。"))
      showToast(normalized ? localize("月次メモを保存しました。") : localize("月次メモを削除しました。"))
    } catch {
      setNotice(localize("締めメモを保存できませんでした。もう一度お試しください。"))
    } finally { setSaving(false) }
  }
  return <section className="panel monthly-review-memo"><div className="panel-head"><div><h2>{localize("締めの要約")}</h2><p>{month}{localize("・暗号化台帳に保存")}</p></div><FileCheck2 size={18} /></div><textarea aria-label={localize("月次レビューの締めメモ")} rows={3} maxLength={1200} value={memo} onChange={(event) => setMemo(event.target.value)} placeholder={localize("例: 貯蓄率は目標内。交際費が予算を超えたため、来月は週単位で確認する。")} /><footer><small>{memo.length} / 1200</small>{notice && <span role="status">{notice}</span>}<button className="primary-btn" disabled={saving || memo.trim() === savedMemo} onClick={() => void save()}>{saving ? localize("保存中…") : localize("メモを保存")}</button></footer></section>
}
...
function ReportsPage({ householdId, accountGroupId, attributionScope, accountGroups, onGroupsChanged, accounts, month, revision, initialView, openPage }: { householdId: string | null; accountGroupId: string | null; attributionScope: AttributionScopeDto; accountGroups: readonly AccountGroupDto[]; onGroupsChanged: (groups: readonly AccountGroupDto[]) => void; accounts: readonly AccountDto[]; month: string; revision: number; initialView: ReportView; openPage: (page: PageId) => void }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([financialCalendarPlatform.getCalendar(request), financialCalendarPlatform.getMonthlyReport(request), forecastActionPlatform.query({ householdId, accountGroupId, attributionScope, asOf: request.asOf })])
      ...
      .catch(() => { if (active) { setCalendar(null); setMonthlyReport(null); setForecast(null); setNotice(localize("家計レビューを読み込めませんでした。")) } })
    return () => { active = false }
  ...
  const saveMonthlyCsv = async () => {
    ...
    try {
      ...
      setMonthlyExportNotice(saved ? localize(`${saved.fileName}（${saved.rowCount}行）を保存しました。`) : localize("月次CSVエクスポートをキャンセルしました。"))
      if (saved) showToast(localize("月次CSVを保存しました。"))
    } catch { setMonthlyExportNotice(localize("月次CSVを書き出せませんでした。対象月とスコープを確認してください。")) }
  ...
  const saveMonthlyXlsx = async () => {
    ...
    try {
      ...
      setMonthlyExportNotice(saved ? localize(`${saved.fileName}（${saved.rowCount}行）を保存しました。`) : localize("月次Excelエクスポートをキャンセルしました。"))
      if (saved) showToast(localize("月次Excelを保存しました。"))
    } catch { setMonthlyExportNotice(localize("月次Excelを書き出せませんでした。対象月とスコープを確認してください。")) }
  ...
  const saveMonthlyPdf = async () => {
    ...
    try {
      ...
      setMonthlyExportNotice(saved ? localize(`${saved.fileName}（${saved.pageCount}ページ）を保存しました。`) : localize("月次PDFエクスポートをキャンセルしました。"))
      if (saved) showToast(localize("月次PDFを保存しました。"))
    } catch { setMonthlyExportNotice(localize("月次PDFを書き出せませんでした。対象月とスコープを確認してください。")) }
  ...
  useEffect(() => {
    ...
    void financialCalendarPlatform.getYearlyReport(request)
      ...
      .catch(() => { if (active) setAnnualNotice(localize("年次レビューを読み込めませんでした。対象年と確定取引を確認してください。")) })
    return () => { active = false }
  ...
  const saveAnnualCsv = async () => {
    ...
    try {
      ...
      setAnnualNotice(saved ? localize(`${saved.fileName}（${saved.rowCount}行）を保存しました。`) : localize("エクスポートをキャンセルしました。"))
      if (saved) showToast(localize("年次CSVを保存しました。"))
    } catch { setAnnualNotice(localize("年次CSVを書き出せませんでした。対象年とスコープを確認してください。")) }
  ...
  const saveAnnualXlsx = async () => {
    ...
    try {
      ...
      setAnnualNotice(saved ? localize(`${saved.fileName}（${saved.rowCount}行）を保存しました。`) : localize("Excelエクスポートをキャンセルしました。"))
      if (saved) showToast(localize("年次Excelを保存しました。"))
    } catch { setAnnualNotice(localize("年次Excelを書き出せませんでした。対象年とスコープを確認してください。")) }
  ...
  const saveAnnualPdf = async () => {
    ...
    try {
      ...
      setAnnualNotice(saved ? localize(`${saved.fileName}（${saved.pageCount}ページ）を保存しました。`) : localize("PDFエクスポートをキャンセルしました。"))
      if (saved) showToast(localize("年次PDFを保存しました。"))
    } catch { setAnnualNotice(localize("年次PDFを書き出せませんでした。対象年とスコープを確認してください。")) }
    finally { setAnnualPdfSaving(false) }
  }
  const reportBodyContent = view === 'CALENDAR'
    ? calendar ? <FinancialCalendarView data={calendar} basis={basis} onBasisChange={setBasis} onSelectDate={() => openPage('transactions')} onSelectEvent={() => openPage('transactions')} onOpenImports={() => openPage('import')} /> : <section className="panel report-loading"><CalendarDays size={28} /><p>{notice || localize("日次カレンダーを読み込んでいます…")}</p></section>
    : view === 'MONTHLY'
      ? monthlyReport ? <><MonthlyReportView data={monthlyReport} comparison={comparison} savingCsv={monthlyCsvSaving} savingXlsx={monthlyXlsxSaving} savingPdf={monthlyPdfSaving} onComparisonChange={setComparison} onSelectDriver={() => openPage('transactions')} onOpenBudget={() => openPage('budgets')} onOpenGoals={() => openPage('budgets')} onOpenImports={() => openPage('import')} onOpenReconciliation={() => openPage('cards')} onSaveCsv={() => void saveMonthlyCsv()} onSaveXlsx={() => void saveMonthlyXlsx()} onSavePdf={() => void saveMonthlyPdf()} /><MonthlyReviewMemo householdId={householdId} month={month} />{monthlyExportNotice && <p role="status">{monthlyExportNotice}</p>}</> : <section className="panel report-loading"><FileText size={28} /><p>{notice || localize("月次比較レポートを読み込んでいます…")}</p></section>
      : view === 'ANNUAL' ? annualReport ? <><AnnualReviewView data={annualReport} savingCsv={annualCsvSaving} savingXlsx={annualXlsxSaving} savingPdf={annualPdfSaving} onSelectDriver={() => openPage('transactions')} onOpenBudget={() => openPage('budgets')} onOpenImports={() => openPage('import')} onOpenReconciliation={() => openPage('cards')} onSaveCsv={() => void saveAnnualCsv()} onSaveXlsx={() => void saveAnnualXlsx()} onSavePdf={() => void saveAnnualPdf()} />{annualNotice && <p role="status">{annualNotice}</p>}</> : <section className="panel report-loading"><FileText size={28} /><p>{annualNotice || localize("前年同期間と年次推移を比較しています…")}</p></section>
      : view === 'FORECAST' ? forecast ? <ForecastActionViews data={forecast} onAction={(action: ActionItemDto) => openPage(pageForAction(action))} /> : <section className="panel report-loading"><TrendingUp size={28} /><p>{notice || localize("予測とアクションを読み込んでいます…")}</p></section>
  ...
  return <><PageHeader eyebrow={localize("家計レビュー")} title={localize("カレンダー・レポート")} description={localize("計算対象の確定台帳（集計対象外を除く）を日次、月次、年次、予測、定期支出・異常支出の視点で確認します。")}><ReportTabs view={view} onChange={setView} /></PageHeader><ReportSubTabs view={view} onChange={setView} />{reportBody}<MonthlyContextNotes householdId={householdId} month={month} /></>
}
...
function BudgetsPage({ householdId, accounts, month, revision, onChanged }: { householdId: string | null; accounts: readonly AccountDto[]; month: string; revision: number; onChanged: () => void }) {
  ...
  useEffect(() => { void reload().catch(() => { setBudgets([]); setGoals([]); setNotice(localize("予算と目標を読み込めませんでした。")) }) }, [desktop, householdId, month, revision]) // eslint-disable-line react-hooks/exhaustive-deps

  const saveBudget = async () => {
    if (!householdId || !budgetAccountId || !/^\d+$/.test(budgetAmount)) { setNotice(localize("カテゴリーと0円以上の予算を入力してください。")); return }
    setBusy(true); setNotice('')
    try { await platformClient.upsertBudget({ householdId, month, categoryAccountId: budgetAccountId, budgetJpy: Number(budgetAmount) }); await reload(); onChanged(); setBudgetAmount(''); setNotice(localize("月間予算を保存しました。")) }
    catch { setNotice(localize("月間予算を保存できませんでした。")) }
    finally { setBusy(false) }
  }

  const createGoal = async () => {
    if (!householdId || !goalName.trim() || !/^\d+$/.test(goalTarget) || Number(goalTarget) <= 0) { setNotice(localize("目標名と1円以上の目標額を入力してください。")); return }
    setBusy(true); setNotice('')
    try { await platformClient.createSavingsGoal({ id: crypto.randomUUID(), householdId, name: goalName.trim(), targetJpy: Number(goalTarget), savedJpy: 0, targetDate: goalDate, status: 'ACTIVE' }); await reload(); onChanged(); setGoalName(''); setGoalTarget(''); setShowGoalForm(false); setNotice(localize("貯蓄目標を追加しました。")) }
    catch { setNotice(localize("貯蓄目標を追加できませんでした。")) }
    finally { setBusy(false) }
  }

  const updateGoal = async (goal: SavingsGoalDto) => {
    const saved = goalDrafts[goal.id]
    if (!/^\d+$/.test(saved ?? '')) { setNotice(localize("貯蓄済み金額を0円以上で入力してください。")); return }
    setBusy(true)
    try { await platformClient.updateSavingsGoal({ id: goal.id, householdId: goal.householdId, name: goal.name, targetJpy: goal.targetJpy, savedJpy: Number(saved), targetDate: goal.targetDate, status: Number(saved) >= goal.targetJpy ? 'COMPLETED' : goal.status === 'COMPLETED' ? 'ACTIVE' : goal.status }); await reload(); onChanged(); setNotice(localize("貯蓄額を更新しました。")) }
    catch { setNotice(localize("貯蓄額を更新できませんでした。")) }
    finally { setBusy(false) }
  }

  const deleteGoal = async (goal: SavingsGoalDto) => {
    setBusy(true)
    try { await platformClient.deleteSavingsGoal(goal.householdId, goal.id); await reload(); onChanged(); setNotice(localize("貯蓄目標を削除しました。")) }
    catch { setNotice(localize("貯蓄目標を削除できませんでした。")) }
    finally { setBusy(false) }
  }

  if (!desktop) {
    const demoGoals = [{ name: localize("家族旅行"), saved: 362_000, target: 600_000, date: '2027-03-31', pace: 22_800 }, { name: localize("教育資金"), saved: 1_480_000, target: 3_000_000, date: '2030-04-01', pace: 33_100 }]
    return <><PageHeader eyebrow={localize("プランニング")} title={localize("予算・貯蓄目標")} description={localize("計画値と確定済み元帳を比較し、家族の目標を追跡します。")} /><section className="budget-layout"><article className="panel budget-panel"><div className="panel-head"><div><h2>{localize("カテゴリー予算")}</h2><p>{localize("2026年7月")}</p></div><strong>{yen(budgetByCategory.reduce((sum, item) => sum + item.amount, 0))}</strong></div>{budgetByCategory.map((item) => { const rate = Math.round(item.amount / item.budget * 100); return <div className={`budget-row budget-row-${rate > 100 ? 'over' : rate > 85 ? 'review' : 'ok'}`} key={item.name}><div><strong>{item.name}</strong><small>{rate > 100 ? localize(`⚠ 超過 ${rate - 100}%`) : `${rate}%`}</small></div><span>{yen(item.amount)} <small>/ {yen(item.budget)}</small></span><div className="progress"><span style={{ width: `${Math.min(100, rate)}%` }} /></div></div>})}<p className="budget-caption">{localize("予算=計画値 / 実績=確定済み元帳値（発生ベース）")}</p></article><article className="panel goal-panel"><div className="panel-head"><div><h2>{localize("貯蓄目標")}</h2><p>{demoGoals.length}{localize("件進行中")}</p></div><Goal size={19} /></div>{demoGoals.map((goal) => <div className="goal-editor demo-goal" key={goal.name}><div><strong>{goal.name}</strong><small>{localize("目標日")} {goal.date}</small></div><span>{yen(goal.saved)} / {yen(goal.target)}</span><div className="progress"><span style={{ width: `${goal.saved / goal.target * 100}%` }} /></div><p>{localize("必要ペース:")} {yen(goal.pace)} {localize("/ 月 —")} <b>{localize("順調")}</b></p></div>)}</article></section><PlanningTools /></>
  }
  ...
  return <>
    <PageHeader eyebrow={localize(`${month.replace('-', localize("年"))}月`)} title={localize("予算・貯蓄目標")} description={localize("確定済み台帳の支出と月間予算を比較します。")}><button className="primary-btn" onClick={() => setShowGoalForm((value) => !value)}><Goal size={17} /> {localize("目標を追加")}</button></PageHeader>
    {notice && <div className="import-notice" role="status">{notice}</div>}
    {showGoalForm && <section className="panel planning-form"><input aria-label={localize("目標名")} value={goalName} onChange={(event) => setGoalName(event.target.value)} placeholder={localize("家族旅行")} /><input aria-label={localize("目標額")} type="number" min="1" value={goalTarget} onChange={(event) => setGoalTarget(event.target.value)} placeholder="1000000" /><input aria-label={localize("目標日")} type="date" value={goalDate} onChange={(event) => setGoalDate(event.target.value)} /><button className="primary-btn" disabled={busy} onClick={() => void createGoal()}>{localize("保存")}</button></section>}
    <section className="budget-layout"><article className="panel budget-panel"><div className="panel-head"><div><h2>{localize("カテゴリー予算")}</h2><p>{budgets.length}{localize("カテゴリー")}</p></div><strong>{yen(totalActual)} / {yen(totalBudget)}</strong></div><div className="planning-form"><select aria-label={localize("予算カテゴリー")} value={budgetAccountId} onChange={(event) => setBudgetAccountId(event.target.value)}><option value="">{localize("カテゴリーを選択")}</option>{expenseAccounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select><input aria-label={localize("月間予算")} type="number" min="0" value={budgetAmount} onChange={(event) => setBudgetAmount(event.target.value)} placeholder="50000" /><button className="secondary-btn" disabled={busy || expenseAccounts.length === 0} onClick={() => void saveBudget()}>{localize("予算を保存")}</button></div>{budgets.length === 0 ? <p className="empty-state">{localize("カテゴリー予算はまだありません。")}</p> : budgets.map((budget, index) => <div className="budget-row" key={budget.categoryAccountId}><div><i style={{ background: palette[index % palette.length] }} /><strong>{budget.categoryName}</strong></div><span>{yen(budget.actualJpy)} <small>/ {yen(budget.budgetJpy)}</small></span><div className="progress"><span style={{ width: `${budget.budgetJpy === 0 ? 100 : Math.min(100, budget.actualJpy / budget.budgetJpy * 100)}%`, background: budget.remainingJpy < 0 ? '#c95b4c' : palette[index % palette.length] }} /></div></div>)}</article><article className="panel goal-panel"><div className="panel-head"><div><h2>{localize("貯蓄目標")}</h2><p>{goals.filter((goal) => goal.status === 'ACTIVE').length}{localize("件進行中")}</p></div><Sparkles size={20} /></div>{goals.length === 0 ? <p className="empty-state">{localize("貯蓄目標はまだありません。")}</p> : goals.map((goal) => { const pace = goalPace(goal); return <div className={`goal-editor goal-editor-${pace.overdue ? 'overdue' : goal.status.toLowerCase()}`} key={goal.id}><strong>{goal.name}</strong><span>{yen(goal.savedJpy)} / {yen(goal.targetJpy)} {localize("・ 目標日")} {goal.targetDate}</span><div className="progress"><span style={{ width: `${Math.min(100, goal.savedJpy / goal.targetJpy * 100)}%` }} /></div><p className="goal-pace">{pace.remaining === 0 ? <b>{localize("目標達成")}</b> : pace.overdue ? <b>{localize("期限超過・残り")} {yen(pace.remaining)}</b> : <>{localize("必要ペース:")} <b>{yen(pace.monthly)} {localize("/ 月")}</b>{localize("・残り")} {pace.months}{localize("か月")}</>}</p><div><input aria-label={localize(`${goal.name}の貯蓄済み金額`)} type="number" min="0" value={goalDrafts[goal.id] ?? ''} onChange={(event) => setGoalDrafts((current) => ({ ...current, [goal.id]: event.target.value }))} /><button className="secondary-btn" disabled={busy} onClick={() => void updateGoal(goal)}>{localize("更新")}</button><button className="text-btn" disabled={busy} onClick={() => void deleteGoal(goal)}>{localize("削除")}</button></div></div> })}</article></section>
    <PlanningTools />
  </>
}
...
function RulesPage({ householdId, accounts }: { householdId: string | null; accounts: readonly AccountDto[] }) {
  ...
  useEffect(() => { void reload().catch(() => setNotice(localize("分類ルールを読み込めませんでした。"))) }, [householdId]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const createRule = async () => {
    ...
    if (!householdId || !name.trim() || (!merchant.trim() && !description.trim()) || !categoryAccountId || !Number.isSafeInteger(parsedPriority) || parsedPriority < 0) {
      setNotice(localize("ルール名、照合条件、カテゴリー、優先度を確認してください。")); return
    }
    setBusy(true); setNotice('')
    try {
      ...
      setName(''); setMerchant(''); setDescription(''); setLabels(''); setTags(''); await reload(); setNotice(localize("分類ルールを保存しました。"))
    } catch { setNotice(localize("分類ルールを保存できませんでした。")) }
    finally { setBusy(false) }
  }
  ...
  const toggleRule = async (rule: ClassificationRuleDto) => {
    ...
    catch { setNotice(localize("ルールの状態を変更できませんでした。")) }
    finally { setBusy(false) }
  }
  const deleteRule = async (rule: ClassificationRuleDto) => {
    if (!householdId) return
    setBusy(true)
    try { await platformClient.deleteClassificationRule(householdId, rule.id); await reload(); setNotice(localize("分類ルールを削除しました。")) }
    catch { setNotice(localize("分類ルールを削除できませんでした。")) }
    finally { setBusy(false) }
  }
  const displayedRules: readonly ClassificationRuleDto[] = platformClient.runtime === 'web' ? [
    { id: 'demo-grocery', householdId: 'demo', name: localize("スーパーを食費へ"), priority: 10, isEnabled: true, merchantContains: localize("成城石井, サミット"), descriptionContains: null, categoryAccountId: 'food', categoryName: localize("食費"), labels: [localize("共通支出")], tags: [localize("日常")], createdAt: '', updatedAt: '' },
    { id: 'demo-utilities', householdId: 'demo', name: localize("電気料金"), priority: 20, isEnabled: true, merchantContains: localize("東京電力"), descriptionContains: localize("電気料金"), categoryAccountId: 'utilities', categoryName: localize("住居・光熱"), labels: [localize("定期")], tags: [], createdAt: '', updatedAt: '' },
    { id: 'demo-legacy', householdId: 'demo', name: localize("旧コンビニ分類"), priority: 90, isEnabled: false, merchantContains: localize("コンビニ"), descriptionContains: null, categoryAccountId: 'food', categoryName: localize("食費"), labels: [], tags: [], createdAt: '', updatedAt: '' },
  ] : rules
  const displayedApplication: LastClassificationApplicationDto | null = platformClient.runtime === 'web' ? {
    transactionId: 'demo', payee: localize("成城石井"), description: null, ruleId: 'demo-grocery', ruleName: localize("スーパーを食費へ"), rulePriority: 10,
    merchantContains: localize("成城石井"), descriptionContains: null, categoryAccountId: 'food', categoryName: localize("食費"), labels: [localize("共通支出")], tags: [localize("日常")], appliedAt: '2026-07-16T09:00:00Z',
  } : lastApplication
  const createFromApplication = () => {
    if (!displayedApplication) return
    setName(localize(`${displayedApplication.payee ?? displayedApplication.ruleName}の分類`))
  ...
  return <><PageHeader eyebrow={localize("自動化")} title={localize("分類ルール")} description={localize("店舗名や摘要に一致する取引へ、説明可能なカテゴリー・ラベル・タグを適用します。")} />
    {notice && <div className="import-notice" role="status">{notice}</div>}
    {platformClient.runtime === 'tauri' && <section className="panel rule-builder"><div className="panel-head"><div><h2>{localize("新しいルール")}</h2><p>{localize("条件と分類結果を設定します。優先度の小さいルールから評価します。")}</p></div></div><div className="rule-form"><label className="rule-field rule-field--name"><span>{localize("ルール名")}</span><input aria-label={localize("ルール名")} value={name} onChange={(event) => setName(event.target.value)} placeholder={localize("コンビニを食費に分類")} /></label><label className="rule-field"><span>{localize("店舗名の条件")}</span><input aria-label={localize("店舗名の条件")} value={merchant} onChange={(event) => setMerchant(event.target.value)} placeholder={localize("店舗名に含む文字")} /></label><label className="rule-field"><span>{localize("摘要の条件")}</span><input aria-label={localize("摘要の条件")} value={description} onChange={(event) => setDescription(event.target.value)} placeholder={localize("摘要に含む文字（任意）")} /></label><label className="rule-field rule-field--category"><span>{localize("分類先カテゴリー")}</span><select aria-label={localize("分類先カテゴリー")} value={categoryAccountId} onChange={(event) => setCategoryAccountId(event.target.value)}><option value="">{localize("カテゴリーを選択")}</option>{expenseAccounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label className="rule-field"><span>{localize("ラベル")}</span><input aria-label={localize("ラベル")} value={labels} onChange={(event) => setLabels(event.target.value)} placeholder={localize("定期購入, 控除対象")} /></label><label className="rule-field"><span>{localize("タグ")}</span><input aria-label={localize("タグ")} value={tags} onChange={(event) => setTags(event.target.value)} placeholder={localize("#家族, #旅行")} /></label><label className="rule-field rule-field--priority"><span>{localize("優先度")}</span><input aria-label={localize("ルール優先度")} type="number" min="0" value={priority} onChange={(event) => setPriority(event.target.value)} /></label><button className="primary-btn rule-save" disabled={busy || expenseAccounts.length === 0} onClick={() => void createRule()}>{localize("ルールを保存")}</button></div></section>}
    <div className="rules-workspace"><section className="panel rule-list"><div className="panel-head"><div><h2>{localize("保存済みルール")}</h2><p>{displayedRules.length}{localize("件・優先度順")}</p></div></div><div className="rule-table-head"><span>{localize("優先")}</span><span>{localize("ルール名・条件")}</span><span>{localize("結果")}</span><span>{localize("状態")}</span></div>{displayedRules.length === 0 ? <p className="empty-state">{localize("分類ルールはまだありません。")}</p> : displayedRules.map((rule) => <article key={rule.id} className={rule.isEnabled ? '' : 'disabled'}><b>{rule.priority}</b><div><strong>{rule.name}</strong><code>{[rule.merchantContains && localize(`店名 ⊃ [${rule.merchantContains}]`), rule.descriptionContains && localize(`摘要 ⊃ [${rule.descriptionContains}]`)].filter(Boolean).join(' AND ')}</code></div><div><strong>{rule.categoryName}</strong><span className="rule-chips">{rule.labels.map((label) => <span key={`l-${label}`}>{label}</span>)}{rule.tags.map((tag) => <span key={`t-${tag}`}>#{tag}</span>)}</span></div><button role="switch" aria-checked={rule.isEnabled} className="rule-switch" disabled={busy || platformClient.runtime === 'web'} onClick={() => void toggleRule(rule)}><span />{rule.isEnabled ? localize("有効") : localize("無効")}</button>{platformClient.runtime === 'tauri' && <button className="text-btn" disabled={busy} onClick={() => void deleteRule(rule)}>{localize("削除")}</button>}</article>)}</section><aside className="panel rule-explanation"><div className="panel-head"><div><h2>{localize("なぜ一致したか")}</h2><p>{localize("最後に適用した分類")}</p></div><FileCheck2 size={18} /></div>{displayedApplication ? <><dl><div><dt>{localize("取引")}</dt><dd>{displayedApplication.payee ?? displayedApplication.description ?? displayedApplication.transactionId}</dd></div><div><dt>{localize("一致条件")}</dt><dd><code>{[displayedApplication.merchantContains && localize(`店名 ⊃ [${displayedApplication.merchantContains}]`), displayedApplication.descriptionContains && localize(`摘要 ⊃ [${displayedApplication.descriptionContains}]`)].filter(Boolean).join(' AND ') || localize("保存済み条件")}</code></dd></div><div><dt>{localize("採用ルール")}</dt><dd>{displayedApplication.ruleName}{displayedApplication.rulePriority == null ? '' : localize(`（優先度 ${displayedApplication.rulePriority}）`)}</dd></div><div><dt>{localize("結果")}</dt><dd>{[displayedApplication.categoryName, ...displayedApplication.labels, ...displayedApplication.tags.map((tag) => `#${tag}`)].join(' ・ ')}</dd></div><div><dt>{localize("適用日時")}</dt><dd>{new Date(displayedApplication.appliedAt).toLocaleString('ja-JP')}</dd></div></dl><p>{localize("ルール適用履歴から読み込んだ結果です。信頼度スコアによる自動確定は行いません。")}</p><button className="primary-btn" disabled={platformClient.runtime === 'web'} onClick={createFromApplication}>{localize("修正からルールを作成")}</button></> : <div className="rule-explanation-empty"><p>{localize("まだ分類ルールの適用履歴がありません。")}</p><small>{localize("取引詳細からルールを適用すると、採用条件と結果がここに残ります。")}</small></div>}</aside></div>
  </>
}
...
function AccountEditor({ householdId, account, members, onChanged, setNotice }: { householdId: string; account: AccountDto; members: readonly HouseholdMemberDto[]; onChanged: () => Promise<void>; setNotice: (notice: string) => void }) {
  ...
  const displayedName = name === account.name ? canonicalAccountName(account, text) : name
  ...
  const [busy, setBusy] = useState(false)
  const rename = async () => { if (!name.trim()) return; setBusy(true); try { await platformClient.renameAccount({ householdId, accountId: account.id, name: name.trim() }); await onChanged(); setNotice(localize("口座名を更新しました。")) } catch { setNotice(localize("口座名を更新できませんでした。")) } finally { setBusy(false) } }
  const updateOwnership = async () => {
    const ownershipKind: AccountOwnershipKindDto = owner === 'HOUSEHOLD' ? 'HOUSEHOLD' : 'MEMBER'
    setBusy(true)
    try { await platformClient.updateAccountOwnership({ householdId, accountId: account.id, ownershipKind, ownerMemberId: owner === 'HOUSEHOLD' ? null : owner, visibility }); await onChanged(); setNotice(localize("口座の所有者と共有区分を更新しました。")) }
    catch { setNotice(localize("口座の所有者と共有区分を更新できませんでした。")) }
    finally { setBusy(false) }
  }
  const archive = async () => { setBusy(true); try { await platformClient.archiveAccount({ householdId, accountId: account.id }); await onChanged(); setNotice(localize("未使用の口座をアーカイブしました。")) } catch { setNotice(localize("この口座は台帳・取込・予算で使用中、または必須口座のためアーカイブできません。")) } finally { setBusy(false) } }
  const ownershipChanged = owner !== (account.ownerMemberId ?? 'HOUSEHOLD') || visibility !== account.visibility
  return <div className="account-editor"><span>{accountKindLabel(account.accountKind, text)} / {accountSubtypeLabel(account.accountSubtype, text)}</span><input aria-label={localize(`${account.name}の口座名`)} value={displayedName} onChange={(event) => setName(event.target.value)} /><label>{localize("所有者")}<select aria-label={localize(`${account.name}の所有者`)} value={owner} onChange={(event) => { const next = event.target.value; setOwner(next); if (next === 'HOUSEHOLD') setVisibility('SHARED') }}><option value="HOUSEHOLD">{localize("世帯共有")}</option>{members.filter((member) => member.status === 'ACTIVE' || member.id === account.ownerMemberId).map((member) => <option key={member.id} value={member.id}>{member.displayName}{member.status === 'ARCHIVED' ? localize("（アーカイブ済み）") : ''}</option>)}</select></label><label>{localize("共有区分")}<select aria-label={localize(`${account.name}の共有区分`)} disabled={owner === 'HOUSEHOLD'} value={visibility} onChange={(event) => setVisibility(event.target.value as AccountVisibilityDto)}><option value="SHARED">{localize("共有")}</option>{owner !== 'HOUSEHOLD' && <option value="PERSONAL">{localize("個人")}</option>}</select></label><div className="account-classification"><span>{account.ownerMemberName ?? localize("世帯共有")}</span><span>{account.visibility === 'SHARED' ? localize("共有") : localize("個人")}</span></div><button className="secondary-btn" disabled={busy || name.trim() === account.name} onClick={() => void rename()}>{localize("名前を保存")}</button><button className="secondary-btn" disabled={busy || !ownershipChanged} onClick={() => void updateOwnership()}>{localize("区分を保存")}</button><button className="text-btn" disabled={busy} onClick={() => void archive()}>{localize("アーカイブ")}</button></div>
}
...
function SettingsPage({ householdId, accounts, members, preferences, onPreferencesChanged, onAccountsChanged }: { householdId: string | null; accounts: readonly AccountDto[]; members: readonly HouseholdMemberDto[]; preferences: DashboardPreferencesDto; onPreferencesChanged: (change: DashboardPreferenceChange) => void; onAccountsChanged: () => Promise<void> }) {
  ...
  const createAccount = async () => {
    if (!householdId || !accountName.trim()) { setAccountNotice(localize("口座名を入力してください。")); return }
    ...
    catch { setAccountNotice(localize("口座を追加できませんでした。名前と種類を確認してください。")) }
    finally { setAccountBusy(false) }
  }

  const createBackup = async () => {
    if (passphrase.length < 12) { setNotice(localize("12文字以上のパスフレーズを入力してください。")); return }
    if (passphrase !== confirmation) { setNotice(localize("パスフレーズが一致しません。")); return }
    setBusy(true); setNotice('')
    try {
      const result = await platformClient.createBackup(passphrase)
      if (!result) return
      setPassphrase(''); setConfirmation('')
      setNotice(localize(`Portable v${result.formatVersion} ・ ${result.entryCount}件・${(result.plaintextBytes / 1024 / 1024).toFixed(1)} MB の暗号化バックアップを作成しました。`))
    } catch {
      setNotice(localize("バックアップを作成できませんでした。保存先とパスフレーズを確認してください。"))
    } finally { setBusy(false) }
  }

  const restoreBackup = async () => {
    if (restorePassphrase.length < 12) { setRestoreNotice(localize("バックアップ作成時の12文字以上のパスフレーズを入力してください。")); return }
    if (restorePassphrase !== restoreConfirmation) { setRestoreNotice(localize("復元用パスフレーズが一致しません。")); return }
    ...
    try {
      ...
      setRestoreNotice(localize(`Portable v${result.formatVersion} の復元準備が完了しました。安全に再起動します…`))
    } catch {
      setRestoreNotice(localize("バックアップを復元できませんでした。ファイルとパスフレーズを確認してください。現在のデータは変更されていません。"))
    ...
    try {
      ...
      setRestoreNotice(localize("復元準備は完了しています。復元を適用するため、KakeFlowを終了してもう一度起動してください。"))
      setRestoreBusy(false)
    }
  }

  return <><PageHeader eyebrow={localize("ローカルデータ")} title={localize("設定")} description={localize("口座、表示環境、バックアップを管理します。")} /><section className="panel preferences-panel"><div className="panel-head"><div><h2>{localize("環境設定")}</h2><p>{localize("言語、テーマ、画面密度はこの端末に保存されます。")}</p></div></div><div className="preference-row"><span><strong>{localize("言語")}</strong><small>Language / Ngôn ngữ</small></span><div className="settings-segment" role="group" aria-label={localize("言語")}>{([['ja', localize("日本語")], ['en', 'English'], ['vi', 'Tiếng Việt']] as const).map(([value, label]) => <button key={value} className={locale === value ? 'active' : ''} aria-pressed={locale === value} onClick={() => setLocale(value)}>{localize(label)}</button>)}</div></div><div className="preference-row"><span><strong>{localize("テーマ")}</strong><small>{localize("外観")}</small></span><div className="settings-segment" role="group" aria-label={localize("テーマ")}>{([['LIGHT', localize("ライト")], ['DARK', localize("ダーク")], ['SYSTEM', localize("システム")]] as const).map(([value, label]) => <button key={value} className={preferences.theme === value ? 'active' : ''} aria-pressed={preferences.theme === value} onClick={() => onPreferencesChanged({ theme: value })}>{localize(label)}</button>)}</div></div><div className="preference-row"><span><strong>{localize("密度")}</strong><small>{localize("一覧とカードの間隔")}</small></span><div className="settings-segment" role="group" aria-label={localize("画面密度")}>{([['COMFORTABLE', localize("標準")], ['COMPACT', localize("コンパクト")]] as const).map(([value, label]) => <button key={value} className={preferences.density === value ? 'active' : ''} aria-pressed={preferences.density === value} onClick={() => onPreferencesChanged({ density: value })}>{localize(label)}</button>)}</div></div></section><section className="panel account-settings"><div className="panel-head"><div><h2>{localize("口座・カテゴリー")}</h2><p>{localize("銀行、ウォレット、カード、収入・支出カテゴリーを管理します。")}</p></div></div><p className="account-visibility-note">{localize("「個人」はこの端末内の整理区分であり、閲覧制限やアクセス制御ではありません。")}</p>{platformClient.runtime === 'tauri' && householdId ? <><div className="planning-form account-create-form"><input aria-label={localize("新しい口座名")} value={accountName} onChange={(event) => setAccountName(event.target.value)} placeholder={localize("ゆうちょ銀行")} /><select aria-label={localize("口座種別")} value={accountKind} onChange={(event) => { const kind = event.target.value as AccountDto['accountKind']; setAccountKind(kind); setAccountSubtype(subtypes[kind][0]) }}>{Object.keys(subtypes).map((kind) => <option key={kind} value={kind}>{accountKindLabel(kind as AccountDto['accountKind'], text)}</option>)}</select><select aria-label={localize("口座サブタイプ")} value={accountSubtype} onChange={(event) => setAccountSubtype(event.target.value as AccountDto['accountSubtype'])}>{subtypes[accountKind].map((subtype) => <option key={subtype} value={subtype}>{accountSubtypeLabel(subtype, text)}</option>)}</select><select aria-label={localize("新しい口座の所有者")} value={accountOwner} onChange={(event) => { const next = event.target.value; setAccountOwner(next); if (next === 'HOUSEHOLD') setAccountVisibility('SHARED') }}><option value="HOUSEHOLD">{localize("世帯共有")}</option>{members.filter((member) => member.status === 'ACTIVE').map((member) => <option key={member.id} value={member.id}>{member.displayName}</option>)}</select><select aria-label={localize("新しい口座の共有区分")} disabled={accountOwner === 'HOUSEHOLD'} value={accountVisibility} onChange={(event) => setAccountVisibility(event.target.value as AccountVisibilityDto)}><option value="SHARED">{localize("共有")}</option>{accountOwner !== 'HOUSEHOLD' && <option value="PERSONAL">{localize("個人")}</option>}</select><button className="primary-btn" disabled={accountBusy} onClick={() => void createAccount()}>{localize("口座を追加")}</button></div><div className="account-list">{accounts.map((account) => <AccountEditor key={account.id} householdId={householdId} account={account} members={members} onChanged={onAccountsChanged} setNotice={setAccountNotice} />)}</div>{accountNotice && <p role="status">{accountNotice}</p>}</> : <p className="empty-state">{localize("口座管理はデスクトップ版で利用できます。")}</p>}</section><section className="panel settings-panel"><div><h2>{localize("暗号化バックアップ")}</h2><p>{localize("SQLCipher台帳と暗号化済み原本を、認証付きアーカイブに保存します。パスフレーズを失うと復元できません。")}</p></div><div className="backup-form"><label htmlFor="backup-passphrase">{localize("パスフレーズ")}</label><input id="backup-passphrase" type="password" autoComplete="new-password" value={passphrase} onChange={(event) => setPassphrase(event.target.value)} placeholder={localize("12文字以上")} /><label htmlFor="backup-confirmation">{localize("パスフレーズを確認")}</label><input id="backup-confirmation" type="password" autoComplete="new-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /><button className="primary-btn" disabled={busy || platformClient.runtime !== 'tauri'} onClick={() => void createBackup()}>{busy ? localize("データを固定中…") : localize("バックアップを作成")}</button>{platformClient.runtime === 'web' && <small>{localize("デスクトップ版で利用できます。")}</small>}{notice && <p role="status">{notice}</p>}</div></section><section className="panel settings-panel restore-panel"><div><h2>{localize("バックアップから復元")}</h2><p><strong>{localize("注意:")}</strong> {localize("現在の台帳と原本は、選択したバックアップの内容に置き換わります。復元前に現在のバックアップを作成してください。置き換えの最終確認はOSのダイアログで行います。")}</p></div><div className="backup-form"><label htmlFor="restore-passphrase">{localize("復元用パスフレーズ")}</label><input id="restore-passphrase" type="password" autoComplete="off" value={restorePassphrase} onChange={(event) => setRestorePassphrase(event.target.value)} placeholder={localize("バックアップ作成時のパスフレーズ")} /><label htmlFor="restore-confirmation">{localize("復元用パスフレーズを確認")}</label><input id="restore-confirmation" type="password" autoComplete="off" value={restoreConfirmation} onChange={(event) => setRestoreConfirmation(event.target.value)} /><button className="danger-btn" disabled={restoreBusy || platformClient.runtime !== 'tauri'} onClick={() => void restoreBackup()}>{restoreBusy ? localize("バックアップを検証中…") : localize("バックアップを選択して復元")}</button>{platformClient.runtime === 'web' && <small>{localize("復元はデスクトップ版で利用できます。")}</small>}{restoreNotice && <p role="status">{restoreNotice}</p>}</div></section></>
}
...
function Onboarding({ onCreated, onCancel }: { onCreated: (household: HouseholdDto) => void; onCancel?: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!trimmed) {
      setError(localize("世帯名を入力してください。"))
      return
    }
    setBusy(true)
    setError('')
    try {
      ...
      setError(localize("世帯を作成できませんでした。もう一度お試しください。"))
    } finally {
      setBusy(false)
    }
  }

  return <div className="onboarding-backdrop"><section className="onboarding-card" role="dialog" aria-modal="true" aria-labelledby="onboarding-title">{onCancel && <button className="icon-btn onboarding-close" aria-label={localize("閉じる")} onClick={onCancel}><X size={18} /></button>}<div className="brand-mark"><Leaf size={22} /></div><p>{localize("KakeFlowへようこそ")}</p><h1 id="onboarding-title">{localize("家計簿をはじめましょう")}</h1><span>{localize("データはこの端末で暗号化して保存されます。")}</span><form onSubmit={(event) => void submit(event)}><label htmlFor="household-name">{localize("世帯名")}</label><input id="household-name" autoFocus maxLength={80} value={name} onChange={(event) => setName(event.target.value)} placeholder={localize("例：田中家")} />{error && <small role="alert">{error}</small>}<button className="primary-btn" disabled={busy}>{busy ? localize("作成中…") : localize("安全な家計簿を作成")}</button></form></section></div>
}
...
function IcloudDriveInboxSettingsPanel({ householdId }: { readonly householdId: string | null }) {
  ...
  useEffect(() => {
    ...
    void platformClient.listWatchedFolders(householdId)
      ...
      .catch(() => { if (active) setNotice(localize("iCloud Drive Inbox の接続状態を読み込めませんでした。")) })
    return () => { active = false }
  }, [householdId])
  const connect = async () => {
    if (!householdId) return
    ...
    try {
      ...
      await platformClient.scanWatchedFolder(householdId, selected.id)
      setNotice(localize("iCloud Drive の同期済みローカルフォルダーを永続 Inbox に接続しました。"))
    } catch {
      setNotice(localize("iCloud Drive フォルダーを接続できませんでした。macOS または Windows の iCloud Drive がローカルに同期済みか確認してください。"))
    } finally { setBusy(false) }
  }
...
function App() {
  ...
  const activeAttributionLabel = activeAttributionScope.kind === 'HOUSEHOLD_COMMON'
    ...
    : activeAttributionScope.kind === 'MEMBER'
      ? householdMembers.find((member) => member.id === activeAttributionScope.memberId)?.displayName ?? localize("不明なメンバー")
      : localize("世帯全体")
  const scopeAppliesToPage = page === 'overview' || page === 'transactions' || page === 'cards' || page === 'investments' || page === 'reports' || page === 'recurring'
  ...
  const runGlobalSearch = (query: string) => {
    ...
    globalThis.setTimeout(() => globalThis.dispatchEvent(new CustomEvent('kakeflow:global-search', { detail: { query } })), 0)
  }
