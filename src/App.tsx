...
function ImportPage({ previews, setPreviews, householdId, accounts, summary, onChanged, folderInbox }: { previews: ImportPreview[]; setPreviews: React.Dispatch<React.SetStateAction<ImportPreview[]>>; householdId: string | null; accounts: readonly AccountDto[]; summary: ImportRunCountsDto | null; onChanged: () => void; folderInbox: DurableFolderInboxView }) {
  ...
  const stageImport = async (item: ImportPreview) => {
    ...
    const standardAccountRequirements = {
      'japanese-bank-ledger-v1': { kind: 'ASSET', subtype: 'BANK', message: '銀行CSVの取込先銀行口座を選択してください。' },
      'paypay-history-v1': { kind: 'ASSET', subtype: 'WALLET', message: 'PayPay履歴の取込先ウォレット口座を選択してください。' },
      'rakuten-enavi-v1': { kind: 'LIABILITY', subtype: 'CREDIT_CARD', message: '楽天カード明細の取込先カード口座を選択してください。' },
      'amazon-mastercard-statement-v1': { kind: 'LIABILITY', subtype: 'CREDIT_CARD', message: 'Amazon Mastercard明細の取込先カード口座を選択してください。' },
      'jcb-myjcb-statement-v1': { kind: 'LIABILITY', subtype: 'CREDIT_CARD', message: 'JCB明細の取込先カード口座を選択してください。' },
    } as const
    const standardRequirement = standardAccountRequirements[item.detectedAdapterId as keyof typeof standardAccountRequirements]
  ...
  return <>
    <PageHeader eyebrow="データ取り込み" title="インポート Inbox" description="ファイルから読み取った候補を確認して台帳へ反映します。">
      ...
      <input ref={inputRef} aria-label="CSV、Excel、PDF、レシート画像、ゆうちょ公式ZIPを選択" className="visually-hidden" type="file" accept=".csv,.xlsx,.pdf,.png,.jpg,.jpeg,.zip,text/csv,application/zip,application/x-zip-compressed,application/pdf,image/png,image/jpeg,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" multiple onChange={(event) => { const files = event.currentTarget.files; event.currentTarget.value = ''; if (files) void processFiles(files) }} />
    </PageHeader>
    ...
    <section className="panel import-panel">
      ...
      <button className="drop-zone" onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void processFiles(event.dataTransfer.files) }}><Import size={20} /><span>CSV / Excel / PDF / レシート画像 / ZIPをここにドロップ</span><small>PayPay・銀行・カード・ゆうちょ公式CSV一括ZIP・PNG / JPEGレシート</small></button>
      ...
      {previews.some((item) => ['japanese-bank-ledger-v1', 'paypay-history-v1', 'rakuten-enavi-v1', 'amazon-mastercard-statement-v1', 'jcb-myjcb-statement-v1'].includes(item.detectedAdapterId ?? '') && item.status === 'ready') && <div className="custom-parser-files"><div><strong>取込先口座を明示的に選択</strong><small>ファイル名、カード会社名、既定口座から推測せず、原本ごとに対応する口座を選択します。</small></div>{previews.filter((item) => ['japanese-bank-ledger-v1', 'paypay-history-v1', 'rakuten-enavi-v1', 'amazon-mastercard-statement-v1', 'jcb-myjcb-statement-v1'].includes(item.detectedAdapterId ?? '') && item.status === 'ready').map((item) => {
        const isBank = item.detectedAdapterId === 'japanese-bank-ledger-v1'
        const isWallet = item.detectedAdapterId === 'paypay-history-v1'
        const eligibleAccounts = accounts.filter((account) => isBank ? account.accountKind === 'ASSET' && account.accountSubtype === 'BANK' : isWallet ? account.accountKind === 'ASSET' && account.accountSubtype === 'WALLET' : account.accountKind === 'LIABILITY' && account.accountSubtype === 'CREDIT_CARD')
        const kindLabel = isBank ? '銀行口座' : isWallet ? 'ウォレット口座' : 'カード口座'
        return <div className="custom-parser-file" key={item.id}><span><strong>{item.filename}</strong><small>{item.adapterId} ・ {kindLabel}のみ選択できます。</small></span><select aria-label={`${item.filename}の取込先${kindLabel}`} value={standardImportAccounts[item.id] ?? ''} onChange={(event) => setStandardImportAccounts((current) => ({ ...current, [item.id]: event.target.value }))}><option value="">{kindLabel}を選択</option>{eligibleAccounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></div>
      })}</div>}
...
function App() {
  ...
  const pageContent = {
    ...
    settings: <><SettingsPage householdId={activeHouseholdId} accounts={accounts} members={householdMembers} onAccountsChanged={async () => { if (activeHouseholdId) setAccounts(await platformClient.listAccounts(activeHouseholdId)) }} /><LocalSyncFoundationPanel householdId={activeHouseholdId} /><PortableEvidenceBundlePanel householdId={activeHouseholdId} /><LocalChangePackagePanel householdId={activeHouseholdId} />{platformClient.runtime === 'tauri' && <DelimitedParserProfilesPanel householdId={activeHouseholdId} />}</>,
  }[page]
