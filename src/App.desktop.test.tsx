...
describe('KakeFlow desktop read models', () => {
  beforeEach(() => {
    ...
    nativeInvoke.mockReset().mockImplementation(async (command: string, args?: Record<string, unknown>) => {
      ...
      if (command === 'transaction_ledger_pdf_save') return { fileName: 'kakeflow-transactions-2026-07-01-2026-07-31.pdf', rowCount: 1, pageCount: 2, byteSize: 24_000 }
  ...
  it('persists and forwards the global attribution scope while disclosing household-wide metrics', async () => {
    ...
    await waitFor(() => expect(nativeInvoke).toHaveBeenCalledWith('monthly_household_review_pdf_save', { request: {
      householdId: 'family', accountGroupId: null, attributionScope: memberScope, month: '2026-07', asOf: '2026-07-31',
    } }))
    const monthlyPdfRequest = nativeInvoke.mock.calls.find(([command]) => command === 'monthly_household_review_pdf_save')?.[1]?.request
    expect(monthlyPdfRequest).toEqual(monthlyExportRequest)
    ...
    fireEvent.click(screen.getByRole('button', { name: '取引台帳PDFを保存' }))
    await waitFor(() => expect(nativeInvoke).toHaveBeenCalledWith('transaction_ledger_pdf_save', { request: ledgerCsvRequest }))
    expect(await screen.findByText(/kakeflow-transactions-2026-07-01-2026-07-31\.pdf（1行・2ページ）を保存しました/)).toBeInTheDocument()
  ...
  it('reports canceled and failed transaction ledger PDF saves', async () => {
    const fallback = nativeInvoke.getMockImplementation()!
    ...
    nativeInvoke.mockImplementation(async (command: string, args?: Record<string, unknown>) => {
      if (command === 'transaction_ledger_pdf_save') throw new Error('save failed')
      return fallback(command, args)
    })
    fireEvent.click(screen.getByRole('button', { name: '取引台帳PDFを保存' }))
    expect(await screen.findByText('取引台帳PDFを書き出せませんでした。対象期間とグループを確認してください。')).toBeInTheDocument()
  })
  ...
  it('reports canceled and failed monthly CSV, Excel, and PDF saves without changing the review', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '月次PDFを保存' }))
    expect(await screen.findByText('月次PDFエクスポートをキャンセルしました。')).toBeInTheDocument()

    nativeInvoke.mockImplementation(async (command: string, args?: Record<string, unknown>) => {
      if (command === 'monthly_household_review_pdf_save') throw new Error('save failed')
      return fallback(command, args)
    })
    fireEvent.click(screen.getByRole('button', { name: '月次PDFを保存' }))
  ...
  it.each([
    ...
    },
    {
      ...
      accountId: 'family-bank',
      missing: '三菱UFJ銀行 BizSTATION CSVの取込先銀行口座を選択してください。',
      adapterId: 'mufg-bizstation-all-details-v1',
  ...
  it.each([
    {
      provider: 'SBI',
      filename: 'sbi-trades.csv',
      adapterId: 'sbi-securities-trade-history-v1',
      title: 'SBI証券 取引履歴CSV',
      csv: '約定日,銘柄,取引,預り,約定数量,約定単価,受渡日,受渡金額／決済損益\n2026/07/01,7203 トヨタ自動車 東証,株式現物買,特定,100,2500,2026/07/03,250000',
    },
    {
      provider: 'Rakuten Securities',
      filename: 'rakuten-securities-trades.csv',
      adapterId: 'rakuten-securities-domestic-trade-history-v1',
    ...
    desktop.listAccounts.mockResolvedValue([
      ...
      { id: 'broker-general', name: '証券 一般口座', accountKind: 'ASSET', accountSubtype: 'SECURITIES', currency: 'JPY', ownershipKind: 'HOUSEHOLD', ownerMemberId: null, ownerMemberName: null, visibility: 'SHARED' },
      { id: 'broker-nisa', name: '証券 NISA', accountKind: 'ASSET', accountSubtype: 'SECURITIES', currency: 'JPY', ownershipKind: 'HOUSEHOLD', ownerMemberId: null, ownerMemberName: null, visibility: 'SHARED' },
      { id: 'family-other-expense', name: 'その他', accountKind: 'EXPENSE', accountSubtype: 'OTHER', currency: 'JPY', ownershipKind: 'HOUSEHOLD', ownerMemberId: null, ownerMemberName: null, visibility: 'SHARED' },
      { id: 'family-income', name: '収入', accountKind: 'INCOME', accountSubtype: 'OTHER', currency: 'JPY', ownershipKind: 'HOUSEHOLD', ownerMemberId: null, ownerMemberName: null, visibility: 'SHARED' },
    ])
    ...
    desktop.commitImport.mockResolvedValue({ runId: 'run-investment', postedCount: 0 })
    const fallback = nativeInvoke.getMockImplementation()!
    nativeInvoke.mockImplementation(async (command: string, args?: Record<string, unknown>) => command === 'brokerage_events_import'
      ? { sourceDocumentId: 'document-investment', importedEventCount: 1, importedLegCount: 4 }
      : fallback(command, args))

    const { container } = render(<App />)
    await screen.findByText('生協')
    fireEvent.click(screen.getByRole('button', { name: 'インポート' }))
    fireEvent.change(container.querySelector<HTMLInputElement>('input[type="file"]')!, { target: { files: [new File([csv], filename, { type: 'text/csv' })] } })

    const save = await screen.findByRole('button', { name: '証券取引に保存' })
    expect(screen.getByText(title)).toBeInTheDocument()
    expect(save).toBeDisabled()
    expect(desktop.startImport).not.toHaveBeenCalled()
    const account = screen.getByLabelText(`${filename}の取込先証券口座`)
    expect(within(account).queryByRole('option', { name: '銀行' })).not.toBeInTheDocument()
    fireEvent.change(account, { target: { value: 'broker-nisa' } })
