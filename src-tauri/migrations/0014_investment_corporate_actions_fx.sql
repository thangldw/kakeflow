-- Corporate actions are explicit brokerage events. A ratio is expressed as
-- new units received per old unit surrendered. Rebuild the v1 tables because
-- SQLite cannot extend the original event_type CHECK constraint in place.
CREATE TABLE brokerage_events_v2 (
...
INSERT INTO brokerage_events_v2 (
...
CREATE TABLE brokerage_event_legs_v2 (
    id TEXT PRIMARY KEY NOT NULL,
    brokerage_event_id TEXT NOT NULL REFERENCES brokerage_events_v2(id) ON DELETE CASCADE,
...
INSERT INTO brokerage_event_legs_v2
SELECT * FROM brokerage_event_legs;

DROP VIEW investment_trade_events_v1;
DROP TABLE brokerage_event_legs;
DROP TABLE brokerage_events;
ALTER TABLE brokerage_events_v2 RENAME TO brokerage_events;
ALTER TABLE brokerage_event_legs_v2 RENAME TO brokerage_event_legs;
...
-- Rates are immutable observations with enough provenance to reproduce a
-- reporting conversion.  They do not rewrite native-currency brokerage data.
CREATE TABLE investment_fx_rates (
    id TEXT PRIMARY KEY NOT NULL,
    household_id TEXT NOT NULL REFERENCES households(id) ON DELETE CASCADE,
    rate_date TEXT NOT NULL CHECK (rate_date GLOB '????-??-??'),
    ...
    source_kind TEXT NOT NULL CHECK (source_kind IN (
        'BROKERAGE_STATEMENT', 'PORTFOLIO_SNAPSHOT', 'MANUAL', 'OFFICIAL_REFERENCE'
    )),
    ...
    UNIQUE (household_id, rate_date, base_currency, quote_currency, provider, source_document_id, source_row)
) STRICT;

CREATE INDEX idx_investment_fx_rates_lookup
    ON investment_fx_rates (household_id, base_currency, quote_currency, rate_date DESC);
...
SELECT
    ...
    e.tax_amount,
    e.settlement_amount,
    e.corporate_action_ratio,
    e.target_instrument_code,
    e.target_instrument_name,
    e.target_currency
