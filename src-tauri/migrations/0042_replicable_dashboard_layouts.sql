-- Package schema 4 makes the five template-specific dashboard layouts one
-- authoritative DASHBOARD_PREFERENCES aggregate. Older package rows remain
-- byte-for-byte compatible and deliberately retain their parent-only payload.

DROP TRIGGER trg_applied_change_package_matches_stage;

CREATE TABLE change_packages_v4 (
    package_id TEXT PRIMARY KEY NOT NULL CHECK (length(trim(package_id)) BETWEEN 1 AND 128),
    schema_version INTEGER NOT NULL DEFAULT 1 CHECK (schema_version IN (1,2,3,4)),
...
CREATE TABLE change_package_records_v4 (
    package_id TEXT NOT NULL REFERENCES change_packages_v4(package_id) ON DELETE CASCADE,
...
CREATE TABLE applied_change_packages_v4 (
    package_id TEXT PRIMARY KEY NOT NULL REFERENCES change_packages_v4(package_id) ON DELETE CASCADE,
...
CREATE TABLE sync_replica_entity_heads_v4 (
    household_id TEXT NOT NULL REFERENCES households(id) ON DELETE CASCADE,
    entity_kind TEXT NOT NULL CHECK (entity_kind IN (
      'HOUSEHOLD','HOUSEHOLD_MEMBER','ACCOUNT','TRANSACTION','MONTHLY_BUDGET_PLAN',
      'SAVINGS_GOAL','CLASSIFICATION_RULE','ACCOUNT_GROUP','CARD_SETTLEMENT_MAPPING',
...
INSERT INTO change_packages_v4 SELECT * FROM change_packages;
INSERT INTO change_package_records_v4 SELECT * FROM change_package_records;
INSERT INTO applied_change_packages_v4 SELECT * FROM applied_change_packages;
INSERT INTO sync_replica_entity_heads_v4 SELECT * FROM sync_replica_entity_heads;
...
ALTER TABLE change_packages_v4 RENAME TO change_packages;
ALTER TABLE change_package_records_v4 RENAME TO change_package_records;
ALTER TABLE applied_change_packages_v4 RENAME TO applied_change_packages;
ALTER TABLE sync_replica_entity_heads_v4 RENAME TO sync_replica_entity_heads;
...
-- Repair any incomplete restored graph before the new aggregate becomes authoritative.
DELETE FROM dashboard_template_layouts
WHERE NOT EXISTS(SELECT 1 FROM dashboard_preferences p WHERE p.household_id=dashboard_template_layouts.household_id);
INSERT INTO dashboard_template_layouts(household_id,dashboard_template,widget_order,hidden_widgets,created_at,updated_at)
SELECT p.household_id,t.dashboard_template,
  CASE WHEN t.dashboard_template=p.dashboard_template THEN p.widget_order ELSE t.widget_order END,
  CASE WHEN t.dashboard_template=p.dashboard_template AND t.dashboard_template!='CASH_FLOW' THEN p.hidden_widgets ELSE '[]' END,
  p.created_at,p.updated_at
FROM dashboard_preferences p
JOIN (
 SELECT 'FINANCIAL_OVERVIEW' dashboard_template,'["TREND","SPENDING","RECENT","CARDS"]' widget_order
...
WHERE NOT EXISTS(SELECT 1 FROM dashboard_template_layouts l WHERE l.household_id=p.household_id AND l.dashboard_template=t.dashboard_template);

CREATE VIEW sync_dashboard_preferences_v4_payloads AS
SELECT p.household_id,
 json(json_object(
  ...
  'templateLayouts',json(json_array(
    json_object('dashboardTemplate','FINANCIAL_OVERVIEW','widgetOrder',json(COALESCE((SELECT widget_order FROM dashboard_template_layouts WHERE household_id=p.household_id AND dashboard_template='FINANCIAL_OVERVIEW'),CASE WHEN p.dashboard_template='FINANCIAL_OVERVIEW' THEN p.widget_order ELSE '["TREND","SPENDING","RECENT","CARDS"]' END)),'hiddenWidgets',json(COALESCE((SELECT hidden_widgets FROM dashboard_template_layouts WHERE household_id=p.household_id AND dashboard_template='FINANCIAL_OVERVIEW'),CASE WHEN p.dashboard_template='FINANCIAL_OVERVIEW' THEN p.hidden_widgets ELSE '[]' END))),
    json_object('dashboardTemplate','HOUSEHOLD_LEDGER','widgetOrder',json(COALESCE((SELECT widget_order FROM dashboard_template_layouts WHERE household_id=p.household_id AND dashboard_template='HOUSEHOLD_LEDGER'),CASE WHEN p.dashboard_template='HOUSEHOLD_LEDGER' THEN p.widget_order ELSE '["SPENDING","RECENT","TREND","CARDS"]' END)),'hiddenWidgets',json(COALESCE((SELECT hidden_widgets FROM dashboard_template_layouts WHERE household_id=p.household_id AND dashboard_template='HOUSEHOLD_LEDGER'),CASE WHEN p.dashboard_template='HOUSEHOLD_LEDGER' THEN p.hidden_widgets ELSE '[]' END))),
    json_object('dashboardTemplate','ASSETS_LIABILITIES','widgetOrder',json(COALESCE((SELECT widget_order FROM dashboard_template_layouts WHERE household_id=p.household_id AND dashboard_template='ASSETS_LIABILITIES'),CASE WHEN p.dashboard_template='ASSETS_LIABILITIES' THEN p.widget_order ELSE '["TREND","SPENDING","CARDS","RECENT"]' END)),'hiddenWidgets',json(COALESCE((SELECT hidden_widgets FROM dashboard_template_layouts WHERE household_id=p.household_id AND dashboard_template='ASSETS_LIABILITIES'),CASE WHEN p.dashboard_template='ASSETS_LIABILITIES' THEN p.hidden_widgets ELSE '[]' END))),
    json_object('dashboardTemplate','CARD_RECONCILIATION','widgetOrder',json(COALESCE((SELECT widget_order FROM dashboard_template_layouts WHERE household_id=p.household_id AND dashboard_template='CARD_RECONCILIATION'),CASE WHEN p.dashboard_template='CARD_RECONCILIATION' THEN p.widget_order ELSE '["CARDS","RECENT","TREND","SPENDING"]' END)),'hiddenWidgets',json(COALESCE((SELECT hidden_widgets FROM dashboard_template_layouts WHERE household_id=p.household_id AND dashboard_template='CARD_RECONCILIATION'),CASE WHEN p.dashboard_template='CARD_RECONCILIATION' THEN p.hidden_widgets ELSE '[]' END))),
    json_object('dashboardTemplate','CASH_FLOW','widgetOrder',json(COALESCE((SELECT widget_order FROM dashboard_template_layouts WHERE household_id=p.household_id AND dashboard_template='CASH_FLOW'),CASE WHEN p.dashboard_template='CASH_FLOW' THEN p.widget_order ELSE '["TREND","RECENT","CARDS","SPENDING"]' END)),'hiddenWidgets',json(COALESCE((SELECT hidden_widgets FROM dashboard_template_layouts WHERE household_id=p.household_id AND dashboard_template='CASH_FLOW'),CASE WHEN p.dashboard_template='CASH_FLOW' THEN CASE WHEN EXISTS(SELECT 1 FROM json_each(p.hidden_widgets) WHERE value='TREND') AND EXISTS(SELECT 1 FROM json_each(p.hidden_widgets) WHERE value='RECENT') AND EXISTS(SELECT 1 FROM json_each(p.hidden_widgets) WHERE value='CARDS') THEN '["RECENT","CARDS"]' ELSE (SELECT json_group_array(value) FROM json_each(p.hidden_widgets) WHERE value!='SPENDING') END ELSE '[]' END)))
  )),
  'createdAt',p.created_at,'updatedAt',p.updated_at
 )) payload_json
FROM dashboard_preferences p;

DROP TRIGGER trg_sync_capture_dashboard_insert;
DROP TRIGGER trg_sync_capture_dashboard_update;
DROP TRIGGER trg_sync_capture_dashboard_delete;
CREATE TRIGGER trg_sync_capture_dashboard_insert AFTER INSERT ON dashboard_preferences BEGIN
 ...
 SELECT household_id,'DASHBOARD_PREFERENCES',household_id,'UPSERT',payload_json FROM sync_dashboard_preferences_v4_payloads WHERE household_id=NEW.household_id;
END;
...
WHEN EXISTS(SELECT 1 FROM households WHERE id=OLD.household_id) BEGIN
 INSERT INTO sync_local_change_capture(household_id,entity_kind,entity_id,operation,payload_json)
 VALUES(OLD.household_id,'DASHBOARD_PREFERENCES',OLD.household_id,'DELETE',json(json_object('recordKind','DASHBOARD_PREFERENCES','householdId',OLD.household_id)));
END;
CREATE TRIGGER trg_sync_capture_dashboard_layout_insert AFTER INSERT ON dashboard_template_layouts BEGIN
 INSERT INTO sync_local_change_capture(household_id,entity_kind,entity_id,operation,payload_json)
 SELECT household_id,'DASHBOARD_PREFERENCES',household_id,'UPSERT',payload_json FROM sync_dashboard_preferences_v4_payloads WHERE household_id=NEW.household_id;
END;
CREATE TRIGGER trg_sync_capture_dashboard_layout_update AFTER UPDATE ON dashboard_template_layouts BEGIN
 INSERT INTO sync_local_change_capture(household_id,entity_kind,entity_id,operation,payload_json)
 SELECT household_id,'DASHBOARD_PREFERENCES',household_id,'UPSERT',payload_json FROM sync_dashboard_preferences_v4_payloads WHERE household_id=NEW.household_id;
END;
CREATE TRIGGER trg_sync_capture_dashboard_layout_delete AFTER DELETE ON dashboard_template_layouts BEGIN
 INSERT INTO sync_local_change_capture(household_id,entity_kind,entity_id,operation,payload_json)
 SELECT household_id,'DASHBOARD_PREFERENCES',household_id,'UPSERT',payload_json FROM sync_dashboard_preferences_v4_payloads WHERE household_id=OLD.household_id;
END;

-- Existing preferences changed semantic shape even if their scalar fields did not.
INSERT INTO sync_local_change_capture(household_id,entity_kind,entity_id,operation,payload_json)
SELECT household_id,'DASHBOARD_PREFERENCES',household_id,'UPSERT',payload_json
FROM sync_dashboard_preferences_v4_payloads;
