...
pub fn query_holdings(
    ...
    validate_scope(connection, &request.household_id, request.account_id.as_deref())?;
    let events = read_events(connection, &request.household_id, request.account_id.as_deref(), Some(&request.as_of))?;
...
pub fn query_performance(
    ...
    let events = read_events(connection, &request.household_id, request.account_id.as_deref(), request.date_to.as_deref())?;
    let analysis = analyze(&events);
    let in_period = |date: &str| {
        request.date_from.as_deref().map_or(true, |from| date >= from)
    ...
    for event in events.iter().filter(|event| in_period(&event.event_date)) {
        let total = totals.entry(event.currency.clone()).or_insert_with(|| InvestmentPeriodCurrencyDto {
    ...
    let allocations = analysis.allocations.into_iter().filter(|item| in_period(&item.sold_on)).collect::<Vec<_>>();
    for allocation in &allocations {
        let total = totals.entry(allocation.currency.clone()).or_insert_with(|| InvestmentPeriodCurrencyDto {
    ...
    Ok(InvestmentPerformanceDto {
        ...
        uncovered_sales: analysis.uncovered_sales.into_iter().filter(|item| in_period(&item.sold_on)).collect(),
        skipped_event_ids: analysis.skipped_event_ids,
    })
}
...
fn analyze(events: &[TradeEvent]) -> Analysis {
    ...
    for event in events {
        ...
        let Some(quantity) = event.quantity.filter(|value| value.is_finite() && *value > EPSILON) else {
        ...
        if event.event_type == "BUY" {
            ...
            result.open_lots.entry(key).or_default().push_back(LotState {
...
fn build_positions(
    ...
    let account_names = events.iter().map(|event| (event.account_id.as_str(), event.account_name.as_str())).collect::<BTreeMap<_, _>>();
    lots.values().filter_map(|queue| {
        let first = queue.front()?;
        let quantity = queue.iter().map(|lot| lot.remaining_quantity).sum::<f64>();
        if quantity <= EPSILON { return None; }
        let cost_basis = queue.iter().map(|lot| lot.remaining_quantity * lot.unit_cost).sum::<f64>();
        Some(InvestmentPositionDto {
            account_id: first.account_id.clone(),
            account_name: account_names.get(first.account_id.as_str()).copied().unwrap_or("").to_owned(),
...
fn instrument_key(event: &TradeEvent) -> InstrumentKey {
    ...
    InstrumentKey { account_id: event.account_id.clone(), currency: event.currency.clone(), identity }
}
...
fn read_events(
    ...
    let rows = statement.query_map(params![household_id, account_id, through], |row| Ok(TradeEvent {
        id: row.get(0)?, account_id: row.get(1)?, account_name: row.get(2)?, source_document_id: row.get(3)?, source_row: row.get(4)?, event_type: row.get(5)?, event_date: row.get(6)?, instrument_code: row.get(7)?, instrument_name: row.get(8)?, currency: row.get(9)?, quantity: row.get(10)?, gross_amount: row.get(11)?, fee_amount: row.get(12)?, tax_amount: row.get(13)?,
    })).map_err(db_error)?;
    rows.collect::<Result<Vec<_>, _>>().map_err(db_error)
}

fn validate_scope(connection: &Connection, household_id: &str, account_id: Option<&str>) -> Result<(), InvestmentPerformanceError> {
    if let Some(account_id) = account_id {
        ...
        if valid.is_none() { return Err(InvestmentPerformanceError::Scope); }
    }
    Ok(())
}

fn validate_holdings_request(request: &InvestmentHoldingsRequest) -> Result<(), InvestmentPerformanceError> {
    if request.household_id.trim().is_empty() || !valid_date(&request.as_of) {
        Err(InvestmentPerformanceError::Invalid)
    } else { Ok(()) }
}

fn validate_performance_request(request: &InvestmentPerformanceRequest) -> Result<(), InvestmentPerformanceError> {
    if request.household_id.trim().is_empty()
        || request.date_from.as_deref().is_some_and(|date| !valid_date(date))
        || request.date_to.as_deref().is_some_and(|date| !valid_date(date))
        || request.date_from.as_ref().zip(request.date_to.as_ref()).is_some_and(|(from, to)| from > to)
    { Err(InvestmentPerformanceError::Invalid) } else { Ok(()) }
}

fn valid_date(date: &str) -> bool {
    if date.len() != 10 || date.as_bytes()[4] != b'-' || date.as_bytes()[7] != b'-' { return false; }
    let Ok(year) = date[0..4].parse::<u32>() else { return false; };
    let Ok(month) = date[5..7].parse::<u32>() else { return false; };
    let Ok(day) = date[8..10].parse::<u32>() else { return false; };
    let leap = year % 4 == 0 && (year % 100 != 0 || year % 400 == 0);
    let maximum = match month { 1 | 3 | 5 | 7 | 8 | 10 | 12 => 31, 4 | 6 | 9 | 11 => 30, 2 if leap => 29, 2 => 28, _ => return false };
    day > 0 && day <= maximum
}

fn db_error(_: rusqlite::Error) -> InvestmentPerformanceError { InvestmentPerformanceError::Database }
...
mod tests {
    ...
    fn connection() -> Connection {
        ...
        connection.execute("INSERT INTO households(id,name) VALUES('home','Home')", []).unwrap();
        ...
        connection.execute("INSERT INTO import_runs(id,household_id,status) VALUES('run','home','POSTED')", []).unwrap();
        for index in 1..=4 {
    ...
    fn insert_event(connection: &Connection, id: &str, doc: &str, row: i64, event_type: &str, date: &str, currency: &str, quantity: Option<f64>, gross: f64, fee: f64, tax: f64) {
        connection.execute(
    ...
    fn fifo_allocates_oldest_lots_and_capitalizes_trading_costs() {
        ...
        insert_event(&connection, "buy-1", "doc1", 1, "BUY", "2026-01-01", "JPY", Some(10.0), 1000.0, 10.0, 0.0);
        insert_event(&connection, "buy-2", "doc2", 2, "BUY", "2026-02-01", "JPY", Some(10.0), 2000.0, 0.0, 0.0);
        insert_event(&connection, "sell", "doc3", 3, "SELL", "2026-03-01", "JPY", Some(15.0), 3000.0, 15.0, 0.0);
        let result = query_holdings(&connection, &InvestmentHoldingsRequest { household_id: "home".into(), account_id: None, as_of: "2026-12-31".into() }).unwrap();
    ...
    fn period_query_uses_prior_buys_and_never_combines_currencies() {
        let connection = connection();
        insert_event(&connection, "jpy-buy", "doc1", 1, "BUY", "2025-01-01", "JPY", Some(2.0), 200.0, 0.0, 0.0);
        insert_event(&connection, "jpy-sell", "doc2", 2, "SELL", "2026-03-01", "JPY", Some(1.0), 150.0, 0.0, 0.0);
        insert_event(&connection, "usd-div", "doc3", 3, "DIVIDEND", "2026-03-02", "USD", None, 10.0, 1.0, 2.0);
        let result = query_performance(&connection, &InvestmentPerformanceRequest { household_id: "home".into(), account_id: None, date_from: Some("2026-01-01".into()), date_to: Some("2026-12-31".into()) }).unwrap();
        assert_eq!(result.totals_by_currency.len(), 2);
        let jpy = result.totals_by_currency.iter().find(|item| item.currency == "JPY").unwrap();
        assert!((jpy.realized_pnl - 50.0).abs() < EPSILON);
        let usd = result.totals_by_currency.iter().find(|item| item.currency == "USD").unwrap();
    ...
    fn reports_uncovered_and_unusable_trades_instead_of_inventing_cost_basis() {
        let connection = connection();
        insert_event(&connection, "bad-buy", "doc1", 1, "BUY", "2026-01-01", "JPY", None, 100.0, 0.0, 0.0);
        insert_event(&connection, "sell", "doc2", 2, "SELL", "2026-02-01", "JPY", Some(2.0), 300.0, 0.0, 0.0);
        let result = query_holdings(&connection, &InvestmentHoldingsRequest { household_id: "home".into(), account_id: Some("broker".into()), as_of: "2026-12-31".into() }).unwrap();
