...
import { pdfQaExecutionPolicy, runPdfReportVisualQa } from './pdf-report-visual-qa.mjs'
...
const executionPolicy = pdfQaExecutionPolicy(
  hasPoppler(),
  process.env.KAKEFLOW_REQUIRE_POPPLER === '1',
)

function persistentOutputDirectory() {
  const configured = process.env.KAKEFLOW_PDF_QA_OUTPUT
  if (!configured) return null

  const output = path.resolve(repositoryRoot, configured)
  const relative = path.relative(repositoryRoot, output)
  if (!relative || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
    throw new Error('KAKEFLOW_PDF_QA_OUTPUT must identify a directory inside the repository')
  }
  return output
}
...
describe('PDF report visual QA integration', () => {
  it.skipIf(executionPolicy === 'skip')('renders a deterministic fixture and writes machine plus human review evidence', async () => {
    if (executionPolicy === 'fail') throw new Error('Poppler is required for PDF visual QA')
    ...
    const persistentOutput = persistentOutputDirectory()
    const output = persistentOutput ?? path.join(temporaryRoot, 'qa')
    try {
      if (persistentOutput) await rm(persistentOutput, { recursive: true, force: true })
